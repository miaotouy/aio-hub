...
#### 1.3.2. 命令使用规范 (Command Usage Protocols)

**核心原则：优先使用预设命令，严禁滥用自定义命令。**

- **白名单机制**: 终端环境对执行命令有严格的白名单限制。自定义的、不在项目预设中的复杂命令（如 `npm run lint:fix -- --ext .ts`）会触发人工审批流程，严重阻塞开发效率。
- **AI 行为准则**: AI 在执行代码检查、构建或清理任务时，**必须**优先使用上述 `check` 系列预设命令。
- **禁止行为**: 严禁自行拼接复杂的 CLI 链式命令，除非预设命令确实无法满足需求且已获得明确授权。
...
- **具体模块修改计划**: 针对特定工具（Tool）的修改或重构计划，应存放在该工具目录下的文档目录中。
  - **路径**: `src/tools/{toolId}/docs/Plan/` (桌面端) 或 `mobile/src/tools/{toolId}/docs/Plan/` (移动端)。
