...
> 本文件面向 Codex、Claude Code、Copilot 等 AI 编码智能体。它只记录稳定约束和高风险坑点；版本、脚本、目录、依赖等实时信息请以仓库当前文件为准。
...
- 在 kilo code 等xml工具链应用中，严禁使用 Shell/PowerShell 管道命令或复杂脚本来修改文件**。所有文件读取、修改、写入操作**必须\*\*使用平台提供的专用工具链（如 `read_file`、`apply_diff`、`write_to_file`）。
...
- **提交信息规范**：AI 智能体在生成 Git 提交信息时，须遵循 Conventional Commits 规范，使用【当前用户所使用的语言】撰写 description。
  - 格式：`<type>[optional scope]: <description>`
  - **强制要求**：首行 description 之后**必须**换行，并附带结构化的详细说明（背景/目的 + 变更要点列表），严禁单行提交。格式如下：
  ...
  - 纯文档修改用 `docs`，重构用 `refactor`，配置/依赖/版本用 `chore`，只有真正面向用户的新功能才用 `feat`。
  - 严禁在提交信息中夹带任何 AI 内部状态标记（如 `[Status: Active]` 等）。

- 算了，没有用户的明确允许，禁止AI自己提交，免得提交了之后消息不好改，Error: Unable to Edit Commit Message
  Editing commit messages for non-HEAD commits is not yet supported.了
- 算了，没有用户的明确允许，禁止AI自己提交，免得提交了之后消息不好改，Error: Unable to Edit Commit Message
  Editing commit messages for non-HEAD commits is not yet supported.了

本来不想在规则中写这么硬的，但是不硬AI就要绕过去，我tm都不知道这个逼模型总想绕弯钻空子是什么破习性。

## 3. 命令与包管理

- 在 kilo code 等xml工具链应用中，禁止通过 `execute_command` 运行 `powershell`、`sed`、`awk`、`Get-Content`、`Set-Content` 等命令来读写或修改文件\*\*。命令行工具仅用于运行项目预设脚本（如 `bun run dev`、`check` 等）、依赖管理或 Git 操作。
...
- 除了运行 `check` 或 `check:frontend` 进行类型检查外，在确认改动前须运行 Vite 构建（如桌面端 `bun run build`，移动端对应构建脚本）。因为许多打包期错误（如未使用的资源引用、动态导入路径错误、Vite 插件特有报错、CSS 编译异常等）是单纯的 TypeScript 类型检查（`tsc`）无法覆盖到的。
...
- 插件模块（位于 `plugins/` 目录）是完全独立的 Git 仓库，且该目录已被列入主仓库的 `.gitignore`\*\*。
  - 插件拥有自己独立的 Git 仓库、版本管理和生命周期。
  - 在开发或修改插件时，必须切换到对应的独立插件仓库中进行提交和管理。

### 5.1. VCP (Variable & Command Protocol)

- **VCP** 是一个开源的 AI Agent 运行时中间层（[GitHub: VCPToolBox](https://github.com/lioensky/VCPToolBox)），提供 AI 推理引擎对接、70+ 官方插件执行、持久化记忆系统（TagMemo RAG、元思考链、AgentDream 梦系统）等能力。
- 项目中有两个模块与 VCP 相关：
  - **`tool-calling`**：AIO 内部的工具调用基础设施，实现了 VCP 协议格式（`<<<[TOOL_REQUEST]>>>` 标记、中文书名号参数编码）用于 LLM ↔ 工具的通信。详见 [`src/tools/tool-calling/ARCHITECTURE.md`](src/tools/tool-calling/ARCHITECTURE.md)。
  - **`vcp-connector`**：通过 WebSocket 连接外部 VCP 服务器的桌面工具，提供消息监控、分布式节点注册、工具桥接三大能力。详见 [`src/tools/vcp-connector/ARCHITECTURE.md`](src/tools/vcp-connector/ARCHITECTURE.md)。
- 涉及 VCP 相关开发时，**必须先读取上述两个 ARCHITECTURE.md**，不要自行猜测 VCP 的含义和实现细节。

### 5.2. 模型元数据与媒体生成参数

- 模型元数据规则（`model-metadata` / `DEFAULT_METADATA_RULES` / `getActiveModelProperties`）只用于模型创建、导入、刷新、批量应用预设等“写入模型对象”的阶段。
- `media-generator` 运行时必须只读取模型对象自身的 `mediaGenParams`，不要在运行时用实时元数据规则兜底、合并或覆盖。
- 禁止为了修某个模型适配问题，在运行时从全局元数据规则穿透读取参数规则；否则用户已配置的模型会随着规则变更而隐式改变，容易引入额外 bug。
...
- **防抖保存**: 对于高频触发的配置修改（如窗口拖拽、滑块拖动、文本框实时输入等），**必须**使用 `saveDebounced`，默认延迟 500ms。这能极大减少 IPC 通信频率，防止后端 IO 阻塞。
...
- 输入框发送快捷键默认使用 Ctrl+Enter，可以配置成单 Enter 发送。
