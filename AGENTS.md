...
- 测试由回归风险和可稳定验证的行为驱动，不按文件、组件或实施批次设置数量配额。新增测试前应说明它防止的具体回归；不得为了固化当前实现、满足计划条目或追求覆盖率而照抄实现分支、DOM 结构、样式类名和具体文案编造断言。迁移旧测试时先确认其断言仍代表目标契约，不得把旧实现本身当作正确性依据。
- UI 自动化只承担可确定验证的交互状态、数据写入边界和关键可访问性契约。视觉质量、信息层级、响应式布局、动效手感和整体易用性应在真实目标运行态中走查，不得以 jsdom、快照或“元素存在”类断言代替；低风险展示性 UI 默认使用类型检查、生产构建和真实界面验收即可。
