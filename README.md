...
- **宏系统 (Macro System)**: 在 Prompt 中使用 `{{user}}`, `{{time}}`, `{{clipboard}}`, `{{os}}`, `{{file:ASSET_ID}}` 等动态宏，构建强大的自动化工作流。
...
- **文件上传与引用**: 拖拽或粘贴图片、PDF、TXT 等文件到聊天窗口。文件将被资产管理器统一处理，并可以通过 `{{file:...}}` 宏在 Prompt 中精确引用，实现“总结这份PDF”、“分析这张图表”等高级功能。
