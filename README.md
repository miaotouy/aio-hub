...
- **JSON 格式化**: 可控展开层级的 JSON 格式化工具，在信息密度和可读性之间找到最佳平衡，支持实时格式化、文件拖拽导入和错误提示。
- **Token 计算器**: 精确 Token 计数工具，支持多 Tokenizer 动态加载（GPT-4/Claude/Llama 等），支持多模态 Token 计算（图片），提供 Token 可视化功能。
- **颜色提取器**: 智能图片颜色分析工具，三算法并行分析（Quantize/Vibrant/Average），支持 EyeDropper API 屏幕取色和持久化历史记录。
- **文本差异对比**: 深度集成 Monaco Diff Editor，智能文件分配（拖入两个文件自动分配左右），支持统一 Diff 补丁导出。
- **目录树生成**: Rust 后端高性能遍历（基于 ignore crate），原生支持 `.gitignore`，前端动态二次筛选（显示深度、排除内容）。
- **代码格式化器**: 多语言代码格式化工具，基于 Prettier 引擎，支持 JS/TS/HTML/CSS/JSON/YAML 等。
- **目录清理工具**: 高性能 Rust 后端驱动的目录清理工具，支持灵活的过滤规则（名称模式、文件年龄、大小、目录深度），采用安全清理机制（移至回收站）。
- **符号链接搬迁**: 文件"搬家"工具，移动文件并在原位置创建链接，支持"搬家模式"和"仅创建链接模式"，支持符号链接和硬链接。
- **媒体信息读取**: 专为 AI 生成图片设计的元数据提取器，支持 A1111/Forge WebUI、ComfyUI Workflow、SillyTavern 角色卡等。
- **API 测试器**: 灵活、高效的 HTTP API 测试环境，预设驱动（URL模板、变量、请求头/体），内置变量系统，原生支持 SSE 流式响应处理。
- **LLM 请求检查器**: 本地中间人代理服务器，用于拦截、检查、记录和分析发往远程 LLM API 的 HTTP 请求，支持实时流式处理 (SSE)。
