...
- **动态壁纸** - 支持静态图片和文件夹幻灯片轮播，提供完整的播放控制台，可调节填充模式、缩放、旋转等。
...
- **多类型插件架构**: 支持三种插件类型，满足从轻量级脚本到高性能原生服务的各种需求。
  - **JavaScript 插件**: 轻量级的前端插件，运行在渲染进程，适合 UI 交互、文本处理等任务。
  - **原生插件 (Native)**: 使用 Rust/C++ 编写的高性能后端插件，通过动态链接库 (DLL/SO) 加载到主进程，实现零开销通信。
  - **Sidecar 插件**: 语言无关的独立进程插件，通过 stdio 与主进程通信，适合计算密集型或需要隔离环境的任务。
- **深度 UI 集成**:
  - **可作为独立工具**: 插件可以拥有完整的 Vue 3 UI，像内置工具一样出现在侧边栏，并支持窗口分离。
  - **顶级开发体验**: 开发模式下直接支持 `.vue` 文件组件和 Vite HMR，实现真正的“即改即用”。
- **统一与安全**:
  - **统一服务接口**: 所有类型的插件都无缝集成到应用的服务架构中，通过统一的 `execute` 函数调用。
  - **类型安全与配置系统**: 完整的 TypeScript 支持，并提供基于 `manifest.json` 的强大配置系统，支持自动迁移。
  - **运行时安全**: 原生插件系统包含引用计数机制，确保在更新或禁用插件时应用的稳定性。
- **生态与文档**:
  - **完整开发文档**: 提供详细的[插件开发指南](docs/plugin-development-guide.md)和多个示例仓库，助你快速上手。

### UI 与交互

- **自定义标题栏** - 统一的视觉风格和窗口控制。
- **全局图片查看器** - 内置基于 Viewer.js 的高性能图片查看器，支持所有工具内图片预览。
- **可收缩侧边栏** - 灵活的工具导航。
- **仪表盘主页** - 卡片网格展示所有工具。
- **文件拖放** - 多数工具支持拖放文件处理。
- **全局快捷键** - 快速显示/隐藏主窗口。
...
- **可视化对话树图** - 新增基于 `Vue Flow` 和 `D3.js` 的图形化视图，直观展示对话分支结构，支持力导向和树状两种布局，以及拖拽、嫁接等高级交互。
- **上下文分析器** - 强大的调试工具，可重建并分析任意消息节点的完整 LLM 请求上下文，提供结构化、原始请求和图表等多种视图。
- **消息编辑与控制** - 支持随时编辑已发送的用户或助手消息，或暂时禁用对话树中的任一节点，灵活构建和调整上下文。
...
- **渲染器深度优化** - 持续完善的渲染引擎，新增 **KaTeX 数学公式**、**Mermaid 图表**支持，并允许在支持的模块（如智能体、用户档案设置）中**自定义 Markdown 元素样式**。
...
- **智能切图** - 自动检测空白区域，切割长图提高识别率
- **交互式处理** - 可视化展示切割过程，支持单块重试
- **发送到聊天** - 识别文字可直接导入对话，让 AI 理解图片内容

#### 图片色彩分析器

- **多算法支持** - 支持从图片中提取主色调 (Vibrant) 和量化调色板 (Quantize)。
- **手动取色** - 支持浏览器原生的`EyeDropper`吸管工具，可从屏幕任意位置拾取颜色。
- **资产联动** - 自动将分析的图片作为资产存入资产管理器，并保留分析历史。
- **多种格式复制** - 支持一键复制 HEX, RGB, HSL 等多种格式的颜色代码。
...
- **AI 图片元信息** - 读取 SD WebUI、ComfyUI 等生成的图片元数据
