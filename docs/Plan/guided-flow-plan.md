...
> 状态：Phase 1–3 已实现；升级与知识库迁移的可见步骤已由约 10 步收敛为 3 步，步骤条已改为居中紧凑的点式导航；知识库迁移已完成合成旧数据的真实 Tauri 检测、确认、迁移、最终状态一致性重启验证与独立清理验收，迁移异常重试、完整报告判定、fixture 路径防护和 runner 异常收尾已补齐，并保留已通过 Rust 外部迁移验证的本地旧数据候选集；进程中断恢复、正式旧发布版本溯源与安装包验收待补；Phase 4 待后续接入
>
> 最近更新：2026-07-27
>
...
Guided Flow 和应用生命周期记录是在既有版本上新增的能力，因此不能假设升级前已经存在可比较的 `lastLaunchedVersion`。首次创建 `app-lifecycle.json` 时，缺少该文件或缺少 `lastLaunchedVersion` 统一解释为 **`unknown-baseline`**，不能猜测用户来自某个具体旧版本，也不能把当前版本写回后再用它伪造升级路径。

以 0.7.0 为首次引入版本时，启动判定遵循以下规则：

1. 先读取当前包版本和本地生命周期记录；没有历史基线则得到 `unknown-baseline`；
2. 0.7.0 的本地 manifest 显式使用 `unknownBaselinePolicy: "show-current"`，因此老用户第一次启动可以看到 0.7.0 的当前版本说明；这不是对 0.6.x 或更早版本的历史说明补发；
3. 旧数据迁移不能只依赖应用版本比较。每个迁移 contribution 必须根据旧目录、旧文件、旧数据库或领域元数据的实际存在性和格式进行只读检测；检测到旧数据才进入待迁移状态，没有旧数据就不显示迁移事项；
4. 待处理流程状态成功持久化后，才把 `lastLaunchedVersion` 写为当前版本。此后 0.7.0 会成为下一次启动的可比较基线，升级到 0.7.1 或更高版本时才按正常 `upgrade` 路径处理。

因此，“老用户从未引入 Guided Flow 的版本升级到 0.7.0”和“0.7.0 全新安装”不需要靠猜测旧应用版本来区分：版本说明由 `unknown-baseline` 策略决定，数据迁移由领域事实检测决定。若产品未来确实需要区分两者，应增加独立的安装/数据存在性事实，不得把 `lastLaunchedVersion` 当作安装来源证明。

### 5.2.2 应用版本、生命周期 schema 与领域数据版本分层

本方案明确不引入一个试图代表全仓库所有数据的单一 `dataVersion`。三种版本含义不同，必须分开：

| 版本                                                 | 事实来源                                            | 用途                                        | 迁移责任                               |
| ---------------------------------------------------- | --------------------------------------------------- | ------------------------------------------- | -------------------------------------- |
| 应用版本（`appVersion`）                             | 根 `package.json` / 当前运行包                      | 判断发布版本转换、选择本地 release manifest | 升级流程与发布资源                     |
| 生命周期 schema（`AppLifecycleState.schemaVersion`） | `app-lifecycle.json`                                | 解释生命周期文件自身的字段结构              | AppLifecycleService 在读取时做结构迁移 |
| 领域数据 schema / 格式版本                           | 各配置文件、SQLite schema、领域 manifest 或任务记录 | 解释具体数据的结构和可否迁移                | 所属领域 repository/service            |

`createConfigManager` 的 `version` 只是配置管理器写入的通用字段，不能替代领域迁移协议：它不记录用户从哪个应用版本升级，也不自动执行字段重命名、删除、类型转换或跨文件迁移。需要结构演进的配置必须显式定义 `schemaVersion`（或等价的领域版本）和有序、幂等的迁移步骤；SQLite 等存储继续使用自己的 schema migration。旧 Recall 数据迁移还必须同时保存 `migrationId`、source fingerprint、执行报告和可恢复状态，不能只靠应用版本号判定是否做过。
...
步骤页按用户决策而不是内部函数或数据处理阶段划分。只读检测、字段说明和执行前确认可以在同一页分区呈现；执行与执行后的结构化校验可以在同一页连续呈现。检测、确认、执行、校验和清理仍是独立的业务动作边界，但不要求一一对应为独立页面。常规流程优先控制在 3–6 个可见步骤；超过 6 步时应先合并没有独立决策价值的相邻页面，并在计划中说明保留额外步骤的原因。

步骤导航采用简约点式表达：所有步骤只显示点和连接线，仅当前步骤显示标题，右侧另行显示“第 N 步，共 M 步”。导航必须在弹窗可用宽度内收缩，不得依靠横向滚动容纳标题；完整的非当前步骤标题继续通过可访问性文本提供。
...
- 选中 1 个历史集合，包含 473 条条目、472 个旧向量和 1,537 个标签向量；不混入同一来源中的空集合、后续新增的小集合或 `knowledge_meta.db`、`workspace.json` 等非旧迁移来源文件；
- 该候选集的迁移相关文件共 949 个、约 51.8 MB。来源中条目数比旧向量数多 1，但已验证全部可用旧向量都能保留迁移；
- 已运行 `recall::storage::legacy_import::tests::verifies_external_legacy_directory_copy` 进行外部目录复制验证：主数据与向量状态均为 `completed`，迁移 1 个集合、473 条条目、472 个向量，问题数为 0，并验证重复执行保持幂等；
- 来源目录没有可核验的应用发布版本信息。因此该候选集可用于本地迁移验证，但**不能单独替代** `migration-release` 的发布候选包门禁。该门禁仍需由旧正式发布二进制或其他可复核的版本溯源补齐，再执行真实 Tauri 生命周期、中断恢复和安装包 smoke test。
...
1. **版本概览**：在同一页显示版本跃迁、离线版本说明和待处理 contribution 摘要；
2. **贡献步骤**：按贡献注册顺序插入，但贡献方必须优先合并只读说明页和连续结果页；
3. **完成摘要**：区分说明已读、事项已完成、事项已延后、部分成功和报告入口。

当前知识库迁移 contribution 只贡献 1 个可见步骤：

1. **旧知识库数据迁移**：在步骤内部管理“方案与确认 → 执行迁移 → 校验与清理”子状态；旧数据清理仅在报告完全通过时开放，并保持独立确认和默认保留策略。

因此自动升级且命中知识库迁移时，流程为“版本概览 → 旧知识库数据迁移 → 完成”3 步，纯版本说明为 2 步。这里减少的是顶层页面数量，不合并 Detect、Confirm、Execute、Verify、Cleanup 的领域动作与安全校验。
...
- 已注册 `knowledge-migration` upgrade contribution，并将来源检测、方案与备份确认、执行进度、校验报告和可选清理收敛为单个“旧知识库数据迁移”可见步骤；与版本概览和通用完成页组合后共 3 个可见步骤；
...
- 未经用户确认，不执行不可逆业务数据操作；
- 迁移的任务进度与流程步骤进度分开显示；
- 步骤导航仅显示当前标题，其余步骤使用点位表示，在支持的弹窗宽度内不出现横向滚动；
- 常规流程优先为 3–6 个可见步骤，升级与知识库迁移组合流程当前为 3 步；
- 领域迁移失败或部分成功时，用户能看到明确结果和恢复入口；
- 同一流程不会因为开发环境重复启动而无条件重新执行。
- 在没有生命周期历史基线的 0.7.0 首次引入场景中，老用户与全新安装均不会被伪造为某个旧版本升级；当前版本说明只按 `unknownBaselinePolicy` 展示一次，旧数据迁移只由领域事实检测触发；
- 应用版本、生命周期文件 schema 和各领域数据 schema 分层持久化，任一领域的数据迁移不依赖一个不存在的全局 `dataVersion`。
