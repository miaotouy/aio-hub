...
当前知识库迁移 contribution 只贡献 3 个可见步骤：

1. **迁移方案与确认**：合并来源检测、字段处理预览、备份提示和写入风险确认；
2. **迁移与校验**：在同一页先显示任务进度，执行完成后原位切换到结构化校验报告；
3. **旧数据清理**：仅在报告完全通过时显示，保持独立确认和默认保留策略。

因此自动升级且命中知识库迁移时，流程最多为“版本概览 → 迁移方案与确认 → 迁移与校验 → 旧数据清理 → 完成”5 步；清理条件不满足时为 4 步，纯版本说明为 2 步。这里减少的是页面数量，不合并 Detect、Confirm、Execute、Verify、Cleanup 的领域动作与安全校验。
...
- 常规流程优先为 3–6 个可见步骤，升级与知识库迁移组合流程当前最多 5 步；
