...
| KI-008 | 跨会话全文搜索每次遍历并读取全部会话文件               | 已确认，需设计索引方案 | 中   | P1         |
...
- [ ] 设计并评审持久化索引：优先比较 SQLite FTS（含中文分词/匹配策略）与增量倒排索引，明确会话/Agent 的写入、删除、导入、迁移、损坏重建和取消语义。
...
6. **KI-008 进入 P1 设计阶段**：比较 SQLite FTS 与增量倒排索引，定义写入同步、重建与端到端性能门槛后实施。
