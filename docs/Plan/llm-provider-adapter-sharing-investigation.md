...
- 在共享 Core 新增 Anthropic Messages Provider，覆盖 system、多模态图片与 PDF/Files API 文档、cache control、thinking、web search、函数工具与 tool result；非流式响应与增量 Decoder 统一解析正文、thinking、并行 tool call、停止原因/序列和 cache usage。流式 usage 改为按 Anthropic 协议合并 `message_start` 输入 Token 与 `message_delta` 输出 Token，避免旧实现产生错误总数。
- 桌面与移动 Claude Facade 均改为 canonical DTO + 共享执行器，thinking delta 分别映射到 `onReasoningStream`，旧请求类型、builder、直接 fetch 和 SSE 循环已删除；桌面仅保留模型列表与 URL 提示，移动仅保留 URL 提示和应用类型映射。
- 在共享 Core 新增 Cohere Chat V2 Provider，统一 `/v1` 到 `/v2/chat` 的 URL 校正、图片/采样/thinking/工具请求、非流式 usage/tool call 和 `content-delta`/`message-end` 流式语义；`requestId` 继续只进入 Header，不泄漏到 Cohere JSON body。
...
该里程碑的桌面 OpenAI-Compatible 过渡 `WireRequest` 映射已随批次 7 收口；Android/iOS 真机行为继续并入批次 9 的 Transport 验收。

#### 批次 7：主流文本协议族 I（OpenAI Responses + Anthropic Claude + Cohere，已完成）

完成状态：OpenAI Responses、Anthropic Messages 与 Cohere Chat 已完成共享 Core、桌面/移动接线、旧实现删除和全量验收；桌面 OpenAI-Compatible 也已完成 canonical 收口。批次 7 于 2026-07-15 闭环。
