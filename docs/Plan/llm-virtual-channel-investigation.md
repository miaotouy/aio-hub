...
**不建议在当前 Phase 5 引入虚拟渠道概念**。理由：

1. **需求不明确**：当前没有用户提出跨渠道容灾的强需求。
2. **Phase 4 已完成聚合渠道接入**：New API / Sub2API / OpenCode Go 的基础能力已落地，剩余应按 Phase 5 范围排期。
