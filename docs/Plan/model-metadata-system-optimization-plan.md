...
- 为宽屏和窄屏补充稳定布局及组件测试，包括自定义规则默认视图、内置规则分层折叠和长选项下拉菜单。
...
- v2/v3 导入预览和阻塞诊断。
- 模型刷新失败时正确报告成功/失败渠道。
- 默认视图仅展示用户新增规则；内置规则按分类可折叠展开，并可通过来源筛选检索。
- 桌面宽屏、窄屏和移动端无文本溢出、控件重叠或状态跳动；长选项下拉菜单有受限高度、内部滚动和可用的已选摘要。
