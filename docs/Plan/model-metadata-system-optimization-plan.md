...
> 状态：施工中（批次 1～4 已完成；批次 5 的共享核心、移动端物化、文档和内置目录自动更新已收敛；移动端目录管理 UI 与真实运行态验收待后续收口）
>
> 计划日期：2026-07-16
>
...
2. 内置规则随应用版本自动升级：新增、修改、删除直接生效，无需用户确认。
3. 只有用户修改过（override）、删除过（屏蔽）或自定义的规则保留，不被自动覆盖。
4. 明确所有元数据属性到 `LlmModelInfo` 的写入映射和运行时读取边界。
5. 为模型记录元数据来源、应用版本和受管理字段，支持安全刷新。
6. 统一桌面和移动端的类型、匹配、合并、校验、目录同步与迁移核心。
...
- 不实现在线远程下载、签名校验或服务端推送式目录更新；内置目录随应用安装包发布，只在应用版本升级后重载。
...
- Catalog 只描述 AIO 提供的默认知识，不直接改变模型对象；加载时自动重载为当前应用版本。
- User Rule State 决定设置页和下一次物化使用的有效规则；只持久化用户改动，不保存已接受的目录副本。
...
├── types.ts                 # 通用规则、目录、存储和诊断类型
├── schema.ts                # 持久化/导入/目录结构校验
├── matcher.ts               # 规范化匹配和稳定排序
├── merge.ts                 # 明确的对象/数组/unset 合并语义
├── compiler.ts              # Catalog + 覆盖 + 屏蔽 + 自定义规则 -> 有效规则
├── migration.ts             # v2 -> v3 迁移 + 目录自动重载
├── materialization.ts       # 模型物化与受管字段脱离
...
- `sourceSnapshot` 是当前应用版本的内置目录副本，在加载时由共享核心的 `syncCatalogSnapshot()` 自动重载；它不是用户状态。
- `builtinOverrides` 保存用户修改后的完整内置规则；完整对象便于恢复和审查，不采用隐式深层 Patch。
- `suppressedBuiltinRuleIds` 表示用户删除或明确屏蔽的内置规则。
- `customRules` 只保存用户新增规则，ID 继续使用 `custom-*` 前缀。
- 有效规则由当前目录 `sourceSnapshot.rules` 应用覆盖和屏蔽后，再追加 `customRules` 编译得到；用户改过但上游已删除的内置规则作为用户拥有的规则继续保留。
...
内置目录随应用版本发布，加载时直接切换为当前版本，不做逐字段确认：

| 情况                                             | 结果                                           |
| ------------------------------------------------ | ---------------------------------------------- |
| 内置规则未被用户改动                             | 采用当前版本目录规则（自动新增/修改/删除）     |
| 内置规则被用户改动（存在 override）              | 保留用户版本，不被上游覆盖                     |
| 内置规则被用户删除（存在 suppressed 项）         | 保持删除                                       |
| 用户改过的内置规则被上游删除                     | 作为用户拥有的规则继续生效                     |
| 自定义规则                                       | 原样保留                                       |

同步规则：

1. `sourceSnapshot` 重载为当前应用版本目录，并持久化新 revision；
2. 已不存在于新目录中的屏蔽项被清理；
3. 不保存“用户已接受的目录基线”，因此不产生三方冲突和逐字段选择；目录更新本身不自动改写已保存模型。

`compileActiveRules()` 的输入顺序为：当前目录规则（应用 override、过滤 suppressed）→ 上游已删除但本地改过的规则 → `customRules`。
...
主页面保留紧凑工具栏和规则列表。内置目录随应用版本自动更新，因此不提供“可用目录版本 / 待处理更新 / 查看更新”状态；规则来源筛选（内置、内置已修改、自定义）和覆盖分析保留。

“刷新模型配置”是把当前元数据写入已保存模型的显式动作，与目录自动更新相互独立。
...
内置目录自动更新后不再需要更新预览与逐字段确认：纯内置变化自动生效，用户改动通过 override 和屏蔽项保留。原“查看目录更新”入口、字段级冲突表和批量接受/保留动作已删除。

### 8.3 模型刷新预览

内置目录自动更新只改变规则状态，不立即改写 Profile 模型。需要同步到模型时单独执行“刷新模型配置”：
...
由于 v2 没有记录用户最初使用的内置版本，同 ID 差异无法可靠区分“旧内置值”和“用户修改”。首轮迁移必须按用户修改保留为 override，不能自动覆盖。
...
- 实现目录自动同步：加载时把 `sourceSnapshot` 重载为当前应用版本，并保留用户 override、屏蔽项和自定义规则。
- 桌面 Store 切换到 v3，保留旧 Composable 查询 API 的过渡 Facade。
- 增加损坏配置回退、迁移备份和错误诊断测试。

完成标准：现有配置无损迁移；内置规则的新增、修改、删除随目录版本自动生效，用户改动不被覆盖；暂不改模型对象。
...
- 将“合并最新配置”替换为目录自动同步：加载时重载为当前应用版本，用户改动保留。
- 实现“刷新模型配置”预览，在写入前逐项确认要同步到已保存模型的字段。
- 改造规则编辑器的来源、覆盖、恢复、匹配类型和校验交互。
- 改造导入流程为解析、诊断、预览、确认四步。
- 对目录同步后的来源判定和模型刷新写入确认等高风险状态转换补充必要的组件测试；宽窄屏布局、信息层级和长选项操作体验改由真实 Tauri 运行态走查，不用 DOM 或快照断言代替可用性验收。

完成标准：用户的内置改动和自定义规则不因版本升级丢失，且能明确知道哪些模型字段会被“刷新模型配置”改写。
...
- 内置规则随目录 revision 自动新增、修改、删除。
- 用户 override 在目录升级后保留，屏蔽项保持。
- 上游已删除但用户改过的规则继续生效。
- 目录未变化时同步返回原 Store，不产生无谓写入。
- 自定义规则在同步后原样保留。
...
- 用户能理解内置规则已随版本自动更新，并完成“编辑/恢复内置规则 -> 预览并刷新模型”，且不会误以为目录更新已经改写模型。
...
共享核心建立后增加定向脚本 `test:model-metadata-core`，并在涉及共享核心、迁移、目录同步或物化逻辑的批次运行。若实际改动触及现有 LLM Core 或平台测试范围，再补充运行对应的 `test:llm-core`、桌面定向 Vitest 或移动端定向 Vitest；不再默认以全仓 `test:run` 代替风险相关验证。

## 12. 验收标准

1. 内置目录、内置覆盖、屏蔽项和自定义规则可以被明确区分和独立恢复。
2. 内置规则随应用版本自动更新，无需用户处理更新确认。
3. 用户修改、删除或自定义的规则在目录升级后不被静默覆盖。
...
15. 共享核心、迁移、目录同步和物化等高风险契约的定向测试通过，桌面/移动类型检查和两端生产构建通过，真实运行态 UI 任务走查已记录且不存在阻塞性可用性问题。

## 13. 风险与应对

| 风险                           | 影响                        | 应对                                                        |
| ------------------------------ | --------------------------- | ----------------------------------------------------------- |
| v2 无旧内置版本信息            | 旧默认与用户修改无法区分    | 按用户修改保守保留为 override，不自动覆盖                   |
...
| 目录自动更新与模型刷新被误认为一步 | 用户以为规则更新已改写模型 | UI 将“刷新模型配置”作为独立显式动作，目录更新不自动改写模型 |
...
### 2026-09-14：内置目录自动更新收口（设计按原始意图修正）

背景：批次 4/5 曾把内置目录更新实现为“检测差异 -> 用户逐字段确认 -> 应用”的保守流程，与本计划“分层后纯内置规则随版本自动更新”的原始目标不一致。本次按原始意图收敛：内置目录在加载时自动重载为当前应用版本，用户改动通过 override/屏蔽保留，不再需要更新确认。

已完成：

- 共享核心删除 `diff.ts` 的三方差异与 `applyBuiltinCatalogUpdate()`，新增 `syncCatalogSnapshot()`；`compileActiveRules()` 增加“上游已删除但本地改过的规则继续生效”的语义。
- 桌面端 Store 与移动端 Facade 在加载 v3 配置后调用 `syncCatalogSnapshot()` 并持久化新 revision；删除 `catalogDiffs`、`pendingUpdatesCount`、`applyCatalogUpdate()`。
- 删除设置页“查看目录更新”入口、更新横幅和 `ModelMetadataCatalogUpdateDialog.vue`；保留“刷新模型配置”作为把元数据写入已保存模型的显式动作。
- 共享核心测试改为覆盖目录自动同步：自动新增/修改/删除、override/屏蔽/自定义规则保留、上游删除但本地改过保留、revision 未变时不重写 Store。
- 同步修正架构文档、用户指南与计划表述。
...
bun run build:vite                                        # 通过（既有 externalize / eval / chunk-size 警告）
bun run --cwd mobile build                                # 通过（既有 eval / chunk-size 警告）
```
