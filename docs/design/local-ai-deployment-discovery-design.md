...
用户安装开源 AI 项目时，常得到的是带有 Web UI、Python/Node 环境、模型目录和启动脚本的整合包。用户关心的不是磁盘上有多少个项目文件夹，而是：
...
| 对象           | 关键字段                                                               |
| -------------- | ---------------------------------------------------------------------- |
| `AiDeployment` | `detectorId`、名称、能力分类、项目根、状态、置信度、证据列表、占用摘要 |
| `Runtime`      | 类型、可执行路径、版本、工作目录、关联部署                             |
| `LocalService` | PID、命令行、地址、端口、协议、探测结果、关联运行时                    |
| `ModelAsset`   | 路径、格式、大小、修改时间、可选流式 SHA-256、被引用部署               |
| `Evidence`     | 来源、原始值、强度、采集时间、解释文本                                 |

置信度应由证据组合计算：结构目录或文件名是弱证据；启动命令与项目根相符是中证据；端口归属、HTTP 协议响应、配置文件引用是强证据。只有达到中证据的候选才进入部署计数；弱证据留在“待确认候选”。
...
1. **已知位置扫描**：只从 detector 声明的用户目录、应用数据目录、标准安装目录、PATH 和用户主动选取的根目录开始。
...
4. **端口与协议验证**：获取监听端口到 PID 的归属；仅对 detector 声明的回环端口请求健康检查或模型列表端点。
5. **模型资产摘要**：按格式、目录、大小和修改时间汇总。需要确定重复时才由用户发起流式哈希扫描。
6. **证据聚合**：按物理项目根、运行时路径和 PID 合并候选，输出已确认、已发现未运行、待确认三种状态。

## 6. Detector 注册表

Detector 是声明式规则，不是前端散落的 `if`。每个 detector 至少包含：能力分类、已知根目录、项目标记、运行时/进程标记、候选端口、协议探针和结果解释。
...
| ComfyUI                            | `main.py`、`nodes.py`、`folder_paths.py`、`custom_nodes`；模型根可在项目内或共享目录 | ComfyUI 队列/系统端点，默认 8188 可改 | 首版                     |
| audio.cpp 系列                     | 项目根、模型规格、ASR/TTS/WebUI 启动脚本、项目 venv 启动命令                         | 仅在探测到实际监听端口后验证          | 首版调研后确定具体子类型 |
...
Detector 的命中必须保留原始证据。用户需要能看到“为什么识别为 ComfyUI”，也需要能将误报标为忽略，避免同一目录在后续扫描中反复出现。
...
前端可作为独立工具“本机 AI 环境”，环境设置中只提供扫描根目录、默认排除目录、是否包含 PATH/进程探测等偏好项。资产管理只通过跳转或引用显示关联模型资产，不成为外部部署的真源。

扫描缓存应保存签名结果和目录修改时间，而不是保存模型文件内容；下一次扫描只重新检查变更目录、进程和端口。用户手动选择的额外根目录必须可查看、编辑和移除。
...
- 默认端点仅允许回环地址 `127.0.0.1` 或 `::1`；使用局域网地址须由用户逐次明确确认，首版不提供远端扫描能力。
- 启用或“检测服务”时，先请求 `GET /`，确认响应正文或标识头符合 `Everything HTTP Server`，再允许执行检索；连接失败只显示未就绪，不自动启动 Everything 或修改其 HTTP Server 设置。
- 检索仅用于按文件名和路径定位候选，例如 ComfyUI 的 `main.py`、`nodes.py`、`folder_paths.py`；命中后仍必须在本地读取有限标记并运行 detector 复核，不能仅凭 Everything 索引计数。
- 不读取、保存或上传索引之外的文件内容；也不把 Everything HTTP 的 Basic Auth 密码写入普通应用配置。后续如支持认证，必须先提供系统凭据存储方案。
...
- HTTP 探针只访问 `127.0.0.1`、`::1` 或经用户确认的本地网络地址；请求结果中不持久化 API 密钥或完整响应体。
...
- 实现受预算的已知目录扫描、PATH 检查、进程快照、证据合并和回环 HTTP 探针。
- 在结果中区分“已运行”“已发现未运行”“待确认”，并允许查看证据。

### Phase 2：Windows 端口归属与渠道导入

- 新增监听端口到 PID 的 Windows 适配；将服务和进程可靠关联。
- 对可验证端点提供“导入为渠道”预览，复用已有 LLM 模型列表与连接检查，不自动写入配置。
...
测试应覆盖 detector 的正例、同名误报、符号链接、空模型根、共享模型根、运行但无模型的服务、非 AI Python 进程、端口被非目标进程占用、取消和资源预算到达。真实验收需在 Windows Tauri 运行态用受控目录夹具及真实本地服务完成；普通浏览器不替代进程、端口和 IPC 验证。
...
- 端口归属能力应优先覆盖 Windows，还是在 Phase 1 同时提供 macOS/Linux 的最小实现。
