...
- **详细上下文**: 支持附加数据和堆栈信息
- **可靠前端持久化**: 所有 WebView 的前端日志通过 Rust 端串行写入；初始化阶段、跨日和多窗口写入不会再各自维护有竞态的文件状态
- **前端轮转与保留**: 默认每 512 KiB 归档为 `app-YYYY-MM-DD.HH-mm-ss-SSS.log`，按日志事件日期切换活动文件，并保留最近 30 天

### 后端日志策略

Rust / Tauri 后端日志与前端 `logger.ts` 的文件日志分开保存，但使用同一个应用数据目录中的 `logs` 文件夹。后端活动文件命名为 `backend-YYYY-MM-DD.log`：每次写入都会根据应用配置的时区检查日期，因此应用跨午夜持续运行时会自动切换到新的一天，而不需要重启。

- **即时可读**：每条后端记录写入后都会 `flush`，错误记录、文件轮转和正常退出额外执行数据同步。
- **大小轮转**：单个后端日志文件默认最大 **5 MiB**；超过阈值时归档为 `backend-YYYY-MM-DD.HH-mm-ss-SSS.log`，不会覆盖旧内容。
- **保留期**：仅清理修改时间超过 **30 天** 的 `backend-*.log` 文件；其它日志和用户文件不受影响。
- **高频依赖过滤**：`hyper` 仅保留 WARN 及以上，`hnsw_rs` 仅保留 INFO 及以上，避免第三方噪音淹没应用日志。
...
```typescript
errorHandler.error(error, "自定义的用户友好消息", {
