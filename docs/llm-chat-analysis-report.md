...
- **阶段一：核心体验补完 (基于现有UI)**
  1.  **实现消息 Markdown 渲染 (技术选型: `vue-renderer-markdown`)**:
      ...
      - **核心价值**:
        - **基础支持**: 实现代码块（可切换 Shiki/Monaco）、数学公式、表格、Mermaid 图表等富文本格式。
        - **流式渲染**: 完美契合 LLM 的流式输出，提供极致的实时响应体验。
        - **性能保障**: 其 `viewportPriority` 特性可优先渲染视口内内容，为长对话的流畅性提供保障，部分解决了阶段二的性能问题。
        - **可扩展性**: 支持自定义节点组件，为未来在消息中嵌入更复杂的交互元素（如按钮、表单）提供了可能。
  2.  **实现消息编辑与重发**: 允许用户编辑自己发送的消息，并能触发以新内容重新向模型提问，优化纠错流程。
  3.  **实现分支切换与管理**: 在 UI 上（如消息菜单或侧边栏）提供切换 `activeLeafId` 到不同分支末端的功能。
  4.  **实现节点启用/禁用**: 在消息操作菜单中增加“从上下文中排除/包含”的开关，直接控制 `isEnabled` 状态，实现精确的上下文管理。

- **阶段二：可视化与性能优化**
  1.  **引入图表库与只读视图**: 集成前端图表库（如 `VueFlow`），开发一个独立的“编辑模式”，用只读的方式将对话渲染成树状图，并实现基础的平移缩放。
  2.  **实现无限滚动加载**: 改造消息列表，在主干视图和线性视图中支持长对话的性能优化，解决初始加载慢和内存占用的问题。
  3.  **实现基础图上交互**: 在树状图中，允许用户点击节点执行阶段一已有的操作（切换分支、启用/禁用）。

- **阶段三：高级编辑与上下文注入**
  1.  **实现剪枝与嫁接**: 在可视化编辑器中，实现拖拽式的剪枝和嫁接操作，并更新 `store` 中对应的 action，完成对话的非线性重构。
  2.  **上下文注入 MVP**: 借鉴 SillyTavern 的“世界书”，实现一个简单的关键词->内容注入功能，并提供插入位置选项。

- **阶段四：交互体验与高级功能增强**
  1.  **丰富交互能力**: 借鉴 Cherry Studio，逐步增加**多模态输入（文件上传）**、**快捷命令 (`/`)**、**模型提及 (`@`)**、**消息多选**、**上下文分割线**和**智能粘贴**等高级功能。
  2.  **完善 Agent 系统**: 借鉴 SillyTavern 的“角色卡”，为 `ChatAgent` 增加更多高级配置，如角色专属的提示覆盖、角色笔记等。
