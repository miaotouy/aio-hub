...
- ✅ 采用 **Pinia Store + Runner Composable** 架构（去中心化组合范式）
- ✅ 创建 `smartOcr.store.ts` - Pinia Store 管理所有响应式状态
- ✅ 创建 `useSmartOcrRunner.ts` - 业务逻辑编排层
- ✅ 改造 `smartOcr.service.ts` - 无状态服务层，直接使用底层 Composables
- ✅ 重构 `SmartOcr.vue` - 通过 `storeToRefs` 获取状态，通过 Runner 获取业务方法
...
- ✅ 创建 `useCloudOcrRunner` - 云端 OCR 引擎
- ✅ 重构 `useOcrRunner` - 轻量级编排者
- ✅ 创建 `useImageSlicer` - 图片切割逻辑
- ✅ 完整的元数据和类型定义

12. **GitAnalyzer** - 复杂级工具（完成）

- ✅ 采用 **Pinia Store + Runner Composable** 架构（去中心化组合范式）
- ✅ 创建 `useGitAnalyzerState.ts` - Pinia Store 管理所有响应式状态
- ✅ 创建 `useGitLoader.ts` - 数据获取层（与 Tauri 后端交互）
- ✅ 创建 `useGitProcessor.ts` - 数据处理层（纯函数）
- ✅ 创建 `useGitAnalyzerRunner.ts` - 业务编排层，直接使用状态 Store
- ✅ 创建 `gitAnalyzer.service.ts` - 无状态服务层，直接使用底层 Composables
- ✅ 实现 Agent 友好的高级接口（`getFormattedAnalysis`, `getAuthorCommits`, `getBranchList`）
- ✅ 重构 `GitAnalyzer.vue` - 直接使用 `useGitAnalyzerState` 和 `useGitAnalyzerRunner`
- ✅ 更新辅助 composables（`useCharts.ts`, `useReportGenerator.ts`）
- ✅ 完整的元数据定义

13. **DirectoryJanitor** - 中等复杂度工具（完成）

- ✅ 采用 **状态 Composable + Runner Composable** 架构（去中心化组合范式）
- ✅ 创建 `useDirectoryJanitorState.ts` - 管理所有响应式状态
- ✅ 创建 `useDirectoryJanitorRunner.ts` - 业务编排层，直接使用状态 Composable
- ✅ 创建 `directoryJanitor.service.ts` - 无状态服务层，直接使用底层 Composables
- ✅ 实现 Agent 友好的高级接口：
  - ✅ `scanDirectory()` - 扫描目录并返回格式化结果
  - ✅ `cleanupItems()` - 清理指定的文件和目录
  - ✅ `scanAndCleanup()` - 一步到位的扫描并清理
- ✅ 重构 `DirectoryJanitor.vue` - 直接使用 `useDirectoryJanitorState` 和 `useDirectoryJanitorRunner`
...
### 🎯 去中心化组合 (Decentralized Composition)

**核心理念**: UI 层和 Service 层应作为独立的消费者，自由地从原子化的 Composables 中按需组合功能，而非依赖于预设的"公共逻辑层"或"上下文聚合层"。

#### ❌ 已废除：上下文模式 (Context Pattern)

在项目早期，曾尝试使用"上下文模式"来统一管理复杂工具的状态和业务逻辑：
...
export class OcrContext {
  ...
  imageBlocksMap = ref<Map<string, ImageBlock[]>>(new Map());
  // ... 更多状态
...
1. **预设性抽象**: 假设 UI 和 Service 需要相同的状态和方法，忽略了它们在使用场景、数据流和控制粒度上的真实差异。
2. **紧密耦合**: UI 和 Service 都依赖于同一个"大而全"的上下文，难以独立演化。
3. **违背 Vue 3 范式**: Composition API 的核心思想是"组合"，而非"继承"或"聚合"。
4. **可测试性差**: 上下文类混合了状态、业务逻辑和副作用，单元测试困难。
...
┌─────────────────────────────────────────────────────────────┐
│                       原子化 Composables                      │
│  useImageSlicer | useOcrRunner | useTesseractEngine | ...   │
└─────────────────────────────────────────────────────────────┘
          ↑                                      ↑
          │                                      │
    ┌─────┴──────┐                         ┌────┴─────┐
...
1. **状态管理层**:
   - **简单工具**: 使用 `useState` Composable (如 `useDirectoryJanitorState.ts`)
   - **复杂工具**: 使用 Pinia Store (如 `smartOcr.store.ts`, `useGitAnalyzerState.ts`)

2. **业务逻辑层**:
   - 创建 `useXRunner.ts` Composable
   - 直接使用状态层和底层 Composables
   - 不预设"公共逻辑"，UI 和 Service 各自按需组合
...
3. **UI 层**:
   ...
   const { uploadedImages, isProcessing } = storeToRefs(store);
   const { initialize, runFullOcrProcess } = useSmartOcrRunner();
   ```

4. **Service 层**:
   ```typescript
   // ✅ 正确示例：smartOcr.service.ts
   const { sliceImage } = useImageSlicer();
   const { runOcr } = useOcrRunner();
   // 直接使用底层 Composables，不依赖 Runner
   ```

**优势**：

1. **最大化灵活性**: UI 可以为用户体验选择"渐进式、可中断"的组合方式；Service 可以为性能选择"批量、并发"的组合方式。
2. **符合 Composition API 哲学**: 真正实现了"组合"而非"聚合"。
3. **易于测试**: 每个 Composable 都是独立的、可测试的纯函数或状态管理单元。
4. **清晰的职责边界**: 状态管理、业务逻辑、UI 交互各司其职。

### 🔄 架构演进历程

1. **第一次尝试**: 创建 `OcrContext.ts` Class - ❌ 不符合 Vue 3 范式
2. **第二次尝试**: 将 Class 改为 `useSmartOcr.ts` Composable - ❌ 仍是"伪装的上下文"
3. **最终方案**: Pinia Store + Runner Composable - ✅ 真正的去中心化组合

**经验教训**：

> "不要预设一个'公共逻辑层'。UI 会自己组织，Service 会自己组织。它们应该是两个独立的'消费者'，自由地从最底层的、原子化的 composables 中按需组合。" — 项目架构重构总结
