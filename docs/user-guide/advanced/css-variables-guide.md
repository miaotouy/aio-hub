...
| `--sidebar-bg`            | 侧边栏背景                 |
| `--ui-blur`               | 当前全局毛玻璃模糊强度     |
| `--chat-message-bg-blur`  | 聊天区域层使用的实际模糊强度 |
| `--glass-region-blur`      | 非滚动区域共享的实际模糊强度；无有效壁纸或系统窗口模糊时为 `0px` |
| `--card-opacity`          | 卡片透明度数值             |

如果要写可维护的 HTML，优先让 AI 在 CSS 中使用变量名本身，例如 `background: var(--card-bg)`；只有当模型需要知道实际颜色值时，再用宏读取。

> 应用壳层开发请遵循区域玻璃契约：在非滚动父容器使用 `.glass-region` 统一提供模糊，区域内元素只提供半透明背景。仅不属于区域层的浮层或小控件可使用独立 `backdrop-filter`，并需要 `/* glass-overlay */` 注释。不要将该属性放在滚动容器、虚拟列表项或大面积动画元素上。
