...
1. **查询预处理**：对原始查询进行清洗、分词、停用词过滤，并尝试在全局标签池中进行语义匹配。
2. **上下文投射**：如果是 `vector` 或 `hybrid` 引擎，系统会将最近 3 轮的对话向量进行加权聚合（Context Projection），解决语义漂移问题。
3. **多路检索**：根据选定的引擎（如 `Blender` 融合引擎）同时发射字面量、语义和引力信号。
4. **结果聚合与去重**：结合历史检索记录，利用时间衰减算法对结果进行重排序。
...
| 参数                 | 说明                                 | 默认值 |
| :------------------- | :----------------------------------- | :----- |
| **上下文窗口**       | 检索时参考的最近用户消息轮数         | `1`    |
| **查询衰减 (Decay)** | 历史向量在当前查询中的权重衰减系数   | `0.8`  |
| **结果聚合**         | 是否开启跨轮次结果加权聚合           | `开启` |
| **最大召回字数**     | 注入上下文的总字符上限（0 为不限制） | `4000` |
