...
在 Agent 编辑器的 **「知识库 (Knowledge) -> Knowledge 资料访问权限」** 面板中编辑 `knowledgeAccess`：是否启用、允许的资料库、是否允许省略资料库范围、是否允许继续读取文档，以及是否允许研究任务。检索策略、预算和过滤条件属于单次工具调用参数，不会写入 Agent 的资料库授权。
