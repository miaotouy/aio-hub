...
### 7. Knowledge 工具（不属于上下文处理器）

Knowledge 不扫描 `【kb】`、`【knowledge】` 或其他检索占位符。Agent 需要资料时调用 Knowledge 工具，用户也可以在聊天输入区显式选择资料库；工具结果以可见节点和结构化引用回传。权限和调用参数见 [Knowledge Agent 集成](../../knowledge-base/agent-integration)。
...
关闭**世界书处理器**和**Recall 处理器**的开关即可。Knowledge 不属于上下文管道；未调用 Knowledge 工具时不会发生检索。
