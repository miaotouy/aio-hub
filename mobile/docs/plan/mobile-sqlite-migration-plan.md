...
└───────────┬──────────────────────────────┬─────────────┘
            │ 简单配置                               │ 复杂/海量数据
┌───────────▼────────────┐              ┌───▼────────────┐
│     ConfigManager      │              │  Tauri Command │
│ (app_settings.json 等)  │              │  (Rust SQLite) │
└────────────────────────┘              └────────┬───────┘
                                                 │ 读写
                                        ┌────────▼───────┐
                                        │   llm_chat.db  │
...
const SCHEMA_SQL = `-- 见 3.7 节完整 DDL，此处省略 50+ 行`;
const SCHEMA_VERSION = 1;
...
- **`loadSession`**：改为 `SELECT * FROM chat_messages WHERE session_id = ? ORDER BY sibling_order`，在前端用 O(N) 算法将扁平行组装成 `Record<string, ChatMessageNode>` 树状结构（见第 5 节坑点 2）。
- **`persistSession`**：使用事务（Transaction），`INSERT OR REPLACE` 会话行，然后批量 `INSERT OR REPLACE` 所有消息节点行。
- **`deleteSession`**：由于外键级联删除（`ON DELETE CASCADE`），只需 `DELETE FROM chat_sessions WHERE id = ?`，对应消息自动清理。
...
8. **附件存储预案**：当前移动端类型定义不含 `attachments`，但桌面端已支持多模态消息（图片/文件附件）。当移动端需要支持时，建议新建独立的 `chat_attachments` 表（通过 migration v2 添加），而非塞入 `metadata_json`：
   ```sql
   -- Migration v2: 附件表
   CREATE TABLE IF NOT EXISTS chat_attachments (
     id          TEXT PRIMARY KEY,
     message_id  TEXT NOT NULL,
     asset_type  TEXT NOT NULL,     -- 'image' | 'file' | 'audio'
     file_path   TEXT NOT NULL,     -- AppData 相对路径
     mime_type   TEXT,
     file_size   INTEGER,
     created_at  TEXT NOT NULL,
   ...
   ```
   这样既保持了当前 Schema 的简洁性，又为未来扩展留出了清晰的 migration 路径。
