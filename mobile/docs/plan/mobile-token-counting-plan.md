...
> 状态：计数服务、展示接入和文本上下文预算编排已完成；非文本成本、真机性能和 iOS 验证待完成
> 日期：2026-07-14；2026-07-27 更新上下文管线状态与移动端 Chat 边界
> 已实施消费架构见 [`llm-chat/ARCHITECTURE.md`](../../src/tools/llm-chat/ARCHITECTURE.md) 与 [`agent-manager/ARCHITECTURE.md`](../../src/tools/agent-manager/ARCHITECTURE.md)；本文继续维护 Token 管线接入和设备验证门禁。

## 1. 背景

移动端当前已在预设编辑器、聊天输入区、上下文管线和发送前快照中使用 Rust `o200k_base` 计数；IPC 失败时才回退到字符估算。文本 Token limiter 已按预设优先、最新历史优先的顺序参与预算裁剪和部分截断。
...
当前聊天输入区、上下文 Token limiter、发送前管线快照和响应后统计已经复用同一 IPC 封装：

- 发送前估算当前完整上下文占比；
- 发送后的最终文本保存消息级估算和上下文风险快照；
- 响应含 usage 时保存实际统计，并优先用于该次请求的展示；
- 不根据渠道类型切换 tokenizer。

当前状态：

- [x] 在预设注入之后、最终消息格式化之前接入文本 Token 预算计算；
- [x] 完成 `token-limiter` 的文本预算、历史消息保留和部分截断逻辑；
- [x] 保留发送后最终计数和消息级结果，用于 usage 缺失时的展示与上下文快照；
- [x] 明确附件和非文本多模态开销不由通用文本 tokenizer 隐式覆盖。

附件和多模态 Token 不属于首个 Rust o200k command 的范围，需要时按实际模型和渠道请求结构追加独立估算。工具 schema 只在未来确实设计移动端工具调用时处理，不属于当前 Chat 完整性或 Token 管线完成门禁。

## 7. 性能与验证

以下项目构成 Token 能力的技术验证与平台门禁；固定样本、构建和前端接入已经完成，真实设备性能与 iOS 验证仍待补齐：
...
- [x] 在当前已落地的预设注入之后接入 Token 预算逻辑，并保持后续聊天处理器可按顺序插入；
- [x] 直接复用现有 `countTokensBatch()` 和 Rust `o200k_base`，未提前抽取共享包或下沉新的 Rust 管线；
- [x] 完成 `token-limiter` 的文本消息预算与历史截断，并覆盖预设占满预算、部分截断和无可用历史等测试；
- [x] 保留发送后最终计数，用于实际 usage 缺失时的展示和上下文快照；
- [ ] 在真实设备功能链稳定后，补测初始化耗时、批量性能、峰值内存和交互流畅度。
...
| 主要用途       | 分析、比较、计算、开发工具      | 上下文占比和后续裁剪参考 |
