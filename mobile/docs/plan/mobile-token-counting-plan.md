...
> 状态：计数服务与展示接入完成；Token 驱动的上下文编排、真机性能和 iOS 验证待完成
> 日期：2026-07-14；2026-07-24 更新上下文管线边界
...
移动端当前已在智能体预设编辑器、聊天输入区和发送前快照中使用 Rust `o200k_base` 计数；IPC 失败时才回退到字符估算。计数服务已经落地，但它还没有参与上下文管线内部的预算裁剪和消息编排。
...
当前聊天输入区、发送前管线快照和响应后统计已经复用同一 IPC 封装；但 Token 结果还没有在上下文管线内部提供给处理器：
...
- [ ] 在所有会增加或改变消息的处理器之后接入 Token 预算计算；
- [ ] 迁移桌面端 `token-limiter` 的文本预算、历史消息保留和部分截断逻辑；
- [ ] 为需要 Token 成本的后续处理器提供稳定的消息级结果；
- [ ] 明确附件、工具 schema 和非文本多模态开销的独立估算方式。
...
- [ ] 先迁移会产生或重排消息的上下文处理器，再接入 Token 预算逻辑；
- [ ] 直接复用现有 `countTokensBatch()` 和 Rust `o200k_base`，不提前抽取共享包或下沉新的 Rust 管线；
- [ ] 迁移 `token-limiter` 的文本消息预算与历史截断，并补充预设占满预算、部分截断和无可用历史等测试；
- [ ] 保留发送后最终计数，用于实际 usage 缺失时的展示和上下文快照；
