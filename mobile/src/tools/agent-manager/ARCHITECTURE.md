...
- **待移植**：Agent 私有头像与资产管理，以及依赖尚未接入的桌面高级参数（工具调用、Recall/Knowledge、媒体/压缩等）。用户档案已由 `llm-chat` 的基础档案管理页和上下文注入支持；桌面完整宏/用户档案契约仍不在此范围。私有资产不得以全局 `assetId` 代替 Handle 或相对路径。
