...
watch(() => draft.value?.profileId, () => {
  if (draft.value && !availableModels.value.some((model) => model.id === draft.value?.modelId)) {
...
async function save() {
  ...
  if (!draft.value.displayName?.trim() || !draft.value.name.trim() || !draft.value.profileId || !draft.value.modelId) {
...
<template>
  <div class="detail-page">
    ...
    <header class="page-header">
      <button class="icon-button" type="button" aria-label="返回" @click="router.back()"><ChevronLeft :size="24" /></button>
      <h1>{{ tRaw("tools.agent-manager.common.编辑智能体") }}</h1>
      <button class="icon-button primary" type="button" :title="tRaw('tools.agent-manager.AgentDetail.保存')" @click="save"><Save :size="20" /></button>
    </header>

    <main v-if="draft" class="editor">
      <section class="form-section">
        <label><span>{{ tRaw("tools.agent-manager.AgentDetail.显示名称") }}</span><input v-model="draft.displayName" /></label>
        <label><span>{{ tRaw("tools.agent-manager.AgentDetail.内部名称") }}</span><input v-model="draft.name" /></label>
        <label><span>{{ tRaw("tools.agent-manager.AgentDetail.描述") }}</span><textarea v-model="draft.description" rows="3"></textarea></label>
        <label><span>{{ tRaw("tools.agent-manager.AgentDetail.图标") }}</span><input v-model="draft.icon" /></label>
      </section>
...
<style scoped>
.detail-page { min-height: 100%; padding: 16px; box-sizing: border-box; color: var(--text-color); }
.page-header { display: grid; grid-template-columns: 44px minmax(0, 1fr) 44px; gap: 10px; align-items: center; margin-bottom: 20px; }
.page-header h1 { margin: 0; font-size: 1.25rem; text-align: center; }
...
.editor { display: flex; flex-direction: column; gap: 22px; padding-bottom: max(24px, env(safe-area-inset-bottom)); }
.form-section { display: grid; gap: 14px; }
.form-section.two-column { grid-template-columns: 1fr 1fr; }
label { display: flex; flex-direction: column; gap: 7px; min-width: 0; }
label span { font-size: .8rem; color: var(--color-on-surface-variant); }
input, select, textarea { width: 100%; box-sizing: border-box; border: var(--border-width) solid var(--border-color); border-radius: 8px; padding: 11px 12px; background: var(--input-bg); color: var(--text-color); font: inherit; outline: none; }
textarea { resize: vertical; line-height: 1.55; }
input:focus, select:focus, textarea:focus { border-color: var(--color-primary); }
@media (max-width: 440px) { .form-section.two-column { grid-template-columns: 1fr; } }
</style>
