...
- `useManagedMediaPreview` 统一处理 descriptor 请求序号、迟到响应撤销、换资源、重复清理、过期重试和媒体错误映射。host 负责加载/错误层、安全区、背景滚动锁定和“先退出沉浸层、再关闭宿主”的返回顺序；视频在 Web Fullscreen API 被拒绝而进入应用内沉浸层时会交接当前播放位置和播放状态，并在 history 返回关闭沉浸层后恢复内联播放器；各媒体组件分别保留适合触屏的播放或缩放控制。
...
- 新媒体组件的 jsdom 自动化只验证生命周期和 DOM 交互契约。2026-07-26 至 2026-07-27 已在 `emulator-5554`（`Medium_Phone_API_36`）以 APK 端到端回归资产详情图片预览、聊天附件持久化链路，以及 DocumentsUI 导入 WAV/MP4 后的音频和视频受控预览。音频场景验证 `audio/wav`/`audio/x-wav` MIME、`ready` 状态、短期 `aio-asset.localhost` URL、实际播放/暂停与后退/前进、倍速、静音控制；视频场景验证 `video/mp4`、实际 WebView 解码/播放/暂停、受管 URL 回收和强制 Fullscreen API 拒绝时的应用层回退及 WebView history 返回后的内联续播；资产详情通过调用方可选的 `imageTestId` 暴露稳定图片元素标识。图片真实手势、原生全屏/系统返回、长视频拖动、横竖屏、安全区、快速切换和真机复验仍是平台门禁。
