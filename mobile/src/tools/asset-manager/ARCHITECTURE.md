...
- 新媒体组件的 jsdom 自动化只验证生命周期和 DOM 交互契约。2026-07-26 已在 `emulator-5554`（`Medium_Phone_API_36`）以 APK 端到端回归资产详情图片预览、聊天附件持久化链路，以及 DocumentsUI 导入 WAV 后的音频受控预览和沉浸层控制。音频场景验证 `audio/wav`/`audio/x-wav` MIME、`ready` 状态、短期 `aio-asset.localhost` URL 与后退/前进、倍速、静音控制；资产详情通过调用方可选的 `imageTestId` 暴露稳定图片元素标识。图片手势、视频全屏 fallback、音频实际播放、横竖屏、安全区和快速切换仍需在 Android AVD 与真机复验。
