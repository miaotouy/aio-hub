...
- **UI 骨架自研**：移动端 Chat 的页面骨架、抽屉、顶部栏、消息流和输入区使用 Vue 原生组件与项目 `base/common` 组件实现；Varlet 仅作为叶子节点原子件，不承担整体设计语言。
...
- **UI 分层**：使用 Vue 原生组件和 AIO Hub 移动端 `base/common` 组件搭建页面骨架；`@varlet/ui` 仅保留为按钮、输入、选择器、加载态等底层原子件，不采用 Material Design 3 作为整体风格。
