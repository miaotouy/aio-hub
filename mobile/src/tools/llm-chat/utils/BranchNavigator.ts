...
export class BranchNavigator {
   ...
   * 获取节点的所有兄弟节点（包括自己）
   */
  static getSiblings(session: ChatSession, nodeId: string): ChatMessageNode[] {
   ...
   */
  static findLeafOfBranch(session: ChatSession, startNodeId: string): string {
   ...
   */
  static updateSelectionMemory(session: ChatSession, leafNodeId: string): void {
    const path: string[] = [];
   ...
   * 判断某个节点是否在当前活动路径上
   */
  static isNodeInActivePath(session: ChatSession, nodeId: string): boolean {
