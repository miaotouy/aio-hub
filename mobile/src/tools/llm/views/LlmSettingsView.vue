...
<template>
  <div class="llm-settings h-screen flex flex-col">
    ...
    <div class="flex-1 overflow-y-auto p-4">
      <div v-for="profile in store.profiles" :key="profile.id" 
        ...
        <div class="flex items-center gap-2">
          <var-button round text size="small" @click="store.selectProfile(profile.id)">
            <CheckCircle2 :size="20" :color="store.selectedProfileId === profile.id ? 'var(--color-primary)' : '#ccc'" />
          </var-button>
      ...
      <var-empty v-if="store.profiles.length === 0" description="点击右上角添加您的第一个 AI 渠道" />
    </div>

    <!-- 编辑弹窗 -->
    <var-overlay v-model:show="showEditOverlay" :click-overlay-close="false">
      <div class="edit-panel bg-surface w-[90vw] max-h-[80vh] rounded-2xl overflow-hidden flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto p-6" v-if="editingProfile">
          <var-input v-model="editingProfile.name" label="渠道名称" placeholder="例如: My OpenAI" class="mb-4" />
          ...
          <var-input v-model="editingProfile.baseUrl" label="API 基础地址" placeholder="https://api.openai.com/v1" class="mb-4">
            <template #prepend-icon><Globe :size="18" class="mr-2" /></template>
          </var-input>

          <var-input v-model="editingProfile.apiKeys[0]" label="API Key" type="password" placeholder="sk-..." class="mb-6">
          ...
          <div class="section-title flex items-center justify-between mb-2">
            ...
            <var-button size="mini" type="primary" plain :loading="isFetchingModels" @click="handleFetchModels">
              <RefreshCw :size="14" class="mr-1" /> 自动获取
            </var-button>
          </div>

          <div class="model-list border rounded-lg p-2 max-h-[200px] overflow-y-auto bg-surface-variant">
            <div v-for="m in editingProfile.models" :key="m.id" class="text-xs p-1 border-b last:border-0 opacity-80">
              {{ m.name }} ({{ m.id }})
            </div>
            <div v-if="editingProfile.models.length === 0" class="text-center py-4 text-xs opacity-40">暂无模型，请点击自动获取</div>
          </div>
        </div>

        <div class="p-4 border-t">
          <var-button block type="primary" @click="saveEdit">保存配置</var-button>
          <var-button block text type="danger" class="mt-2" @click="store.deleteProfile(editingProfile.id); showEditOverlay = false">删除渠道</var-button>
        </div>
      </div>
    </var-overlay>
  </div>
