...
const { runs, runAutomated, createManualRun, setResumeRun } = useValidationRuns();
const preset = ref<SqlitePreset>("1k");
const faultPoint = ref<"before-commit" | "after-write">("after-write");
const suiteRuns = computed(() => runs.value.filter((run) => run.suiteId === "sqlite"));
...
const cases: Array<{ id: SqliteScenario; title: string; description: string }> = [
  { id: "environment", title: "环境检查", description: "SQLite 版本、compile options、FTS5 能力和固定数据库路径类型。" },
  { id: "connection", title: "连接烟测", description: "创建、关闭、重开、并发读取和写锁等待。" },
  { id: "migration", title: "Migration", description: "空库升级、失败回滚和高版本拒写。" },
  { id: "codec", title: "Codec round-trip", description: "结构化消息、未知 metadata、可选时间戳与附件快照。" },
  { id: "transaction-recovery", title: "事务恢复", description: "固定 fault point 中断，检查半提交、外键和 integrity。" },
  { id: "fts-query", title: "FTS 查询语义", description: "中日英、emoji、引号、连字符及短词查询。" },
  { id: "benchmark", title: "数据规模基准", description: "记录建库、索引、冷/热查询、加载、删除、体积和峰值估算。" },
];
...
async function runCase(scenario: SqliteScenario): Promise<void> {
  if (scenario === "transaction-recovery") {
    const confirmed = await customDialog({
      title: "执行事务强杀恢复验证",
      message: "该操作会在固定事务注入点直接终止应用进程。重启后验证台将自动检查半提交、外键与 integrity。",
...
async function reset(action: "rebuild" | "delete"): Promise<void> {
  const confirmed = await customDialog({
    ...
    message: "该操作只允许作用于 ui_tester_validation.db。Rust 侧会再次校验文件名。",
...
<template>
  <section class="validation-page">
    <ValidationRunHeader :passed="suiteTotals.passed" :failed="suiteTotals.failed" :pending="suiteTotals.pending" :running="running" phase="Rust 正在生成或检查测试数据" />
    <div class="sqlite-toolbar">
      ...
      <var-button v-if="running" type="warning" size="small" @click="cancel"><Square :size="15" />停止</var-button>
    </div>

    <ValidationCaseRow v-for="item in cases" :key="item.id" :title="item.title" :description="item.description" :status="statusFor(item.id)" :disabled="running" @run="runCase(item.id)" />

    <div class="database-actions">
      <var-button size="small" :disabled="running" @click="reset('rebuild')"><DatabaseZap :size="16" />重建测试库</var-button>
      <var-button type="danger" size="small" :disabled="running" @click="reset('delete')"><DatabaseZap :size="16" />删除测试库</var-button>
    </div>

    <ValidationStepList v-if="latestRun" :steps="latestRun.steps" />
    <div v-if="latestRun && Object.keys(latestRun.metrics).length" class="metrics-table-wrap">
      <table class="metrics-table">
        <tbody><tr v-for="(value, key) in latestRun.metrics" :key="key"><th>{{ key }}</th><td>{{ value }}</td></tr></tbody>
      </table>
    </div>
...
.sqlite-toolbar { display: flex; flex-wrap: wrap; align-items: center; gap: 10px; padding: 16px 0 8px; }
.sqlite-toolbar label { font-size: 13px; font-weight: 600; }
.sqlite-toolbar select { min-width: 110px; min-height: 36px; padding: 0 10px; color: var(--text-primary); background: var(--input-bg); border: 1px solid var(--divider-color, rgba(127, 127, 127, 0.25)); border-radius: 6px; }
.database-actions { display: flex; flex-wrap: wrap; gap: 8px; padding: 18px 0; }
.metrics-table-wrap { max-width: 100%; overflow-x: auto; margin-top: 18px; }
.metrics-table { width: 100%; border-collapse: collapse; font-size: 12px; }
.metrics-table th, .metrics-table td { padding: 8px; border-bottom: 1px solid var(--divider-color, rgba(127, 127, 127, 0.18)); text-align: left; overflow-wrap: anywhere; }
.metrics-table th { color: var(--text-secondary, #6b7280); font-weight: 500; }
