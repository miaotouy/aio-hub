...
- [ ] **同步核心类型文件**：
    - 将 `src/tools/llm-chat/types/` 下的所有文件复制到 `mobile/src/tools/llm-chat/types/`。
    - 检查并修复移动端特有的路径引用（如 `@/` 别名在移动端配置是否一致）。
- [ ] **建立移动端 Store 骨架**：
...
- [ ] **实现极简 Executor**：
    ...
    - 剥离 PC 端的管道逻辑，直接使用 `useLlmRequest` 发起请求。
    - 仅处理 `user` 和 `assistant` 角色的消息转换。
- [ ] **消息渲染组件**：
