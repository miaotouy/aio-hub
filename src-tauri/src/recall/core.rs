...
#[derive(Debug, Serialize, Deserialize, Clone)]
#[serde(rename_all = "camelCase")]
pub struct RecallCollectionIndex {
    pub id: Uuid,
    pub name: String,
...
#[derive(Debug, Serialize, Deserialize, Clone)]
#[serde(rename_all = "camelCase")]
pub struct RecallWorkspace {
    pub version: String,
    pub config: WorkspaceConfig,
    pub bases: Vec<RecallCollectionIndex>,
}
...
#[derive(Debug, Serialize, Deserialize, Clone)]
#[serde(rename_all = "camelCase")]
pub struct RecallCollectionMeta {
    pub id: Uuid,
    pub name: String,
    pub description: Option<String>,
    #[serde(default)]
    ...
    /// 条目索引列表 (轻量级，用于快速显示列表)
    #[serde(default)]
    pub entries: Vec<RecallEntryIndexItem>,
    /// 库级别配置 (万能结构，方便前端扩展)
    #[serde(default)]
...
#[derive(Debug, Serialize, Deserialize, Clone)]
#[serde(rename_all = "camelCase")]
pub struct RecallEntryIndexItem {
    pub id: Uuid,
    pub key: String,
    pub summary: String,
    pub tags: Vec<String>,
...
/// 完整思绪集结构 (用于全量导入/导出或旧版兼容)
#[derive(Debug, Serialize, Deserialize, Clone)]
#[serde(rename_all = "camelCase")]
pub struct RecallCollection {
    pub meta: RecallCollectionMeta,
...
pub struct RecallSearchFilters {
    pub recall_ids: Option<Vec<Uuid>>,
...
pub struct RetrievalContext {
    /// 内存数据库的引用
    pub db: std::sync::Arc<std::sync::RwLock<crate::recall::index::InMemoryDatabase>>,
    /// 标签池管理器引用
...
mod tests {
    ...
    fn test_workspace_serialization() {
        ...
        let workspace = RecallWorkspace {
            ...
            config,
            bases: vec![RecallCollectionIndex {
                id: Uuid::new_v4(),
        ...
        let deserialized: RecallWorkspace = serde_json::from_str(&json).unwrap();
    ...
    fn test_knowledge_base_serialization() {
        ...
        let json = serde_json::to_string_pretty(&recall).unwrap();
        println!("RecallCollection JSON: {}", json);

        let deserialized: RecallCollection = serde_json::from_str(&json).unwrap();
        assert_eq!(recall.meta.id, deserialized.meta.id);
        assert_eq!(recall.entries.len(), deserialized.entries.len());
        assert_eq!(recall.entries[0].key, "Rust");
    ...
    fn test_entry_default_values() {
        ...
        let deserialized: RecallEntry = serde_json::from_str(&json).unwrap();
