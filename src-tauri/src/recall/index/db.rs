...
impl InMemoryBase {
    pub fn new(meta: RecallCollectionMeta) -> Self {
        Self {
            meta,
    ...
    pub fn sync_entry(&mut self, entry: RecallEntry) {
        ...
        if let Some(pos) = self.meta.entries.iter().position(|e| e.id == id) {
            ...
            existing.tags = entry.tags.iter().map(|t| t.name.clone()).collect();
            ...
            let content_changed = match (&existing.content_hash, &entry.content_hash) {
            ...
            if content_changed {
                ...
                existing.content_hash = entry.content_hash.clone();
            ...
            let index_item = RecallEntryIndexItem {
                id: entry.id,
                key: entry.key.clone(),
                summary: entry.summary.clone(),
                tags: entry.tags.iter().map(|t| t.name.clone()).collect(),
    ...
    /// 获取包含条目索引的元数据
    pub fn get_meta_with_entries(&self) -> RecallCollectionMeta {
        // 无论是否全量加载，都直接返回 meta
