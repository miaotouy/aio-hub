...
impl RetrievalEngine for KeywordRetrievalEngine {
    ...
    fn search(
        ...
        let query = match payload {
            crate::recall::core::QueryPayload::Text(t) => t,
            _ => return Ok(vec![]),
        };
        ...
        for (recall_id, base_lock) in &imdb.bases {
            ...
            if let Some(min_score) = base
                ...
                .map(|v| v as f32)
            {
                recall_min_scores.insert(*recall_id, min_score);
            }
            ...
            for (entry_id, score_from_index) in candidate_ids {
                if let Some(entry) = base.entries.get(&entry_id) {
                    ...
                    if let Some(ref filter_tags) = filters.tags {
                        let has_tag = entry.tags.iter().any(|t| filter_tags.contains(&t.name))
                            || entry
                                .core_tags
                                .iter()
                                .any(|t| filter_tags.contains(&t.name));
                        if !has_tag {
                            continue;
                        }
                    }
        ...
        results.retain(|r| {
            // 优先使用库级别配置，其次是全局过滤器
            let effective_min_score = recall_min_scores
                .get(&r.recall_id)
                .cloned()
                .or(filters.min_score);
