...
impl RetrievalEngine for VectorRetrievalEngine {
    ...
    fn info(&self) -> crate::recall::core::RetrievalEngineInfo {
        crate::recall::core::RetrievalEngineInfo {
    ...
    fn search(
        &self,
        payload: &crate::recall::core::QueryPayload,
        filters: &RecallSearchFilters,
        context: &RetrievalContext,
    ) -> Result<Vec<RecallResult>, String> {
        use crate::recall::core::QueryPayload;
        ...
        for (recall_id, base_lock) in &imdb.bases {
                ...
                for (tag_idx, tag_dist) in neighbors {
                    ...
                    if let Some(tag_name) = pool.get_tag_name(tag_idx) {
                        ...
                        for entry in base.entries.values() {
                            if entry.tags.iter().any(|t| t.name == *tag_name)
                                || entry.core_tags.iter().any(|t| t.name == *tag_name)
                            {
                                let entry_score = tag_scores.entry(entry.id).or_insert(0.0);
            ...
            if dimension == 0 {
                log::warn!(
                    "[VECTOR_SEARCH] 思绪集向量存储为空，使用纯标签召回: recall={}",
                    recall_id
                );
            ...
            if dimension != query_vector.len() {
                log::error!(
                    "[VECTOR_SEARCH] 维度不匹配，跳过: recall={}, base_dim={}, query_dim={}",
                    recall_id,
                    dimension,
                    query_vector.len()
                );
            ...
            for (entry_id, vector_score) in scores {
                if let Some(entry) = base.entries.get(&entry_id) {
                    ...
                    // 综合评分：内容向量相似度 + 标签引力权重
                    let tag_score = tag_scores.get(&entry_id).cloned().unwrap_or(0.0);
                    ...
                    if !raw_query_lower.is_empty() {
                        if entry.key.to_lowercase().contains(&raw_query_lower) {
                            literal_boost += 0.2; // 标题匹配加成
                        }
                        if entry.content.to_lowercase().contains(&raw_query_lower) {
            ...
            if let Some(limit) = recall_search_top_k {
                recall_results.truncate(limit);
            }
...
fn augment_query_vector(
    query_vector: &[f32],
    tag_pool: &crate::recall::tag_pool::ModelTagPool,
    total_entries: usize,
) -> Vec<f32> {
