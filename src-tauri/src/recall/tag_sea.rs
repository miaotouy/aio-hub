...
pub struct TagSea {
    ...
    /// 标签名 -> 关联的 CAIU ID 及权重
    pub tag_to_entry_weights: HashMap<String, Vec<(Uuid, f32)>>,
    /// 标签名 -> 静态语法权重
...
impl TagSea {
    ...
    pub fn build(base: &InMemoryBase, tag_pool: ModelTagPool) -> Self {
        ...
        for (tag_name, entrys) in &tag_to_entry_weights {
            let doc_freq = entrys.len() as f32;
            // IDF 公式: log(N / (df + 1)) + 1
            let idf = (total_entrys / (doc_freq + 1.0)).ln() + 1.0;
