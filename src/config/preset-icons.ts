...
export const LOCAL_ICONS_MAP = Object.entries(localIcons).reduce(
  (acc, [path, _content]) => {
    const name = path.split("/").pop()!;
...
export const AVAILABLE_ICONS = [
  ...new Set([...Object.keys(LOBE_ICONS_MAP), ...Object.keys(LOCAL_ICONS_MAP)]),
].sort();
 ...
 * 此时 path 已经全部是带路径的完整名
 */
export const PRESET_ICONS: PresetIconInfo[] = [
  ...MANUAL_PRESET_ICONS.map((i) => ({
    ...i,
    path: i.path.startsWith("/") ? i.path : `/model-icons/${i.path}`,
  })),
