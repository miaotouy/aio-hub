...
The [Video API](https://platform.openai.com/docs/api-reference/videos) (in preview) exposes these capabilities to developers for the first time, enabling programmatic creation, extension, and remixing of videos. It provides five endpoints, each with distinct capabilities:
...
Instead of polling job status repeatedly with `GET`, register a [webhook](https://platform.openai.com/docs/guides/webhooks) to be notified automatically when a video generation completes or fails.
