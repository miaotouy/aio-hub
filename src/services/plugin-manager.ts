...
export async function resolvePluginIconUrl(
  ...
  try {
    ...
    if (isDevMode) {
      // 开发模式：直接使用 Vite 路径
      // 确保路径以 / 开头
      const basePluginPath = pluginPath.startsWith("/")
        ? pluginPath
        : `/${pluginPath}`;
 ...
 */
async function createPluginIcon(
  pluginPath: string,
  iconConfig?: string
): Promise<Component> {
  try {
    ...
    return markRaw({
      setup() {
        ...
        const init = async () => {
          // 判断是否为本地绝对路径 (Windows)
          const isLocalPath =
            /^[a-zA-Z]:[\\/]/.test(iconUrl) || iconUrl.startsWith("\\\\");
          if (isLocalPath) {
        ...
        return () => {
          if (!processedSrc.value)
            return h("div", { style: "width: 1em; height: 1em;" });
 ...
 */
function createPluginComponentLoader(
  pluginPath: string,
  ...
  return async () => {
    try {
      ...
      if (isDevMode) {
        ...
        logger.info("插件组件加载成功（开发模式，支持 HMR）", {
          componentFile,
        });
...
class PluginManager {
  ...
  // 响应式插件状态追踪
  public pluginStates = reactive<
    Record<string, { enabled: boolean; isBroken: boolean }>
  >({});
   ...
   */
  private async createPluginStorageAPI(
    pluginId: string
  ): Promise<PluginStorageAPI> {
    const { join } = await import("@tauri-apps/api/path");
      ...
      writeText: async (path: string, data: string) => {
        ...
        const parentDir = fullPath.substring(
          ...
          fullPath.lastIndexOf(
            /[/\\]/.test(fullPath) ? fullPath.match(/[/\\]/)![0] : ""
          )
        );
        if (
          parentDir &&
    ...
    return {
      ...
      storage: {
        ...
        writeText: async (path, data) =>
          (await storagePromise).writeText(path, data),
        writeBinary: async (path, data) =>
          (await storagePromise).writeBinary(path, data),
      ...
      chat: {
        registerProcessor: (processor: any) => {
          logger.info(
            `插件正在注册上下文处理器: ${processor.id} (Plugin: ${pluginId})`
          );
  ...
  async initialize(): Promise<void> {
    ...
    // 监听来自其他窗口的状态变更
    listen<{ pluginId: string; enabled: boolean }>(
      "plugin-runtime-state-changed",
      (event) => {
  ...
  public async updateRuntimeState(
    ...
    if (!isRemote) {
      emit("plugin-runtime-state-changed", { pluginId, enabled }).catch(
        (err) => {
  ...
  getInstalledPlugins(): PluginProxy[] {
    ...
    return allRegistries.filter(
      (registry): registry is PluginProxy =>
        "manifest" in registry && "enabled" in registry
    );
  }
