...
import type { ChatMessageNode, MessageRole } from "@/tools/llm-chat/types";
...
const anchors = computed(() => {
  ...
  localData.value.forEach((msg: ChatMessageNode, index: number) => {
...
function handleBatchEnable(enabled: boolean) {
  selectedRows.value.forEach((row: ChatMessageNode) => {
...
function handleBatchDelete() {
  const idsToDelete = new Set(
    selectedRows.value.map((r: ChatMessageNode) => r.id)
  );

  // 过滤掉不可删除的锚点
  const anchorsCount = selectedRows.value.filter((r: ChatMessageNode) =>
    isAnchorType(r.type)
  ).length;
  if (anchorsCount > 0) {
  ...
  localData.value = localData.value.filter((item: ChatMessageNode) => {
...
function handleMoveTo(position: "top" | "bottom") {
  ...
  const itemsToMove = localData.value.filter((r: ChatMessageNode) =>
    selectedIds.has(r.id)
  );
  const remainingItems = localData.value.filter(
    (r: ChatMessageNode) => !selectedIds.has(r.id)
  );
...
function handleMoveToSpecific() {
  ...
  // 在剩余项中找到目标行的位置
  let insertIndex = remainingItems.findIndex(
    (r: ChatMessageNode) => r.id === targetRow.id
  );
