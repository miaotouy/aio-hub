...
function migratePresetGreetings(agent: ChatAgent): boolean {
  ...
  const greetingMessages = agent.presetMessages.filter(
  ...
  agent.greetings = greetingMessages.map(
    (message: ChatMessageNode, index: number) => ({
  ...
  const greetingIds = new Set(
    greetingMessages.map((message: ChatMessageNode) => message.id)
  );
  agent.presetMessages = agent.presetMessages.filter(
    (message: ChatMessageNode) => !greetingIds.has(message.id)
  );
