...
import type { ChatAgent, AgentAsset } from "../types/agent";
...
export async function initAgentAssetCache(): Promise<void> {
  ...
  _cacheInitPromise = (async () => {
    try {
      ...
      const isWindows =
        _cachedAppDataDir.includes("\\") || _cachedAppDataDir.includes(":");
      if (isWindows) {
      ...
      logger.info("Agent 资产缓存已初始化", {
        appDataDir: _cachedAppDataDir,
        isWindows,
      });
    } catch (error) {
 ...
 */
export function convertAgentAssetToUrl(
  agentId: string,
  assetPath: string
): string | null {
...
function parseAssetUrl(
  assetUrl: string
): { group: string; id: string; ext: string } | null {
...
function findAsset(
  ...
  const filenameOnlyMatch = assets.find(
    (a) => getFilenameWithoutExt(a.filename) === parsed.id
  );
  ...
  if (bestFuzzyMatch) {
    logger.info("通过模糊匹配找到资产", {
      ...
      matchedAsset: {
        ...
        filename: bestFuzzyMatch.filename,
        group: bestFuzzyMatch.group,
      },
 ...
 */
export function resolveAgentAssetUrlSync(
  assetUrl: string,
  agent: ChatAgent
): string {
...
export async function resolveAgentAssetUrl(
  ...
  if (!asset) {
    logger.warn("Asset not found", {
      parsed,
      availableAssets: agent.assets?.map((a: AgentAsset) => ({
        id: a.id,
        group: a.group,
      })),
    });
 ...
 * @returns 替换后的内容
 */
export function processMessageAssetsSync(
  content: string,
  agent?: ChatAgent
): string {
 ...
 */
export async function processMessageAssets(
  content: string,
  agent?: ChatAgent
): Promise<string> {
  const hasAgentAsset = content.includes("agent-asset://");
 ...
 * 解析智能体头像路径的纯函数
 */
export function resolveAgentAvatarPath(
  agent: { id: string; icon?: string } | undefined | null
): string | null {
  if (!agent) {
    return null;
  }
...
/**
 * 一个用于解析智能体最终头像路径的 Composable。
 */
export function useResolvedAgentAvatar(
  agentRef: Ref<{ id: string; icon?: string } | undefined | null>
) {
  return computed(() => {
    return resolveAgentAvatarPath(agentRef.value);
  });
}
