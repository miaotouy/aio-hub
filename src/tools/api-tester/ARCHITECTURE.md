...
- **预设注册**: 预设由变量和请求体构造辅助函数组合而成，覆盖通用 REST 请求、OpenAI 兼容聊天接口及不同协议/版本路径；API Key、模型、消息、温度、最大 token 和流式开关都通过统一变量模板描述。新增服务商预设时应优先复用现有的 OpenAI-compatible preset factory，而不是复制整段 URL、Header 和 Body 模板。
...
- **JSON 类型保真**: 请求体中的独立占位符会尽量保留布尔、数字和 `null` 类型，避免把 `stream: false` 之类的值错误发送成字符串。
- **发送前校验**: Store 会检查必填变量、未定义占位符和 URL 格式，避免无效请求直接发出。
- **共用语义**: Agent 注册元数据与 UI 预设共用同一套变量语义，修改变量 key 时必须同步检查模板替换、历史记录序列化和响应展示。
...
- **数据结构**: 每个档案包含名称、所基于的预设 ID、URL 模板、HTTP 方法、变量、请求头和请求体。
- **请求历史**: 最近 50 次请求会写入历史列表，包含请求快照、响应状态、耗时和响应摘要，可点击恢复当次请求配置。
- **主工作流集成**: 请求历史与响应面板属于主工作流；变量模板在发送前执行解析与校验，恢复档案时据此重建请求。

## 5. 未来展望

- **环境变量**: 支持在不同环境（如开发/生产）之间快速切换变量集。
- **导入/导出**: 支持 cURL、HTTPie 或 Postman Collection 的导入导出。
