...
- **多媒体元数据提取**:
  ...
  - **音频波形采样**: 利用前端 Web Audio API (OfflineAudioContext) 在浏览器端离线解码音频文件，提取低分辨率波形采样数据（100 个 0-255 的整数点），持久化到资产元数据中。详见 [`src/utils/audioSampler.ts`](../../utils/audioSampler.ts)。
