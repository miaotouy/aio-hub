...
本文档旨在深入解析 `canvas` 工具的内部架构、设计理念和数据流，为后续的开发和维护提供清晰的指引。

## 1. 核心概念 (Core Concepts)

`canvas` 是一个基于 **Physical-First** 架构的 Agent 协作画布工具。它不维护"影子文件"或内存中的虚拟文件系统，所有编辑操作**直接**作用于物理磁盘，利用 Git 进行版本追踪。
...
与常见的"内存中编辑 → 统一保存"模式不同，canvas 采用**物理优先**策略：
...
- **优势**:
  ...
  - 外部兼容：用户可以在 VSCode 等外部编辑器中修改文件，canvas 能通过 Git 状态自动感知变更。
...
canvas 通过 `canvas.registry.ts` 暴露了一套完整的 Agent 可调用方法，使其能够被 LLM Agent 驱动。
...
canvas 通过在预览 `iframe` 中注入脚本，实时捕获前端的运行时错误。
...
canvas 支持将预览窗口分离为独立窗口，通过状态同步引擎保持数据一致。
...
Diff 引擎是 `canvas` 工具的核心技术之一，它实现了与 AI 编辑器类似的 **Search/Replace** 代码修改模式。
...
canvas 的设置系统采用声明式配置，与应用的设置渲染器集成。
...
canvas 采用**多级存储策略**，将索引、元数据和项目文件分层存储：
