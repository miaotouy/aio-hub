...
- `ColorFamilyPointsEditor.vue` 在设置栏内展开圆盘与全部点位编辑行，亮度阈值位于色系配置上方；每个点位的色标、名称、H/S 和删除操作紧凑排列为单行。亮度与 H/S 复用 `ScrubNumberInput.vue`，支持横向拖拽、Shift 精细调节、点击/Enter 输入及方向键调节；色系拖拽预览后松手提交，取消恢复原值。预设选择和边界开关使用主题化 Element Plus 组件，支持选中联动、命名、增删及常用九色/基础七色预设。圆盘右侧为 0°，顺时针递增；Canvas 底图以 S² 渲染饱和度，让中心呈低饱和灰色、外缘保持满饱和；该曲线仅影响背景预览，不改变线性的点位坐标、数值、色标和分类。SVG 用半平面裁剪得到的最近点分区并应用圆形裁剪。拖动只改草稿，松手一次提交；越界钳制到圆周，Escape/指针取消恢复。名称/数值 Enter 或失焦验证后生效，非法草稿不写入配置。添加支持圆盘点击或键盘 Enter/空格；切换预设覆盖自定义修改前确认。名称同时用于归档目录，前后端拒绝路径穿越、Windows 非法字符及设备名。
...
- 批量配置 `batch-organizer-config.json` 使用 `3.0.0` 格式的 `colorPoints`，复用 ConfigManager 防抖保存且加载完成前不写回。尚未发布的旧 HSL 范围规则直接替换为默认点位，不做近似迁移；目录、亮度阈值和归档偏好继续保留。损坏点集整体回退默认，新格式显式空数组保留。预设调整不做迁移，已有点位保持原值，显式选择预设时应用新值。点位持久化，分析数据和勾选仅在当前会话保留；不影响单图分析。
