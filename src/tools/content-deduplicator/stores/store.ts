...
export const useContentDeduplicatorStore = defineStore(
  ...
  () => {
    ...
    const filteredGroups = computed<DuplicateGroup[]>(() => {
      ...
      if (searchKeyword.value.trim()) {
        ...
        groups = groups.filter((g) => {
          ...
          const simMatch = g.similarFiles.some(
            (sf) =>
              sf.file.name.toLowerCase().includes(kw) ||
              sf.file.path.toLowerCase().includes(kw)
          );
      ...
      switch (sortBy.value) {
        case "wastedBytes":
          groups.sort(
            (a, b) => b.metadata.totalWastedBytes - a.metadata.totalWastedBytes
          );
          break;
        case "similarity":
          groups.sort(
            (a, b) => b.metadata.avgSimilarity - a.metadata.avgSimilarity
          );
          break;
        case "fileCount":
    ...
    const activeGroup = computed<DuplicateGroup | null>(() => {
      ...
      return (
        result.value.groups.find((g) => g.id === activeGroupId.value) ?? null
      );
    });
    ...
    function removeDeletedPaths(deletedPaths: string[]) {
      ...
      for (const group of result.value.groups) {
        group.similarFiles = group.similarFiles.filter(
          (sf) => !deletedSet.has(sf.file.path)
        );
        // 重新计算元数据
