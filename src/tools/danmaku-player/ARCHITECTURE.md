...
- **外部播放器覆盖**：连接 MPC-BE 的 Web 接口，同步透明覆盖窗口到播放器客户区，在第三方播放器上显示弹幕。
...
- [`useExternalPlayer.ts`](composables/useExternalPlayer.ts) 保存外部播放器配置，扫描窗口，测试 MPC-BE Web 接口，并每 1s 拉取一次播放状态用于面板预览。
- [`useDanmakuOverlay.ts`](composables/useDanmakuOverlay.ts) 创建透明覆盖窗口，同步弹幕数据和配置，并持续跟随目标窗口位置。
- [`overlay/DanmakuOverlayApp.vue`](overlay/DanmakuOverlayApp.vue) 是覆盖窗口内的最小渲染应用，只包含一个透明 Canvas、`DanmakuEngine`、`MpcBeClient` 和虚拟时钟。

目前 UI 只暴露 MPC-BE 类型；`ExternalPlayerConfig` 中的 `potplayer` 还只是类型预留。
...
- `ExternalPlayerConfig` / `OverlayState`：外部播放器端口、覆盖裁切、全屏增强、目标窗口和播放状态。
...
1. `ExternalPlayerPanel` 挂载后自动扫描 MPC-BE 常见类名：`MediaPlayerClassicW`、`MPC-BE`。
2. 用户也可以切换为扫描所有可见顶层窗口，手动选择目标 HWND。
3. `MpcBeClient` 通过 Rust command `get_mpc_be_status` 请求 `http://localhost:{port}/variables.html`，测试连接并展示状态。
...
6. 覆盖窗口每 200ms 轮询 MPC-BE 状态，使用虚拟时钟平滑推进弹幕时间。
...
- `get_mpc_be_status`：后端代理请求 MPC-BE `/variables.html`，按 `<p id="...">` 提取 file/state/position/duration/volumelevel。
...
- 外部播放状态只实现 MPC-BE Web 接口，端口默认 `13579`。
- 外部模式只显示弹幕，不加载外挂字幕。
- 图形字幕 IDX/SUP 暂不支持。
- Tauri 前端不直接请求 MPC-BE Web 接口，统一通过 Rust 代理规避 CSP / scope 限制。
