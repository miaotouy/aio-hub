        ...
        >
          <el-radio v-for="windowInfo in playerWindows" :key="windowInfo.hwnd" :value="windowInfo.hwnd">
            <span class="window-item">
              <span class="window-item__title" :title="windowInfo.title">{{ windowInfo.title || "未命名播放器窗口" }}</span>
...
import { Circle, Crop, Film, Info, Link, Monitor, Pause, Play, Search, Square, ArrowDown } from "lucide-vue-next";
...
const canStartOverlay = computed(() => {
  return props.danmakus.length > 0 && overlayState.targetHwnd !== null && overlayState.connected && !startingOverlay.value;
});
