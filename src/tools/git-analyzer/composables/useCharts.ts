...
export function useCharts(
  ...
  function drawFrequencyChart() {
    ...
    // 检查 DOM 尺寸
    if (
      frequencyChartEl.clientWidth === 0 ||
      frequencyChartEl.clientHeight === 0
    ) {
  ...
  function drawContributorChart() {
    ...
    // 检查 DOM 尺寸
    if (
      contributorChartEl.clientWidth === 0 ||
      contributorChartEl.clientHeight === 0
    ) {
      logger.warn("图表容器尺寸为零，无法渲染", {
        chart: "Contributor",
  ...
  function drawHeatmapChart() {
    ...
    for (let day = 0; day < 7; day++) {
      for (let hour = 0; hour < 24; hour++) {
        const item = heatmapDataRaw.find(
