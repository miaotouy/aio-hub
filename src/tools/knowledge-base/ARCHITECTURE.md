...
4. **查询构建 (向量空间融合策略)**:
   - 固定从最近一轮对话中分别提取最新的 User 文本和 AI 文本（被动召回线）。严格排除 tool 消息，避免多轮历史与工具调用原始数据引入噪声。
   - 分别对两侧文本进行 Embedding 向量化。
   - 在向量空间中加权平均（默认 User 0.7 : AI 0.3），生成融合查询向量。
   - 仅对 User 文本执行标签匹配预处理（避免 AI 回复中的噪音词误触发标签）。
   - _注_: 被动召回固定单轮，确保注入上下文干净精准；如需更深的历史或文档检索，智能体可通过"主动查询线"（工具调用）由 LLM 实时决策发起，两线并行。
5. **后端检索**:
   - 后端根据 `engineId` 调用对应引擎。
   - 执行过滤 (Filter) -> 计算 (Compute) -> 排序 (Sort)。
6. **分数截断与过滤**:
   - 低于最低分数阈值 (`minScore`) 的条目直接丢弃（主要截断依据）。
   - 按召回上限 (`limit`) 截断剩余结果。
   - 按字数上限 (`maxRecallChars`) 进一步过滤（可选）。
7. **结果增强**: 返回带高亮片段和匹配度评分的结果列表。

### 3.3. 配置层级 (Configuration Hierarchy)

知识库检索涉及多层配置，优先级从高到低：

1. **占位符参数** (`【kb::name::limit::minScore::...】`): 最高优先级，直接覆盖。
2. **Agent 绑定级** (`AgentKnowledgeBaseBinding.limit/minScore`): 单个知识库的覆盖配置。
3. **Agent 全局级** (`AgentKnowledgeSettings.defaultLimit/defaultMinScore`): Agent 的默认检索参数。
4. **知识库级** (`KnowledgeBaseConfig.searchTopK/minScore`): 库自身的配置。
5. **引擎默认值**: 后端引擎的内置默认（通常为 limit=20, minScore=0）。

### 3.4. 缓存策略

- **精确文本匹配**: 缓存 key 基于 `userText + aiText` 组合，完全一致才命中。
- **适用场景**: 同轮多占位符（避免重复检索）和用户重试（相同上下文直接复用）。
- **不使用向量相似度匹配**: 避免语义漂移导致的缓存误命中。
