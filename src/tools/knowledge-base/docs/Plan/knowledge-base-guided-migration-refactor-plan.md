...
> 状态：Phase 0–3 已实现；本地旧数据候选集已选定并通过 Rust 外部迁移验证；正式旧发布版本溯源与真实 Tauri 发布验收待补
>
> 日期：2026-07-26
>
...
- [待发布验收] 该来源未提供可核验的应用发布版本，仍需使用旧正式发布二进制或其他版本溯源生成/确认 `migration-release` 数据，再执行真实 Tauri 完整迁移、中断恢复和安装包 smoke test。
