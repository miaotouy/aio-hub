...
但目前 `RecallState::initialize()` 在发现旧目录后会直接调用导入逻辑。即使导入器内部可以通过状态和指纹返回缓存报告，启动层仍然把“发现旧数据”和“执行迁移”连接在一起，导致开发环境每次重启都会再次进入迁移入口，也无法让用户先阅读方案、备份和确认风险。
...
- 更新架构文档和用户指南；
- 使用正式发布数据进行完整迁移验收和安装包 smoke test。
