...
> 状态：Phase 0–3 已实现；合成旧数据已通过真实 Tauri 检测、显式确认、迁移、同根重启幂等与独立清理验收；本地旧数据候选集已通过 Rust 外部迁移验证；进程中断恢复、正式旧发布版本溯源与安装包验收待补
>
> 日期：2026-07-26
>
...
版本说明和迁移虽然可以在同一次升级中被同时检测，但展示层必须分离：版本说明进入通知中心并从通知打开独立阅读面板；迁移使用专用的 Guided Flow 长交互容器。两者不连续弹出，也不共用同一个步骤流程。迁移入口可以引用“本次迁移与模块重构有关”，但不复制整篇版本说明。
...
开发构建在“设置 → 关于”提供“更新引导手测”入口：

- “重开当前引导”使用当前已合成的上下文从头打开；
- “重新检测并重开”重新读取版本说明和迁移后端状态；
- “忽略迁移记录并重开”只清除 Recall 主库与向量库中的 `legacy_import_state` 完成记录，保留已导入数据，再重新检测并打开流程；
- 调试重置命令在非 debug Rust 构建中直接拒绝执行，生产界面不展示入口。
...
- 更新架构文档和用户指南；
- [本地数据验证完成] 已筛选一个不入库的旧版知识库候选集（1 个集合、473 条条目、472 个旧向量），并通过 `verifies_external_legacy_directory_copy` 验证完整迁移、向量保留和重复执行幂等；来源与内容不记录到仓库或测试产物中。
- [待发布验收] 该来源未提供可核验的应用发布版本，仍需使用旧正式发布二进制或其他版本溯源生成/确认 `migration-release` 数据，再执行真实 Tauri 中断恢复和安装包 smoke test。
- [待 UI 重构] 迁移步骤需要接入专用长交互容器；在真实 Tauri 中证明外层 panel 几何稳定前，不得把当前通用弹窗布局标记为最终体验。

## 13. 验收标准

### 用户体验

- 用户在迁移前能看到迁移规模、风险和与版本重构的最小关联说明；完整版本变化从通知中心或独立版本说明面板查看；
- 未点击明确确认前，不执行用户数据迁移；
- 迁移过程中看到分阶段任务进度；
- 失败、部分成功和待重建都有明确结果；
- 关闭后可以从正确步骤恢复或重新检测；
