    ...
    <div class="settings-content">
      <section class="settings-section">
        ...
        <div class="settings-grid">
          <label>
            ...
            <el-input v-model="runtimeForm.defaultEmbeddingRouteKey" clearable />
          </label>
          <label>
            <span>Embedding 并发</span>
            <el-input-number v-model="runtimeForm.embeddingRequestConcurrency" :min="1" :max="8" />
          </label>
          <label>
          ...
          <label>
            <span>摄取 worker</span>
            <el-input-number v-model="runtimeForm.ingestQueueConcurrency" :min="1" :max="8" />
          </label>
          <label>
            <span>Lease 秒数</span>
            <el-input-number v-model="runtimeForm.ingestLeaseTimeoutSeconds" :min="30" :max="3600" />
          </label>
          <label>
            <span>摄取最大尝试</span>
            <el-input-number v-model="runtimeForm.ingestMaxAttempts" :min="1" :max="10" />
          </label>
          <label>
            <span>单文件上限 MiB</span>
          ...
          <label>
            <span>单批文件数</span>
            <el-input-number v-model="runtimeForm.maxImportBatchFiles" :min="1" :max="10000" />
          </label>
        </div>
      </section>
      ...
      <section class="settings-section library-settings">
        ...
        <template v-if="selectedLibrary">
          <div class="settings-grid">
            ...
            <label>
              <span>分块字符数</span>
              <el-input-number v-model="libraryConfig.chunking.targetChars" :min="200" :max="8000" />
            </label>
            <label>
              <span>重叠字符数</span>
              <el-input-number v-model="libraryConfig.chunking.overlapChars" :min="0" :max="2000" />
            </label>
            ...
            <label class="wide-field">
              ...
              <el-input v-model="libraryConfig.embedding.routeKey" :disabled="!semanticEnabled" />
            </label>
            <label>
              <span>请求维度</span>
              <el-input-number v-model="requestedDimensions" :disabled="!semanticEnabled" :min="1" :max="65536" clearable />
            </label>
          </div>
          <div class="settings-actions">
            <el-button :loading="savingLibrary" @click="saveLibraryMetadata">保存名称与说明</el-button>
            <el-button type="primary" :loading="rebuilding" @click="applyLibraryConfig">应用配置并重建</el-button>
          </div>
      ...
      <section v-if="selectedLibrary" class="settings-section">
        ...
        <el-table :data="tasks" size="small" max-height="280">
          <el-table-column prop="sourcePath" label="文件" min-width="240" show-overflow-tooltip />
...
const runtimeForm = reactive<KnowledgeRuntimeConfig>(createDefaultKnowledgeRuntimeConfig());
const runtimeReady = ref(false);
const selectedLibraryId = ref("");
const libraryName = ref("");
const libraryDescription = ref("");
const libraryConfig = reactive<KnowledgeLibraryIndexConfig>(normalizeKnowledgeLibraryConfig());
...
const selectedLibrary = computed(() => store.libraries.find((item) => item.id === selectedLibraryId.value) || null);
const maxFileMiB = computed({ get: () => Math.round(runtimeForm.maxImportFileBytes / 1024 / 1024), set: (value: number) => { runtimeForm.maxImportFileBytes = value * 1024 * 1024; } });
const semanticEnabled = computed({ get: () => libraryConfig.indexes.semantic, set: (value: boolean) => { libraryConfig.indexes.semantic = value; libraryConfig.embedding.enabled = value; } });
...
watch(runtimeForm, (value) => {
  if (!runtimeReady.value) return;
  saveKnowledgeRuntimeConfigDebounced({ ...value }, (error) => errorHandler.error(error, "保存 Knowledge 运行设置失败"));
}, { deep: true });
...
watch(selectedLibrary, (library) => {
  ...
  Object.assign(libraryConfig, normalizeKnowledgeLibraryConfig(library.config));
  void refreshDiagnostics();
}, { immediate: true });

watch(() => store.activeLibraryId, (libraryId) => { if (libraryId) selectedLibraryId.value = libraryId; }, { immediate: true });

async function resetRuntimeSettings() {
  await ElMessageBox.confirm("恢复运行设置默认值？", "重置设置", { type: "warning", lockScroll: false });
...
async function saveLibraryMetadata() {
  ...
  try {
    ...
    await store.updateActiveLibrary({ name: libraryName.value.trim(), description: libraryDescription.value.trim() || undefined });
...
async function applyLibraryConfig() {
  try {
    ...
    await ElMessageBox.confirm("将按新配置重新分块并重建基础索引；现有活动向量会被清除。", "应用配置并重建", { type: "warning", confirmButtonText: "应用并重建", lockScroll: false });
    rebuilding.value = true;
    await store.selectLibrary(selectedLibraryId.value);
    const count = await store.applyActiveLibraryConfig(normalizeKnowledgeLibraryConfig(libraryConfig));
...
async function refreshDiagnostics() {
  ...
  try {
    [sources.value, tasks.value] = await Promise.all([listKnowledgeSources(selectedLibraryId.value), listKnowledgeIngestTasks(selectedLibraryId.value, 200)]);
    if (store.activeLibraryId === selectedLibraryId.value) await store.refreshIndexStatus();
  } catch (error) { errorHandler.error(error, "读取 Knowledge 诊断信息失败"); } finally { diagnosticsLoading.value = false; }
}
...
async function deleteSource(sourceId: string) {
  try {
    await ElMessageBox.confirm("移除来源及其已导入文档？", "移除来源", { type: "warning", lockScroll: false });
...
onActivated(() => { if (selectedLibraryId.value) void refreshDiagnostics(); });
</script>

<style scoped>
.settings-view { height: 100%; overflow: auto; background: var(--el-bg-color-page); }
.settings-content { width: min(1040px, calc(100% - 32px)); margin: 0 auto; padding: 20px 0 40px; }
.settings-section { padding: 18px 0 22px; border-bottom: var(--border-width) solid var(--border-color); }
.settings-section > header { display: flex; align-items: flex-start; justify-content: space-between; gap: 16px; margin-bottom: 16px; }
h2 { margin: 0; font-size: 16px; letter-spacing: 0; color: var(--el-text-color-primary); }
header p { margin: 4px 0 0; font-size: 12px; color: var(--el-text-color-secondary); }
.settings-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 14px 22px; }
.settings-grid label { display: grid; grid-template-columns: minmax(120px, 1fr) minmax(160px, 1.4fr); align-items: center; gap: 12px; min-width: 0; font-size: 13px; color: var(--el-text-color-regular); }
.settings-grid :deep(.el-input-number), .settings-grid :deep(.el-input) { width: 100%; }
.settings-actions { display: flex; justify-content: flex-end; gap: 10px; margin-top: 18px; }
.diagnostic-strip { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 1px; margin-top: 18px; background: var(--border-color); border: var(--border-width) solid var(--border-color); border-radius: 6px; overflow: hidden; }
.diagnostic-strip div { min-width: 0; padding: 10px 12px; background: var(--card-bg); }
.diagnostic-strip span, .diagnostic-strip strong { display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.diagnostic-strip span { margin-bottom: 4px; font-size: 11px; color: var(--el-text-color-secondary); }
