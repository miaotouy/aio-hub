...
| 职责                                                                | 所属模块                                 | 说明                                                                                                                   |
| ------------------------------------------------------------------- | ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| 类型、索引和完整配置、创建/编辑/复制/删除、导入导出、资产和配置迁移 | `agent-manager`                          | `useAgentStore` 与 `useAgentStorage` 是智能体数据的唯一所有者；大厅和编辑器也在该模块。                                |
| 当前选择、侧边栏和聊天内快速切换                                    | `llm-chat`                               | `currentAgentId` 位于 `useLlmChatUiState`，属于聊天 UI 偏好，不写入智能体索引。                                        |
| 会话展示绑定、开场白、消息生成、历史回放                            | `llm-chat`                               | 会话生命周期管理器在切换 `displayAgentId` 时处理 live greeting；执行器按需读取智能体完整配置并生成本次请求的执行快照。 |
| 从大厅开始聊天                                                      | `agent-manager` 到 `llm-chat` 的导航协作 | `AgentManager` 设置聊天当前智能体后打开 `/llm-chat`；这是一层门户交互，不属于智能体数据层的状态职责。                  |

### 1.4. 用户档案 (UserProfile)

用户档案是一个可复用的用户身份描述，用于在对话中插入用户的背景信息、角色设定等。其类型、状态、存储和管理界面已拆分至平级工具 [`user-profile-manager`](../user-profile-manager/)，`llm-chat` 只负责解析并消费当前对话的有效档案。

- **全局与智能体级别**: `user-profile-manager` 持有 `globalProfileId`；智能体可通过 `userProfileId` 绑定专属档案。有效档案优先级固定为 **Agent 绑定档案 > 全局默认档案**。
- **灵活插入**: 通过预设消息中的 `user_profile` 占位符，可以精确控制档案内容的插入位置。
- **扩展配置**: 档案除身份正文和头像外，还可携带富文本样式、正则规则、世界书和快捷操作集关联。
- **历史稳定性**: 创建用户消息时会把有效档案的 ID、名称、显示名和头像写入消息元数据快照，避免后续切换或修改档案导致历史消息身份展示漂移。

#### 1.4.1. 跨工具职责边界

| 职责                                                       | 所属模块                                      | 说明                                                                                                                                              |
| ---------------------------------------------------------- | --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| 类型、完整档案、全局默认选择、创建/编辑/删除、头像和持久化 | `user-profile-manager`                        | `useUserProfileStore` 与 `useUserProfileStorage` 是用户档案数据的所有者，管理中心和编辑组件也在该模块。                                           |
| 有效档案解析                                               | `llm-chat` 消费 `user-profile-manager`        | 请求前调用 `getEffectiveProfile(agent.userProfileId)`，没有 Agent 专属档案时回退到全局默认档案。                                                  |
| 上下文注入和关联能力                                       | `llm-chat`                                    | 有效档案进入 `PipelineContext`，参与宏、预设锚点、正则、世界书、快捷操作和开场白处理。                                                            |
...
- **统一二进制读取**: [`getAttachmentBuffer()`](./core/context-utils/attachment-binary.ts) 工具函数统一处理 `inline`（Base64 内嵌）、`asset-library`（资产库引用）和 `agent-private`（由 `agent-manager` 管理的智能体私有资产）三种数据源的二进制读取。
...
- **UI 呈现限制**: 撤销/重做操作面板仅在**节点视图（树状对话图）**中提供，线性视图中不展示该操作面板。
...
- **占位符语法**: `【recall::collection=<collection-id>::preset=comprehensive::limit=8::min-score=0.35::when=always】`；参数使用严格命名协议，目标只接受 Agent 已启用 binding 的稳定 ID。`profile` 仅作为 legacy 输入映射到预设。
...
- **召回预设**: 产品配置使用 `algorithmic` / `comprehensive`。前者离线可用且不生成查询向量；后者融合关键词、内容向量、标签和关联信号。底层 engine ID 仅保留给迁移和 Playground 调试。
- **向量空间融合查询**: 主查询取最后一条会话 user 消息，次查询取该消息之前最近的非空 assistant 回复。`comprehensive` 由检索 Runner 分别 embed 两者并按默认 `0.7 / 0.3` 融合成一个请求级 bundle；无 AI 历史时退化为单查询。`algorithmic` 不解析模型或生成向量。user 侧文本先经查询预处理管线（Markdown/HTML/占位符清洗 → `Intl.Segmenter` 分词 → 停用词过滤 → Tag 池 n-gram 匹配）。
- **后端 LRU 检索缓存**: 检索结果缓存位于 Rust Recall 后端（`src-tauri/src/recall/commands/retrieval_cache.rs` 的 `recall_retrieval_cache_*` 系列命令），**全局共享**；缓存键由规范化后的主/次查询、`recallIds`、标签、融合权重、数量、阈值、预设、编译配置哈希、Embedding 身份和算法版本计算 SHA-256，任一参数变化都会形成不同缓存项。Embedding 向量缓存由 `vectorCacheManager` 独立管理。
...
本模块遵循关注点分离的原则，将状态、逻辑和视图清晰地分开。智能体配置与用户档案都是跨工具依赖：聊天运行时分别依赖 `agent-manager` 和 `user-profile-manager` 提供数据，但不拥有它们。

```mermaid
graph LR
    AM[agent-manager 数据层<br/>类型、Store、Storage、编辑器、资产] --> LC[llm-chat 运行时]
    UPM[user-profile-manager 数据层<br/>档案、全局默认、Storage、编辑器、头像] --> LC
    LC --> UI[聊天 UI 状态<br/>currentAgentId]
    LC --> SL[会话生命周期<br/>displayAgentId / greetings]
    LC --> EX[请求执行器<br/>执行快照]
    LC --> EP[有效用户档案<br/>Agent 绑定优先于全局默认]
    PORTAL[agent-manager 大厅] -->|设置当前智能体并导航| UI
```
...
- **State (Pinia Stores & Singletons)**:
  ...
  - `useAgentStore`（来自 `agent-manager`）: 智能体配置的外部数据源。`loadAgents()` 统一全量加载，调用方不应把索引项当成可安全持久化的半完整对象；需要时可通过 `loadAgentDetails()` 显式刷新指定智能体。
  - `useUserProfileStore`（来自 `user-profile-manager`）: 用户档案和 `globalProfileId` 的外部数据源。`loadProfiles()` 使用防重复请求的全量加载，`getEffectiveProfile()` 统一封装 Agent 绑定优先、全局默认回退的解析规则。
  - `useChatInputManager`: 会话级输入框草稿与附件状态管理。
  - `useLlmChatUiState`: UI 状态（侧边栏折叠、`currentAgentId` 等）。选择变化会更新最近使用时间，并在聊天上下文中将当前会话绑定到对应 `displayAgentId`。
- **Logic (Composables)**:
...
graph TD
    ...
    subgraph B [useChatHandler - 指挥中心]
        B1(创建消息节点) --> B2(写入有效用户档案元数据快照)
        B2 --> B3(清空输入框)
        B3 --> B4(清空撤销栈)
        B4 --> B5(调用执行器 `executeRequest`)
    end

    subgraph C [useChatExecutor - 执行器]
        C1(从 agent-manager 获取完整配置) --> C2(生成本次请求的执行 Agent 快照)
        C2 --> C3(从 user-profile-manager 解析有效档案)
        C3 --> C4(创建 PipelineContext) --> C5(调用统一管道)
    end
...
- **智能体消费与切换**: `useLlmChatUiState` 维护选择及会话绑定；`useChatExecutor` / `useSingleNodeExecutor` 获取 `agent-manager` 的完整配置并构造执行 Agent；`sessionLifecycleManager` 在会话切换或展示智能体切换时按需加载配置、刷新 live greeting。
- **用户档案消费**: `useChatHandler` 在用户节点上保存有效档案快照；`useChatExecutor` 将有效档案传入 `PipelineContext` 并更新最后使用时间；`useChatAreaContext` 负责聊天区域的有效档案展示和对外部编辑器的就地调用。
...
- **`ChatAgent`**: 可复用的智能体配置模板（含预设、参数、知识库、工具、私有资产、世界书、虚拟时间等多达 20+ 个字段），定义于 `agent-manager` 并由本模块重导出。
- **`UserProfile`**: 可复用的用户身份配置（含正文、头像、显示样式、正则、世界书和快捷操作集关联），定义于 `user-profile-manager` 并由本模块重导出。
