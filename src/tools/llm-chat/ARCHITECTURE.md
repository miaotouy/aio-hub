...
宏系统是一个强大的动态内容生成引擎，它允许在智能体的预设消息、用户输入等任何地方嵌入可执行的占位符。详细的宏定义与用法请参考：[MACRO.md](./MACRO.md)。
...
转写系统旨在弥合多模态资产与纯文本模型之间的鸿沟。

- **统一集成**: 转写功能已完全集成到上下文管道中。这意味着转写结果不仅用于显示，更是上下文构建的一等公民。
...
- **视频处理**: 集成 FFmpeg 对视频进行压缩或抽帧，支持大文件处理。
