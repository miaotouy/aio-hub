...
### 1.11. SillyTavern 兼容性 (SillyTavern Compatibility)
...
### 1.13. 模型参数配置系统 (Model Parameter System)
...
### 1.14. 正则管道系统 (Regex Pipeline System)
...
- **绑定模式**: 用户可在设置中选择正则规则的绑定模式：
  - **消息绑定 (message)**: 默认模式，每条消息使用其生成时的 Agent/User 配置。
  - **会话绑定 (session)**: 所有消息统一使用当前会话的 Agent/User 配置，便于在切换智能体后统一应用新规则。
...
### 1.15. 翻译系统 (Translation System)

为了打破语言障碍，系统集成了原生的多语言翻译能力，支持消息内容和用户输入的实时翻译。
...
### 1.16. 上下文压缩 (Context Compression)
...
### 1.17. 转写系统 (Transcription System)
...
### 2.3. 消息列表与渲染 (Message List & Rendering)
...
### 2.6. 窗口分离与同步 (Detached Window & Sync)
...
- **`useTranscriptionManager`**: **转写任务调度器**。
  - 管理转写任务队列（并发、重试）。
  - 执行智能转写策略判断（Smart/Always）。
  - 协调视频压缩（调用 Rust/FFmpeg）和 LLM 请求。
  - 管理衍生数据的持久化和状态更新。
