...
详细字段定义见 [`key-types.md`](./docs/architecture/key-types.md)。
...
- **预设消息多模态附件**: 预设消息支持引用智能体资产作为多模态附件（图片、音频、视频等），通过 `PresetAttachmentRef` 声明附件引用关系，在发送时由 `preset-attachment-resolver` 解析为管道附件。编辑器提供 [`PresetAttachmentPicker`](src/tools/llm-chat/components/agent/editors/PresetAttachmentPicker.vue:1) 组件管理附件选择、缩略图预览与音视频试听。导入导出系统会检测悬空附件引用并给出警告，Token 计算器支持按附件类型估算消耗。
...
- **统一二进制读取**: [`getAttachmentBuffer()`](src/tools/llm-chat/core/context-utils/attachment-binary.ts:1) 工具函数统一处理 `inline`（Base64 内嵌）、`asset-library`（资产库引用）和 `agent-private`（智能体私有资产）三种数据源的二进制读取。
...
- **对话树图**: 基于 Vue Flow + D3.js 实现的可视化工具，支持三种布局模式（树状/物理/静态）与子树拖拽嫁接。性能设计要点：
  ...
  - **D3 性能**: Tick 回调及热路径使用 `Map` 索引，子树拖拽使用 `Set` 判定；HUD 隐藏时暂停 FPS 统计 RAF 循环；主题调色板变更前做等值比较，防止无效触发。
...
- **Logic (Composables)**:
  ...
  - **[`useStreamingMessageSources`](src/tools/llm-chat/composables/chat/useStreamingMessageSources.ts:1)**：管理独立的流式消息源，将流式内容的实时渲染通道与降频持久化通道解耦（详见第 5 节数据流）。
- **View (Vue Components)**:
