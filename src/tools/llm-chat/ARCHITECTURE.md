...
- **智能检索策略**:
  - **上下文感知查询**: 取最近 N 轮完整对话（User + AI + Tool）组合为检索查询，对齐 VCP 记忆系统策略。
  - **双引擎检索**: 支持向量检索 (Vector) 和关键词检索 (Keyword/Fulltext)，以及混合检索 (Hybrid)。
  - **精确文本缓存**: 实现会话级检索缓存，采用精确文本匹配策略（完全一致才命中），适用于同轮多占位符和重试场景。
- **每次独立检索**: 对齐 VCP 设计，每次检索独立执行，不混入历史结果。分数阈值 (`minScore`) 作为主要截断依据，`limit` 仅为上限。
...
- **核心功能**: 提供一个基于 Monaco Editor 的 JSON 编辑界面，用于修改 `ChatMessageNode` 的所有属性，支持一键复制。
...
2.  **异步任务处理器 (`async-task-processor`)**
    - **职责**: 检测工具调用节点中的异步任务，从 `asyncTaskStore` 获取最新状态，并动态替换节点内容。
    - **位置**: 必须在消息加载后、正则处理前执行。
...
- **分离层 (Detached Layer)**: 处理悬浮窗模式下的特殊样式（如壁纸、悬浮窗手柄）。
...
- **拖拽控制**: 集成了 `ComponentHeader`，在悬浮窗模式下作为窗口拖拽手柄。
...
- **`ChatAgent`**: 可复用的配置模板。
  ...
  - `knowledgeSettings`: RAG 检索配置，包含召回上限、分数阈值、上下文窗口（轮数）、精确文本缓存开关等。
