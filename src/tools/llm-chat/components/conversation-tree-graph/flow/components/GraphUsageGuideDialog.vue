  ...
  <BaseDialog v-model="isDialogVisible" title="会话关系图使用说明" width="70vw">
    <div class="usage-guide-content">
      ...
      <ul>
        ...
        <li>
          <strong>右键菜单:</strong>
          右键点击节点可进行复制、禁用/启用、删除等操作。
        </li>
      </ul>
      <h3>历史记录</h3>
      <ul>
        <li>
          <strong>撤销/重做:</strong> 点击顶部的
          <el-icon><ArrowLeft /></el-icon> 和
        ...
        <li>
          ...
          按钮可以查看详细的操作历史记录，并跳转到任意历史状态。
        </li>
        <li>
          <strong>历史断点:</strong>
          请注意，<strong>发送新消息</strong>或<strong>重新生成回答</strong>属于“历史断点”操作。执行这些操作后，当前会话的撤销/重做历史将被清空，以反映新的对话分支。
        </li>
      </ul>

      <h3>调试模式</h3>
      <p>
        如需使用调试功能，请先在“聊天设置” -> “开发者选项”中开启“启用调试模式”。
      </p>
      <p>
      ...
      <h3>快捷键配置</h3>
      <p>
        所有交互快捷键（如拖拽、嫁接、撤销、重做等）均可在“聊天设置” ->
        “快捷键”中进行自定义。
      </p>
    </div>
    <template #footer>
...
const undoKey = computed(() =>
  formatShortcutDisplay(settings.value.shortcuts.undo)
);
const redoKey = computed(() =>
  formatShortcutDisplay(settings.value.shortcuts.redo)
);
