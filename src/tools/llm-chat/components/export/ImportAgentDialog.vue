  ...
  >
    <template #content>
      <div v-if="preflightResult" class="import-dialog-content">
        <!-- 总体情况 -->
        <ElAlert title="即将导入智能体" type="info" :closable="false" show-icon class="summary-alert">
          <template #default>
            <p>
            ...
            <p v-if="Object.keys(preflightResult.bundledWorldbooks || {}).length > 0">
              包含
              <strong>{{ Object.values(preflightResult.bundledWorldbooks || {}).flat().length }}</strong> 个随包世界书。
            </p>
        ...
        <div class="agents-list-section">
          <h4>请确认以下智能体的导入方式</h4>
          <div v-for="(agent, index) in preflightResult.agents" :key="index" class="agent-resolve-item">
            ...
            <div
              ...
              v-if="preflightResult.bundledWorldbooks?.[agent.id!] || preflightResult.embeddedWorldbooks?.[agent.id!]"
            >
              <h5>世界书处理</h5>
              <div class="wb-import-item" v-if="preflightResult.bundledWorldbooks?.[agent.id!]">
                ...
                <div class="wb-list-hint">
                  <span v-for="wb in preflightResult.bundledWorldbooks[agent.id!]" :key="wb.id" class="wb-name-tag">
                    ...
                    <template
                      v-if="
                        preflightResult.worldbookConflicts?.[agent.id!]?.bundled?.find((b) => b.name === wb.name) as any
                      "
                    >
                      <ElTooltip
                        v-if="
                          preflightResult.worldbookConflicts![agent.id!].bundled.find((b) => b.name === wb.name)
                            ?.isDuplicate
                        "
                        content="已存在完全相同的世界书，将自动复用"
                        placement="top"
                      >
                        <ElTag type="success" size="small" class="wb-dup-tag">已有</ElTag>
                      </ElTooltip>
                      <ElTooltip
                        v-else-if="
                          preflightResult.worldbookConflicts![agent.id!].bundled.find((b) => b.name === wb.name)
                        ...
                        placement="top"
                      >
                        <ElTag type="warning" size="small" class="wb-dup-tag">同名</ElTag>
                      </ElTooltip>
                    </template>
                  </span>
                </div>
              ...
              <div class="wb-import-item" v-if="preflightResult.embeddedWorldbooks?.[agent.id!]">
                ...
                <span
                  v-if="preflightResult.worldbookConflicts?.[agent.id!]?.embedded?.isDuplicate"
                  class="wb-embedded-hint"
                >
                  <ElTooltip content="已存在完全相同的世界书，将自动复用" placement="top">
            ...
            <!-- 冲突解决选项 -->
            <div class="resolve-options" v-if="preflightResult.unmatchedModels.find((m) => m.agentIndex === index)">
              <!-- 模型不匹配解决 -->
    ...
    <template #footer>
      ...
      <ElButton type="primary" @click="handleConfirm" :disabled="!preflightResult || loading" :loading="loading">
        确认导入
      </ElButton>
    </template>
  </BaseDialog>
