...
import type { LlmContextData, ContextPreviewData, ProcessableMessage } from "../types/context";
...
export function useContextPreview(buildLlmContext: (...args: any[]) => Promise<LlmContextData>) {
  ...
  const getLlmContextForPreview = async (
    ...
    if (!effectiveAgentId) {
      logger.warn("⚠️ 无法确定 Agent，将只计算会话历史（不包含智能体预设）", { targetNodeId, providedAgentId: agentId });
    }
    ...
    if (effectiveAgentId) {
      agentConfig = agentStore.getAgentConfig(effectiveAgentId, { parameterOverrides: session.parameterOverrides });
    ...
    if (agentConfig) {
      ...
      const mergedRules = [...agentRules, ...modelRulesObjects.filter((r: any) => !agentRuleTypes.has(r.type))];
      ...
      messages = await Promise.all(nodePath
        ...
        .map(async (node: ChatMessageNode) => {
          ...
          const content = await buildMessageContentForLlm(node.content, node.attachments, undefined);
    ...
    if (agentConfig) {
      ...
      for (let i = 0; i < rawPresets.length; i++) {
        ...
        if (msg.type === "user_profile") {
          if (effectiveUserProfile) {
            itemsToProcess.push({ role: msg.role || "system", content: `# 用户档案\n${effectiveUserProfile.content}`, originalContent: `# 用户档案\n${effectiveUserProfile.content}`, index: i, isUserProfile: true });
          }
          continue;
        }
        const contentStr = typeof msg.content === "string" ? msg.content : JSON.stringify(msg.content);
        itemsToProcess.push({ role: msg.role, content: contentStr, originalContent: contentStr, index: i });
      }

      if (!hasUserProfilePlaceholder && effectiveUserProfile) {
        const newItem = { role: "system" as const, content: `# 用户档案\n${effectiveUserProfile.content}`, originalContent: `# 用户档案\n${effectiveUserProfile.content}`, index: rawPresets.length, isUserProfile: true };
        let lastSystemIndex = itemsToProcess.slice().reverse().findIndex(item => item.role === "system");
        if (lastSystemIndex !== -1) lastSystemIndex = itemsToProcess.length - 1 - lastSystemIndex;
      ...
      presetMessagesData = await Promise.all(
        itemsToProcess.map(async (item) => {
          let content = await processMacros(item.content, { session, agent: agent ?? undefined, userProfile: effectiveUserProfile, timestamp: targetTimestamp });
          ...
          try {
            ...
            if (tokenResult.tokenizerName && !tokenizerName) tokenizerName = tokenResult.tokenizerName;
          } catch (error) {
            logger.warn("计算预设消息 token 失败", { index: item.index, error });
          }
          return { role: item.role, content, originalContent: item.originalContent, charCount: sanitizedContent.length, tokenCount, source: "agent_preset", index: item.index, userName: item.role === "user" ? effectiveUserProfile?.name : undefined, userIcon: item.role === "user" ? effectiveUserProfile?.icon : undefined, isUserProfile: item.isUserProfile };
        })
      );
    }
    ...
    const chatHistoryData = await Promise.all(
      nodePath
        ...
        .map(async (node: ChatMessageNode, index: number) => {
          ...
          if (agentConfig) {
            try {
              const textTokenResult = await tokenCalculatorService.calculateTokens(originalText, agentConfig.modelId);
              textTokenCount = textTokenResult.count;
              if (textTokenResult.isEstimated) isEstimated = true;
              if (textTokenResult.tokenizerName && !tokenizerName) tokenizerName = textTokenResult.tokenizerName;
            } catch (error) {
              logger.warn("计算历史消息文本 token 失败", { nodeId: node.id, error });
            }
          }
          ...
          if (agentConfig && node.attachments && node.attachments.length > 0) {
            ...
            for (const item of textAttachments) {
              let tokenCount: number | undefined; let isAttachmentEstimated = false;
              try {
                const result = await tokenCalculatorService.calculateTokens(item.content, agentConfig.modelId);
                tokenCount = result.count; isAttachmentEstimated = result.isEstimated ?? false;
              } catch (error) { isAttachmentEstimated = true; }
              if (tokenCount !== undefined) attachmentsTokenCount += tokenCount;
              if (isAttachmentEstimated) isEstimated = true;
              attachmentsData.push({ id: item.asset.id, name: item.asset.name, type: item.asset.type, path: item.asset.path, importStatus: item.asset.importStatus, originalPath: item.asset.originalPath, size: item.asset.size, tokenCount, isEstimated: isAttachmentEstimated, metadata: item.asset.metadata });
            }
            for (const asset of imageAttachments) {
              let tokenCount: number | undefined; let isAttachmentEstimated = false; let attachmentError: string | undefined;
              if (visionTokenCost) {
                if (asset.metadata?.width && asset.metadata?.height) {
                  try { tokenCount = tokenCalculatorEngine.calculateImageTokens(asset.metadata.width, asset.metadata.height, visionTokenCost); } catch (e) { attachmentError = e instanceof Error ? e.message : "图片 Token 计算异常"; isAttachmentEstimated = true; }
                } else { attachmentError = "缺少图片尺寸信息，使用默认值估算"; tokenCount = tokenCalculatorEngine.calculateImageTokens(1024, 1024, visionTokenCost); isAttachmentEstimated = true; }
              } else { attachmentError = "模型不支持视觉能力或计费规则未知"; isAttachmentEstimated = true; }
              if (tokenCount !== undefined) attachmentsTokenCount += tokenCount;
              if (isAttachmentEstimated) isEstimated = true;
              attachmentsData.push({ id: asset.id, name: asset.name, type: asset.type, path: asset.path, importStatus: asset.importStatus, originalPath: asset.originalPath, size: asset.size, tokenCount, isEstimated: isAttachmentEstimated, metadata: asset.metadata, error: attachmentError });
            }
            for (const asset of videoAttachments) {
              let tokenCount: number | undefined; let isAttachmentEstimated = false; let attachmentError: string | undefined;
              if (asset.metadata?.duration) { try { tokenCount = tokenCalculatorEngine.calculateVideoTokens(asset.metadata.duration); } catch (e) { attachmentError = e instanceof Error ? e.message : "视频 Token 计算异常"; isAttachmentEstimated = true; } } else { attachmentError = "缺少视频时长信息，无法计算"; isAttachmentEstimated = true; }
              if (tokenCount !== undefined) attachmentsTokenCount += tokenCount;
              if (isAttachmentEstimated) isEstimated = true;
              attachmentsData.push({ id: asset.id, name: asset.name, type: asset.type, path: asset.path, importStatus: asset.importStatus, originalPath: asset.originalPath, size: asset.size, tokenCount, isEstimated: isAttachmentEstimated, metadata: asset.metadata, error: attachmentError });
            }
            for (const asset of audioAttachments) {
              let tokenCount: number | undefined; let isAttachmentEstimated = false; let attachmentError: string | undefined;
              if (asset.metadata?.duration) { try { tokenCount = tokenCalculatorEngine.calculateAudioTokens(asset.metadata.duration); } catch (e) { attachmentError = e instanceof Error ? e.message : "音频 Token 计算异常"; isAttachmentEstimated = true; } } else { attachmentError = "缺少音频时长信息，无法计算"; isAttachmentEstimated = true; }
              if (tokenCount !== undefined) attachmentsTokenCount += tokenCount;
              if (isAttachmentEstimated) isEstimated = true;
              attachmentsData.push({ id: asset.id, name: asset.name, type: asset.type, path: asset.path, importStatus: asset.importStatus, originalPath: asset.originalPath, size: asset.size, tokenCount, isEstimated: isAttachmentEstimated, metadata: asset.metadata, error: attachmentError });
            }
            for (const asset of otherAttachments) {
              attachmentsData.push({ id: asset.id, name: asset.name, type: asset.type, path: asset.path, importStatus: asset.importStatus, originalPath: asset.originalPath, size: asset.size, tokenCount: undefined, isEstimated: true, metadata: asset.metadata, error: "暂不支持此类型附件的 Token 计算" });
              isEstimated = true;
            }
          }

          const totalNodeTokenCount = (textTokenCount ?? 0) + attachmentsTokenCount;
          if (textTokenCount !== undefined) chatHistoryTokenCount += totalNodeTokenCount;

          let msgAgentName: string | undefined, msgAgentIcon: string | undefined, msgUserName: string | undefined, msgUserIcon: string | undefined;
          if (node.role === 'assistant') {
            ...
            if (node.metadata?.agentIcon && msgAgentId) { msgAgentIcon = resolveAvatarPath({ id: msgAgentId, icon: node.metadata.agentIcon }, 'agent') || undefined; }
            if (!msgAgentIcon && storeAgent) { msgAgentIcon = resolveAvatarPath(storeAgent, 'agent') || undefined; }
          } else if (node.role === 'user') {
            msgUserName = node.metadata?.userProfileName;
            if (node.metadata?.userProfileIcon) msgUserIcon = node.metadata.userProfileIcon;
          }

          return { role: node.role, content: originalText, charCount: sanitizedContent.length, tokenCount: textTokenCount !== undefined ? totalNodeTokenCount : undefined, source: "session_history", nodeId: node.id, index, agentName: msgAgentName, agentIcon: msgAgentIcon, userName: msgUserName, userIcon: msgUserIcon, attachments: attachmentsData.length > 0 ? attachmentsData : undefined };
        })
    );
    ...
    if (agentConfig) {
      ...
      if (mergedRules.length > 0) {
        ...
        if (deltaContent) {
          ...
          } catch (error) { logger.warn("计算后处理增量 Token 失败", { error }); }
        }
      }
    }
    ...
    return {
      ...
      statistics: { totalCharCount, presetMessagesCharCount, chatHistoryCharCount, postProcessingCharCount: agentConfig ? postProcessingCharCount : undefined, messageCount: messages.length, totalTokenCount: agentConfig ? totalTokenCount : undefined, presetMessagesTokenCount: agentConfig ? presetMessagesTokenCount : undefined, chatHistoryTokenCount: agentConfig ? chatHistoryTokenCount : undefined, postProcessingTokenCount: agentConfig ? postProcessingTokenCount : undefined, isEstimated: agentConfig ? isEstimated : undefined, tokenizerName: agentConfig ? tokenizerName : undefined },
      agentInfo: { id: effectiveAgentId ?? '', name: targetNode.metadata?.agentName || agent?.name, icon: targetNode.metadata?.agentIcon || resolveAvatarPath(agent, 'agent') || undefined, profileId: targetNode.metadata?.profileId || agentConfig?.profileId || '', modelId: targetNode.metadata?.modelId || agentConfig?.modelId || '', virtualTimeConfig: targetNode.metadata?.virtualTimeConfig || agent?.virtualTimeConfig || agentConfig?.virtualTimeConfig },
      parameters: (() => {
        if (targetNode.metadata?.requestParameters) return targetNode.metadata.requestParameters;
      ...
      userInfo: { id: effectiveUserProfile?.id, name: effectiveUserProfile?.name, displayName: effectiveUserProfile?.displayName, icon: effectiveUserProfile?.icon },
    };
  };
