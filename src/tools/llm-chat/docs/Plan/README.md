...
| 顺序 | 文档                                                                              | 状态   | 当前动作                                                                    |
| ---- | --------------------------------------------------------------------------------- | ------ | --------------------------------------------------------------------------- |
| 1    | [会话持久化损坏与启动阻塞调查](./session-persistence-corruption-investigation.md) | 待实施 | 先做单写者与原子替换，再做备份恢复和非阻塞重建                              |
| 2    | [Agent 配置解耦与智能体大厅](./agent-decoupling-plan.md)                          | 待收口 | 已发布；清理活动旧路径写入和 Rust 搜索旧目录，补跨目录迁移与重复启动测试    |
...
| 5    | [多会话状态与未来规划](./multi-session-status.md)                                 | 待实施 | 先做后台会话执行服务，再排多窗口 UI                                         |
