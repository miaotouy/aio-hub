...
直接导入单个 `config.json` 或 zip 只作为兼容入口，不作为主流程。
...
- 导入包含 `config.json`、`avatar.*`、`regex_rules.json` 的 zip
- 拖拽 `.json` / `.zip`

浏览器 File API 无法从单个 `config.json` 自动读取同级头像，所以裸 JSON 导入只导入配置；需要头像和正则时使用 zip 或拖拽包含资源的包。
...
9. 优化 `ImportAgentDialog.vue`：
   ...
   - 支持问题项筛选和批量模型应用。
   - 保留现有世界书导入选项。
10. 在 zip 解析中识别 VCPChat Agent 目录结构，收集头像和正则文件，作为兼容入口；该识别逻辑应复用 `vcpChatAgentImportService.ts` 的转换函数。
11. 放宽侧栏普通文件导入选择器，允许直接选择普通 `.json` 作为 VCPChat `config.json`。
...
- 目录导入和 zip 导入中的 `avatar.*` 都被作为导入资产迁移。
