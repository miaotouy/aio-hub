...
**核心方案**：专用截图组件 + **并发消息截图** + `modern-screenshot`（纯前端，零 Rust 代码）+ **实用效果开关**。
...
│   └── ShareScreenshotDialog.vue     # 截图预览与配置弹窗（第二步，包含实用效果开关）
...
- **关键改进（V2）**：
  - **不再截取整个大容器**：V1 方案是“隐藏所有消息 -> 逐组显示 -> 截取整个大容器”，导致 `modern-screenshot` 每次都要克隆整个大容器的 DOM，解析样式，下载资源，渲染成 SVG 再转 Canvas。如果消息有 20 条，分了 5 组，它就要克隆 5 次大容器，每次克隆都包含一堆隐藏的元素，这极其低效且容易因为虚拟滚动导致截出空白！
...
> ⚠️ **注意**：AI 之前臆想出来的“防止徽章缩水”的 CSS（如 `flex-shrink: 0 !important` 和 `white-space: nowrap !important`）经实际测试**完全没有效果**，且容易破坏原本精细的 CSS 布局。在 V2 实施中**彻底删除**这些无用样式，只保留最稳健的排版保护。

---

## 4.2 长图生成：纯 Canvas 2D 拼接 + 极简后处理

- **抛弃花哨主题**：砍掉 AI 之前规划的“极光渐变”、“赛博朋克”等 4 种视觉模板。这些模板不仅占据了弹窗极大的配置空间，分散了开发精力，而且气泡背景与渐变底色叠加时边缘非常生硬，可读性极差。
- **回归实用效果开关**：
  1. **还原模糊背景**：
     - 拼接大 Canvas 时，背景色直接读取当前系统主题的 `--container-bg`（带毛玻璃通透感）。
     - 如果开启了“还原模糊背景”，我们可以在 Canvas 底层绘制一层带微弱毛玻璃质感的半透明遮罩，让消息气泡自然地“浮”在上面。
  2. **加外边框与投影**：
...
提取为 `useMessageLayout` composable，`MessageList.vue` 和 `ScreenshotRenderer.vue` 共同使用，删除约 180 行重复布局计算代码。

---

### 4.4 交互抑制：`screenshotMode`
...
- **左侧**：实时预览区域，支持缩放和拖拽查看。展示 `ScreenshotRenderer` 渲染的干净、排版完全正确的独立 DOM 树。
- **右侧配置面板**（极简清爽，把绝大部分空间留给左侧预览）：
  - **效果开关**：
    - `[x] 还原模糊背景` (还原原本容器的通透感)
    - `[x] 显示卡片外边框` (让长图更有边界感)
    - `[x] 开启卡片投影` (增加立体感)
    - `[x] 附加极简水印`
  - **折叠策略**：跟随现状 / 跟随配置 / 强制展开 / 强制收起
...
2. **`ShareScreenshotDialog.vue`**：
   ...
   - 配置面板包含：效果开关、折叠策略、卡片元素开关。
   - 截图时调用 `rendererRef.value?.getMessageElements()` 获取消息节点列表，传给 `useScreenshotGenerator` 的 `generateScreenshot`。
