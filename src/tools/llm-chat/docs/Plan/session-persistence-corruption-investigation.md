...
> **状态**：Phase 0 至 Phase 2 已实施；Phase 3 保持长期评估
>
...
本次已完成 Phase 0 至 Phase 2 的可发布实现：

- 兼容 facade 已将会话内容、索引和当前会话选择的写入接入主窗口
  `SessionPersistenceCoordinator`；每个会话实行“运行一个、等待一个最新状态”，并对持续
  I/O 失败使用有上限的退避重试，避免微任务热循环。
- Rust `llm_chat_persistence` command 以领域标识而不是任意路径进行原子写入，并覆盖同进程
  路径锁、跨进程文件锁、临时文件刷盘、Windows `ReplaceFileW`、索引有效备份和修订号拒绝。
  删除先写墓碑，再把内容移入 trash，批量清空也先关闭对应写槽，避免迟到写入复活文件。
- 加载层会严格验证主索引、备份、残留临时索引和会话 payload；索引不可用时不再伪造空索引。
  主索引损坏样本与会话坏文件会被保留/隔离，恢复在首屏后后台执行，包含进度、取消、失败数量、
  打开隔离目录、导出诊断及清理隔离文件入口。
- 正常启动只加载索引和当前会话，再对目录文件名做增量核对；手动“刷新会话列表索引”复用同一
  进度、取消和结果报告路径，不再存在启动后 3 秒的无条件全量 `repairIndex()`。
- 分离窗口不再调用 `store.loadSessions()`；其会话状态由现有 WindowSyncBus 消费者提供，且
  facade 会拒绝分离组件窗口的原生写入命令。

Phase 3 的 SQLite 演进按“长期评估”保留，未在本次将现有文件格式迁移到 SQLite。真实进程中止、
Windows 文件占用和多 WebView 竞争仍需要在 Tauri E2E / Windows UI Automation 环境按第 16 节单独
执行；它们不是普通浏览器或 Vitest 可替代的验证层。
...
4. 删除无条件 `repairIndex()`，建立 2000 会话性能基线。
