...
- **与 `agent-asset://` 协议的边界**: 两者**作用域完全独立**——`skill_read_file` 读取 **Skill 目录内的文档/代码**（位于 `{appConfigDir}/skill-manager/skills/{skill_id}/...`），主要供模型理解 Skill 用法；`agent-asset://` 协议读取 **Agent 私有的媒体资产**（位于 `{appConfigDir}/agent-manager/agents/{agent_id}/assets/...`，详见 [`agent-assets.md`](./agent-assets.md)），主要供消息内联渲染。两者不共享路径、不共享权限、不互相暴露。
