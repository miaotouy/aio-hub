...
用于在智能体预设消息中注入工具定义、使用说明及运行时上下文。

| 宏名称         | 说明                                                           | 示例                        |
| :------------- | :------------------------------------------------------------- | :-------------------------- |
| `tools`        | 注入可用工具定义。支持通过参数指定工具 ID 列表（无视启用开关） | `{{tools::toolId1}}`        |
| `tool_usage`   | 注入当前工具调用协议的使用说明（如 VCP 格式说明）              | `{{tool_usage}}`            |
| `tool_context` | 注入已启用工具的实时运行时上下文。支持通过参数指定工具 ID 列表 | `{{tool_context::toolId1}}` |

**使用场景**：

- **动态工具注入**: 使用 `{{tools}}` 自动注入 Agent 配置的所有工具。
- **强制工具指定**: 使用 `{{tools::toolId1::toolId2}}` 强制注入特定工具，即使用户在 UI 中关闭了工具调用开关，该宏依然会生效。
- **上下文感知**: 使用 `{{tool_context}}` 让 LLM 了解工具的实时状态（如文件列表、系统负载等）。
...
- 这些宏均在 `SUBSTITUTE` 阶段执行。
- `tools`: 若未传参且 `toolCallConfig.enabled` 为 `false`，则返回空。
- `tool_context`: 会同时聚合工具和插件（Extension）的上下文。

### 3.9. CSS 变量宏

获取当前主题下 CSS 自定义属性的实际值，用于在视觉化输出中提供准确的样式信息。

| 宏名称   | 说明                                                | 示例                                                         |
| :------- | :-------------------------------------------------- | :----------------------------------------------------------- |
| `cssvar` | 获取 CSS 变量的当前实际值（支持带或不带 `--` 前缀） | `{{cssvar::--primary-color}}` 或 `{{cssvar::primary-color}}` |

**使用场景**：

- 在视觉化输出指南中展示当前主题的实际颜色值
- 帮助 LLM 理解当前主题配色，避免误用（如将浅色主题色用作背景）
- 动态生成与当前主题匹配的样式建议

**示例**：

```markdown
当前主题色为 {{cssvar::--primary-color}}，卡片背景为 {{cssvar::--card-bg}}。
请使用 `var(--card-bg)` 作为容器背景，而不是 `var(--primary-color)`。
```

**注意事项**：

- 该宏在 `POST_PROCESS` 阶段执行，确保 DOM 已完全加载
- 如果变量未定义，将返回 `(未定义: 变量名)` 提示
- 返回的是浏览器计算后的实际值（如 `#3b82f6` 或 `rgba(255, 255, 255, 0.8)`）
