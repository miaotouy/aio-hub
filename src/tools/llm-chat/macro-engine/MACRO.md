...
用于在智能体预设消息中注入工具定义，使 LLM 能够感知和调用可用工具。

| 宏名称  | 说明                                                   | 示例        |
| :------ | :----------------------------------------------------- | :---------- |
| `tools` | 注入当前智能体可用的工具定义（根据配置的协议格式生成） | `{{tools}}` |

**使用场景**：

- 在系统提示词中自动注入工具定义，无需手动维护
- 根据智能体的工具调用配置动态生成协议格式（VCP/OpenAI/Anthropic 等）
- 当工具调用功能未启用时，宏会返回空字符串
...
- 该宏在 `SUBSTITUTE` 阶段执行
- 仅当智能体的 `toolCallConfig.enabled` 为 `true` 时才会输出内容
- 输出格式由 `toolCallConfig.protocol` 决定（默认为 `vcp`）
- 工具定义会根据智能体配置的可用工具列表动态生成
