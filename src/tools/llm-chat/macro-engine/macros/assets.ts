  ...
  const assetMacros: MacroDefinition[] = [
    {
      ...
      description:
        "列出当前智能体可用的资产。可选参数：分组ID，只列出该分组下的资产",
      example: "{{assets}}",
      acceptsArgs: true,
      priority: 100,
      supported: true,
      ...
      execute: (context, args) => {
        ...
        if (groupFilter) {
          if (groupFilter === "default") {
            ...
            const definedGroupIds = new Set(
              context.agent?.assetGroups?.map((g) => g.id) || []
            );
            assets = allAssets.filter(
              (a) =>
                !a.group ||
                a.group === "default" ||
                !definedGroupIds.has(a.group)
            );
          } else {
