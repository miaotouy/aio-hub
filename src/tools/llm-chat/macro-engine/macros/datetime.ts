...
function calculateEffectiveTime(context: MacroContext): Date {
  ...
  const currentRealTime = context.timestamp
...
function getShichen(
  hour: number,
  minute: number = 0
): { name: string; index: number; ke: number } {
  // 子时从23点开始，所以需要调整
...
function formatWithPattern(date: Date, pattern: string): string {
  ...
  result = result.replace(
    /YY/g,
    createPlaceholder((year % 100).toString().padStart(2, "0"))
  );
  ...
  result = result.replace(
    /MM/g,
    createPlaceholder((month + 1).toString().padStart(2, "0"))
  );
  ...
  result = result.replace(
    /DD/g,
    createPlaceholder(day.toString().padStart(2, "0"))
  );
  ...
  result = result.replace(
    /HH/g,
    createPlaceholder(hours.toString().padStart(2, "0"))
  );
  result = result.replace(/H/g, createPlaceholder(hours.toString()));
  result = result.replace(
    /hh/g,
    createPlaceholder(hour12.toString().padStart(2, "0"))
  );
  ...
  result = result.replace(
    /mm/g,
    createPlaceholder(minutes.toString().padStart(2, "0"))
  );
  ...
  result = result.replace(
    /ss/g,
    createPlaceholder(seconds.toString().padStart(2, "0"))
  );
