...
export async function importSTWorldbook(file: File): Promise<string | null> {
  try {
    if (file.name.endsWith(".png")) {
      ...
      if (
        aioBundle &&
        aioBundle.type === "AIO_Agent_Export" &&
        aioBundle.agents?.[0]
      ) {
        const agent = aioBundle.agents[0];
        // 尝试提取内嵌的世界书
        const characterBook =
          (agent as any).character_book || (agent as any).data?.character_book;
        if (characterBook && characterBook.entries) {
          const worldbookStore = useWorldbookStore();
          const name =
            characterBook.metadata?.name ||
            agent.displayName ||
            agent.name ||
            file.name.replace(".png", "");
          return await worldbookStore.importWorldbook(
            name,
            characterBook as STWorldbook
          );
        }
      }
      ...
      if (stCharacter && isCharacterCard(stCharacter)) {
        const characterBook =
          stCharacter.character_book || stCharacter.data?.character_book;
        if (characterBook && characterBook.entries) {
          const worldbookStore = useWorldbookStore();
          const name =
            characterBook.metadata?.name ||
      ...
      throw new Error(
        "该 PNG 文件不包含有效的 SillyTavern 角色卡或 AIO Bundle 数据"
      );
    }
 ...
 */
export async function importSTWorldbookFromPath(
  path: string
): Promise<string | null> {
  try {
 ...
 * 内部通用导入逻辑
 */
async function importSTWorldbookFromText(
  text: string,
  fileName: string
): Promise<string | null> {
...
export function normalizeWorldbook(data: any): STWorldbook {
  ...
  entriesArray.forEach((entry: any, index: number) => {
    ...
    result.entries[uidStr] = {
      ...
      selectiveLogic:
        ext.selectiveLogic ?? e.selectiveLogic ?? e.selective_logic ?? 0,
      ...
      useProbability:
        e.useProbability ?? e.use_probability ?? ext.useProbability ?? true,
      ...
      useGroupScoring:
        e.useGroupScoring ?? e.use_group_scoring ?? ext.use_group_scoring,
      excludeRecursion: !!(
        e.excludeRecursion ??
        e.exclude_recursion ??
      ...
      delayUntilRecursionLevel:
        e.delayUntilRecursionLevel ??
        e.delay_until_recursion_level ??
        ext.delay_until_recursion_level ??
        1,
      scanDepth: e.scanDepth ?? e.scan_depth ?? ext.scan_depth,
      caseSensitive: e.caseSensitive ?? e.case_sensitive ?? ext.case_sensitive,
      matchWholeWords:
        e.matchWholeWords ?? e.match_whole_words ?? ext.match_whole_words,
      ...
      matchCharacterDescription:
        ...
        e.match_character_description ??
        ext.match_character_description ??
        true,
      matchCharacterPersonality:
        e.matchCharacterPersonality ??
        e.match_character_personality ??
        ext.match_character_personality ??
        true,
      matchScenario:
        e.matchScenario ?? e.match_scenario ?? ext.match_scenario ?? true,
      matchPersonaDescription:
        e.matchPersonaDescription ??
        e.match_persona_description ??
        ext.match_persona_description ??
        false,
      matchCharacterDepthPrompt:
        e.matchCharacterDepthPrompt ??
        e.match_character_depth_prompt ??
        ext.match_character_depth_prompt ??
        false,
      matchCreatorNotes:
        e.matchCreatorNotes ??
        e.match_creator_notes ??
        ext.match_creator_notes ??
        false,
    };
  });
 ...
 */
export async function extractAndSaveWorldbook(
  cardData: any,
  agentName: string
): Promise<string | null> {
  const characterBook =
    cardData.character_book || cardData.data?.character_book;
  if (
    !characterBook ||
    !characterBook.entries ||
    Object.keys(characterBook.entries).length === 0
  ) {
    return null;
  }
