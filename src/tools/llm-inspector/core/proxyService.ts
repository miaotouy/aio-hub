 ...
 * 更新检查器目标地址
 */
export async function updateInspectorTarget(
  targetUrl: string
): Promise<string> {
  try {
 ...
 * 监听检查器响应事件
 */
export async function onResponseEvent(
  callback: (response: ResponseRecord) => void
): Promise<() => void> {
  try {
 ...
 * 监听流式更新事件
 */
export async function onStreamUpdateEvent(
 ...
 * 等待检查器服务启动
 */
export async function waitForInspectorStart(
  timeout: number = 5000
): Promise<boolean> {
 ...
 * 等待检查器服务停止
 */
export async function waitForInspectorStop(
  timeout: number = 5000
): Promise<boolean> {
  const startTime = Date.now();
