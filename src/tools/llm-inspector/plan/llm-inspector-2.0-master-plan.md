...
- [x] F1. Token 客户端估算卡
- [x] F2. 服务端 usage 提取与偏差对比
- [x] F3. 请求来源链路卡（实装方式：RecordsList 来源徽章 + 详情 Inspector 元数据卡）
- [x] F4. 重新解析按钮（含 G3 setting 抽屉 tokenizer 默认值留待 P2，详见 §15）

### Group G — 收尾

- [x] G1. 时间显示升级
- [x] G2. 总览卡显式展示 Stream 状态（含声明 vs 实际辨析）
- [x] G3. 更新 ARCHITECTURE.md
- [x] G4. 归档旧 plan 文档
- [x] 🏁 **里程碑 v2.0 GA**
...
### 2026-06-02 — 完成 Group F + Group G → 🏁 v2.0 GA

**背景**: 在 detail-panel-rework 已完成 3-Tab 重构的基础上，按计划推进 Token 增强卡、来源链路卡、收尾文档与归档。期间应姐姐要求先修了一个历史小瑕疵。

**实际提交**（按时间序）：

0. **修瑕疵** (`2d342927`) `fix(llm-inspector): 移除 ResponsePanel.vue 中的 as any 强转`
   - detail-panel-rework 阶段 [`ResponsePanel.vue:58`](src/tools/llm-inspector/components/detail/ResponsePanel.vue:58) 用 `as any` 绕过类型检查借用 useRecordDetail 的流式状态。
   - 修复：直接传 `useRecordDetail(props)`，props 类型本身已满足签名（`CombinedRecord` 是 `CombinedRecord | null` 的子类型），无需 getter 对象绕路。

1. **F1 前置** (`d41f2631`) `feat(llm-inspector): 新增 useTokenEstimate composable`
   - 新增 [`composables/useTokenEstimate.ts`](src/tools/llm-inspector/composables/useTokenEstimate.ts:1)，封装 reactive Token 估算 + 服务端 usage 提取。
   - 全局 LRU 缓存（按 recordId，签名 = `${reqLen}|${resLen}|${modelHint}`），上限 200 条。
   - watch 监听 `[r.id, r.response?.body?.length]` 双键 → 切换记录秒出 + 流式响应到达自动重算。
   - 预暴露 `promptDeviation` / `completionDeviation` computed 与 `recompute()` 方法，供 F2/F4 使用。

2. **F1** (`650bc79b`) `feat(llm-inspector): RecordOverviewTab 加 Token 估算卡`
   - 在响应摘要之后挂载 Token 卡片，使用 grid 三列：请求 / 响应 / 总计。
   - 标题栏显示 tokenizer profile 名称 + `~` 标记粗略估算 + LoaderCircle 估算中动画。
   - 附件 Token 占位提示（A3 stub，F3 未真正接入图像，但 UI 已就绪自动适配）。

3. **F2** (`05d4aef1`) `feat(llm-inspector): Token 卡加偏差对比与高亮提示`
   - 三档阈值：< 5% ok / 5-15% warn / >= 15% danger。
   - 三档图标（lucide）+ 三档颜色 chip + tooltip 解释偏差原因（高估/低估 + tokenizer 选错提示）。
   - 偏差正负带符号显示。

4. **F3** (`1c9f0441`) `feat(llm-inspector): RecordsList 加来源徽章与用途标签`
   - 列表每行：internal 显示工具名（Zap 图标 + primary 色）/ external 显示「代理」（Globe + info 色）。
   - tooltip 完整链路：工具/用途/模型。
   - 元信息行追加 purpose 标签（warning 色）。
   - 顺手统一类型：删除本地重复 CombinedRecord 定义，改导入正式类型。
   - **偏离说明**：原计划 F3 还要做"详情顶部加来源链路卡"展示外部代理的 target URL 等信息，但 [`RecordOverviewTab.vue`](src/tools/llm-inspector/components/detail/RecordOverviewTab.vue:1) 已有 Inspector 元数据卡片完美承担 internal 信息展示；external 来源的代理配置（target/端口/Header 规则）属于全局状态而非 record-level 数据，强行塞进详情卡会语义混乱，故未实装这部分。

5. **F4** (`51f8cd99`) `feat(llm-inspector): Token 卡加重新解析按钮`
   - 在 Token 卡片标题栏右侧加 RefreshCw 按钮，调用 `useTokenEstimate.recompute()`。
   - 估算中按钮禁用 + 旋转动画。
   - **偏离说明**：原计划 F4 还提到"设置抽屉里加默认 tokenizer 算法选项"，该功能涉及 token-calculator 全局偏好覆盖，跨度过大，需独立立项。当前 commit 仅完成核心重算入口。

6. **G1+G2** (`c31eefae`) `feat(llm-inspector): 时间显示升级 + Stream 状态来源辨析`
   - G1 时间：`yyyy-MM-dd HH:mm:ss.SSS` 毫秒精度 + 相对时间（date-fns + zhCN locale）+ tooltip 完整 ISO 8601。
   - RecordsList 保持 HH:mm:ss 紧凑显示，tooltip 加 ISO。
   - G2 流式：区分三种状态——「是（按配置）」「是（实际）」「声明但未生效」，tooltip 解释每种原因。
   - 数据来源：请求体 stream:true + 响应 content-type + 运行时 isStreamingRecord 三合一判断。

7. **G3** (`415b5aa7`) `docs(llm-inspector): 重写 ARCHITECTURE.md`
   - 全量重写为 2.0 GA 版本，反映双层监控、状态机、UI Layout、detail-panel-rework 后的 3-Tab 结构、Token 估算、消息解析、contextStore + X-Request-ID 关联机制等。
   - 加新数据流图（含内部钩子全链路 sequenceDiagram）。
   - 附 P2 未来扩展点列表。

8. **G4** (本次提交) `chore(llm-inspector): 归档旧 plan 文档 + 更新主计划进度`
   - [`llm-inspector-internal-monitoring.md`](./llm-inspector-internal-monitoring.md:1) 顶部加 Archived 标记。
   - [`llm-inspector-llm-aware-detail-upgrade.md`](./llm-inspector-llm-aware-detail-upgrade.md:1) 顶部加 Archived 标记。
   - 更新主计划进度表 §14 与变更日志 §15。

**v2.0 GA 里程碑达成度评估**:

- ✅ Token 增强完整：客户端估算 + 服务端 usage + 偏差对比 + 重算入口全部就位。
- ✅ 来源标识完整：列表徽章 + 详情元数据卡，internal/external 全程可视区分。
- ✅ 时间与流式状态：毫秒精度 + ISO tooltip + 流式三态辨析。
- ✅ 文档同步：ARCHITECTURE.md 反映最新架构 + 旧 plan 归档。
- ⚠️ 全程仅做静态类型检查（`bun run check:frontend` 8 次全绿），未在 Tauri dev 环境实地验证。建议姐姐合入主分支前手工跑一次：
  1. 触发内部 LLM 请求，确认列表显示 internal 徽章 + 工具名。
  2. 打开详情 Tab，确认 Token 卡片估算秒出，服务端 usage 正常加载，偏差 chip 颜色正确。
  3. 流式响应进入响应 Tab 原始模式，确认状态条实时跳动 + 100ms 节流不卡。

**影响**:

- LLM Inspector 2.0 主线全部完工，可作为正式版本发布。
- 后续 P2 立项可参考 §9 列出的 H1-H3 + 多模态附件 Token + 设置抽屉 tokenizer 默认值。

<!-- 后续条目从这里追加 -->

---

> 🎉 **v2.0 GA 已达成**。后续优化按 P2 立项处理，本主计划不再追加新任务。
