...
- 渠道去重的模型多选列表（多选 tag 式，统一勾选语义为“Agent 可见”）
- 各个可见模型的卡片列表（包含模型基本信息、模态标签、快速同步开关、参数说明编辑区）
- 每个模型卡片内部的渠道列表（支持拖拽排序，更新全局 profilePriority 路由优先级）
...
│  可见性模式：[● 自动发现 (黑名单)] [○ 仅白名单]          │
├────────────────────────────────────────────────────────┤
│  Agent 可见模型：[ 模型A ✖ ] [ 模型B ✖ ] [ 选择模型... ] │
└────────────────────────────────────────────────────────┘
```

- **可见性模式**：使用 `el-segmented` 切换。
- **Agent 可见模型**：使用 `el-select` 多选 tag 式。
  - 统一勾选语义：选中的模型即为 Agent 可见模型。
  - 内部存储逻辑：
    - 白名单模式：`whitelistModelIds` 存放已勾选的 modelId。
    - 黑名单模式：`blacklistModelIds` 存放未勾选的 modelId。

#### B. 模型配置与渠道优先级卡片列表

对于每一个可见的模型，渲染为一个独立的卡片（`.model-card`）：
...
- **卡片头部**：
  - 模型名称、模型 ID。
  - 模态标签（图片/视频/音频）。
  - ⚡ 快速同步开关（控制 `fastModelIds`）。
  - 📝 参数说明按钮（展开/收起参数说明编辑区）。
- **参数说明编辑区**：
  - 点击 📝 按钮展开，提供 `el-input` textarea 编辑 `modelParamNotes[model.id]`。
- **渠道优先级列表**：
  - 展示该模型在所有已启用渠道中的实例。
  - 使用 `VueDraggable` 支持拖拽排序。
  - 拖拽后，自动计算并更新全局 `profilePriority` 路由优先级。
  - 每一行显示渠道名称、渠道类型，以及优先级标签（第一项显示“首选”，后面的显示“备选”）。
