...
- 已启用滤镜时，处理后的 PNG Blob 只编码一次，并派生为预览 Object URL、历史缩略图 Object URL 和 OCR data URL；Canvas 像素滤镜直接复用 `ImageData` 缓冲区，避免重复分配与拷贝。
- `idle` 状态允许按采样频率刷新预览；用户点击“停止监控”后会停止截图、滤镜和 OCR 队列，只保留最后一帧画面，直到下一次启动。预览 URL 与历史字幕 URL 独立管理；刷新预览不会使时间轴缩略图失效。修改滤镜参数会清空 `lastHash`，下一帧会用新参数重新识别。
