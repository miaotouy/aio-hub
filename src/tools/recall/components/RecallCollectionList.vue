...
const logger = createModuleLogger("recall/RecallCollectionList");
const {
  switchBase,
  createBase,
  updateBaseMeta,
...
const contentMatchedRecallIds = ref<Set<string>>(new Set());
...
const localMatchedBases = computed(() => {
  ...
  return recallStore.sortedBases.filter(
    (b) =>
...
const filteredBases = computed(() => {
  if (!searchQuery.value.trim()) return recallStore.sortedBases;

  // 合并本地匹配和内容匹配
  const matchedIds = new Set(localMatchedBases.value.map((b) => b.id));
  contentMatchedRecallIds.value.forEach((id) => matchedIds.add(id));

  return recallStore.sortedBases.filter((b) => matchedIds.has(b.id));
});
...
watch(searchQuery, (val) => {
  ...
  if (!trimmed) {
    contentMatchedRecallIds.value.clear();
  ...
  searchTimer = setTimeout(async () => {
    try {
      ...
      results.forEach((r) => {
        if (r.recallId) matchedIds.add(r.recallId);
      });
      contentMatchedRecallIds.value = matchedIds;
...
const handleBatchDelete = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      `确定删除选中的 ${selectedIds.value.size} 个思绪集吗？此操作不可恢复。`,
      "批量删除确认",
      {
        type: "warning",
...
const handleRename = async (id?: string) => {
  const targetId = id || recallStore.activeBaseId;
  if (!targetId) return;

  const base = recallStore.bases.find((b) => b.id === targetId);
...
<template>
  <div class="recall-list-container">
    <!-- 头部区域 -->
    <div class="sidebar-top">
      <div class="header-row" v-if="!isBatchMode">
            ...
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item
                  ...
                  :key="item.value"
                  @click="recallStore.baseSort.field = item.value as any"
                >
                  <div class="sort-menu-item">
                    <span>{{ item.label }}</span>
                    <Check
                      v-if="recallStore.baseSort.field === item.value"
                      :size="14"
                    />
                  </div>
                </el-dropdown-item>
                <el-dropdown-item
                  divided
                  @click="recallStore.baseSort.order = 'asc'"
                >
                  <div class="sort-menu-item">
                    <span>升序</span>
                ...
                <el-dropdown-item @click="recallStore.baseSort.order = 'desc'">
                  <div class="sort-menu-item">
                    <span>降序</span>
                    <Check
                      v-if="recallStore.baseSort.order === 'desc'"
                      :size="14"
                    />
                  </div>
                </el-dropdown-item>
        ...
        <div class="toolbar-group">
          <el-tooltip content="导入思绪集备份" placement="top">
          ...
          <el-tooltip content="创建思绪集" placement="top">
            <el-button
              type="primary"
              circle
              size="small"
    ...
    <div class="list-content custom-scrollbar">
        ...
        <div class="info">
          <div class="name-row">
            ...
            <el-checkbox
              ...
              pointer-events-none
            />
            <Database :size="16" class="recall-icon" v-else />
      ...
      <div v-if="filteredBases.length === 0" class="empty-state">
        <template v-if="searchQuery">
          ...
          <div class="empty-text" v-else>未找到相关思绪集</div>
        </template>
        <template v-else>
...
.recall-item.active .name,
.recall-item.active .recall-icon {
  color: var(--el-color-primary);
  font-weight: 600;
}

.recall-item.is-selecting {
  border-color: transparent;
}

.recall-item.is-selecting:hover {
  background-color: color-mix(
    in srgb,
    var(--el-color-primary),
    transparent 95%
...
.recall-item.is-selecting.active {
  background-color: color-mix(
    in srgb,
    var(--el-color-primary),
    transparent 90%
...
.recall-checkbox {
  ...
  display: flex;
  align-items: center;
  pointer-events: none; /* 让点击事件穿透到父级 recall-item */
}
