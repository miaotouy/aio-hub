...
const { updateBaseMeta } = useRecallCollection();
const { indexAllPendingEntries } = useRecallIndexer();
...
// 初始化表单
const initForm = () => {
  if (recallStore.activeBaseMeta) {
    formData.value = {
      name: recallStore.activeBaseMeta.name || "",
      description: recallStore.activeBaseMeta.description || "",
      tags: [...(recallStore.activeBaseMeta.tags || [])],
      config: {
        searchTopK: recallStore.activeBaseMeta.config?.searchTopK ?? 5,
        minScore: recallStore.activeBaseMeta.config?.minScore ?? 0.5,
      },
    };
  }
};
...
onMounted(initForm);
watch(() => recallStore.activeBaseId, initForm);

const handleSave = async () => {
  if (!recallStore.activeBaseId) return;
  await updateBaseMeta(recallStore.activeBaseId, {
...
const embeddingModelInfo = computed(() => {
  const modelId = recallStore.config.defaultEmbeddingModel;
        ...
        <div class="tab-content scrollbar-hidden">
          <div class="stats-grid">
            <div class="stat-card">
              <div class="stat-value">
                {{ recallStore.activeBaseStats?.total || 0 }}
              </div>
              <div class="stat-label">总条目数</div>
            </div>
            <div class="stat-card">
              <div class="stat-value">
                {{ recallStore.activeBaseStats?.indexed || 0 }}
              </div>
              <div class="stat-label">已向量化</div>
            </div>
            <div class="stat-card">
              <div class="stat-value">
                {{ recallStore.activeBaseStats?.indexedRate.toFixed(1) }}%
              </div>
              <div class="stat-label">覆盖率</div>
            </div>
            <div class="stat-card">
              <div class="stat-value">
                {{ formatSize(recallStore.activeBaseStats?.totalChars || 0) }}
              </div>
          ...
          >
            <div class="progress-header">
              <span class="progress-text">
                正在补齐向量... ({{ recallStore.indexingProgress.current }} /
          ...
          <div class="time-info">
            <div class="time-item">
              ...
              <span class="value">{{
                recallStore.activeBaseStats?.createdAt
                  ? format(
                      recallStore.activeBaseStats.createdAt,
                      "yyyy-MM-dd HH:mm:ss"
                    )
            ...
            <div class="time-item">
              ...
              <span class="value">{{
                recallStore.activeBaseStats?.updatedAt
                  ? format(
                      recallStore.activeBaseStats.updatedAt,
                      "yyyy-MM-dd HH:mm:ss"
                    )
          ...
          <InfoCard title="向量化引擎" class="info-section">
            <div class="engine-info">
              ...
              <div class="info-row">
                ...
                <span>{{ recallStore.config.vectorIndex.dimension }}</span>
              </div>
              <div class="info-actions">
                ...
                <el-button
                  ...
                  plain
                  type="danger"
                  @click="recallStore.clearLegacyVectors"
                >
                  清理冗余向量
                </el-button>
              </div>
