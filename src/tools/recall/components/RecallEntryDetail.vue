...
import { getProfileId } from "../utils/recallUtils";
import {
  Trash2,
  Zap,
  Tag,
...
import type { RecallEntryInput } from "../types";
...
const { updateEntry, deleteEntry } = useRecallCollection();
const { indexEntry } = useRecallIndexer();
...
let lastSavedData: RecallEntryInput | null = null;

const isVectorReady = computed(() => {
  if (!recallStore.activeEntryId) return false;
  return recallStore.vectorizedIds.has(recallStore.activeEntryId);
});
...
const form = ref<RecallEntryInput>({
  key: "",
  content: "",
  tags: [],
  assets: [],
...
const debouncedSave = useDebounceFn(async () => {
  ...
  try {
    ...
    await updateEntry(recallStore.activeEntryId, currentData, true);
...
watch(
  ...
  async (newId) => {
    if (newId) {
      ...
      const entry = await recallStore.getOrLoadEntry(newId);

      // 确保还是同一个 ID，防止竞态
      if (entry && recallStore.activeEntryId === newId) {
        const newData = {
          key: entry.key,
...
const handleDelete = async () => {
  if (!recallStore.activeEntryId) return;
  try {
    const idToDelete = recallStore.activeEntryId;
...
const handleEmbedding = async () => {
  ...
  try {
    ...
    await indexEntry(recallStore.activeEntryId, true);
  } finally {
...
const handleGenerateTags = async () => {
  ...
  const config = recallStore.config.tagGeneration;
...
const { isDraggingOver } = useFileInteraction({
  ...
  multiple: true,
  disabled: computed(() => !recallStore.activeEntryId),
            ...
            <el-tooltip
              :content="
                !recallStore.activeEntry
                  ...
                  : isVectorReady
                    ? '向量已就绪'
                    : recallStore.pendingIds.has(recallStore.activeEntryId!)
                      ? '向量处理中'
                      : '点击同步向量'
              "
            >
            ...
            <el-popconfirm title="确定要删除此条目吗？" @confirm="handleDelete">
              <template #reference>
                <el-button
                  ...
                  size="small"
                  class="delete-btn"
                  :disabled="!recallStore.activeEntry"
                >
                  <Trash2 :size="14" />
                </el-button>
              </template>
...
.entry-detail-container.is-loading {
  opacity: 0.7;
  pointer-events: none;
}
