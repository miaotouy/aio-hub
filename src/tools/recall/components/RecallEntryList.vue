...
const logger = createModuleLogger("recall/RecallEntryList");
...
const getEntryVectorStatus = (entry: any) => {
  ...
  if (recallStore.pendingIds.has(entry.id)) return "pending";
  if (recallStore.failedIds.has(entry.id)) return "error";
  if (recallStore.vectorizedIds.has(entry.id)) return "ready";
  return "none";
};

// 搜索状态
const remoteRecallResults = ref<any[]>([]);
...
watch(debouncedSearchQuery, async (val) => {
  ...
  try {
    ...
    // 提取条目信息并附加分数
    // 注意：后端返回结构是 RecallResult { entry: RecallEntry, score: f32, ... }
    // 并且 RecallEntry.tags 是 TagWithWeight[]，模板需要的是 string[]
    remoteRecallResults.value = results
      .map((r) => {
        const entry = r.entry;
...
const processFiles = async (files: File[]) => {
  if (!recallStore.activeBaseId) {
    customMessage.warning("请先选择或创建一个思绪集");
    return;
  }
...
const localFilteredEntries = computed(() => {
  ...
  return recallStore.sortedEntries.filter(
    (e) =>
...
const entryList = computed(() => {
  ...
  if (q) {
    // 逻辑决策
    if (isSearching.value && remoteRecallResults.value.length === 0) {
      return localFilteredEntries.value;
    }

    if (remoteRecallResults.value.length > 0) {
      return remoteRecallResults.value;
    }
  ...
  return recallStore.sortedEntries;
});
...
// 切换库或搜索时重置滚动
watch([() => recallStore.activeBaseId, debouncedSearchQuery], () => {
...
const handleRefresh = async () => {
  if (recallStore.activeBaseId) {
    await switchBase(recallStore.activeBaseId);
    // 深度校验向量状态
    await recallStore.validateVectorStatus();
  }
};
        ...
        <div class="toolbar-left">
          <span class="count-badge">
            <template v-if="debouncedSearchQuery">
              <template v-if="isSearching && remoteRecallResults.length === 0">
            ...
            <template v-else>
              {{ entryList.length }} / {{ recallStore.sortedEntries.length }}
            </template>
          </span>
          <el-icon
            v-if="isSearching"
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item
                    ...
                    :key="item.value"
                    @click="recallStore.entrySort.field = item.value as any"
                  >
                    <div class="sort-menu-item">
                      <span>{{ item.label }}</span>
                      <Check
                        v-if="recallStore.entrySort.field === item.value"
                        :size="14"
                      />
                    </div>
                  </el-dropdown-item>
                  <el-dropdown-item
                    divided
                    @click="recallStore.entrySort.order = 'asc'"
                  >
                    <div class="sort-menu-item">
                      <span>升序</span>
                      <Check
                        v-if="recallStore.entrySort.order === 'asc'"
                        :size="14"
                      />
                    </div>
                  </el-dropdown-item>
                  <el-dropdown-item @click="recallStore.entrySort.order = 'desc'">
                    <div class="sort-menu-item">
                      <span>降序</span>
                      <Check
                        v-if="recallStore.entrySort.order === 'desc'"
                        :size="14"
                      />
                    </div>
                  </el-dropdown-item>
        ...
        >
          <div
            ...
            :class="{
              active:
                !isSelectionMode &&
                recallStore.activeEntryId === entryList[virtualItem.index].id,
...
.entry-list-container.is-dragging {
