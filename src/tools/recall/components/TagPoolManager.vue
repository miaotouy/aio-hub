...
const { batchVectorizeTags } = useRecallCollection();
...
async function loadStats() {
  ...
  try {
    ...
    const vectorizedTags = await invoke<string[]>("recall_list_all_tags", {
      modelId: pureId,
    });
...
async function handleBatchVectorize() {
  ...
  try {
    const allRecallIds = store.bases.map((b) => b.id);
    await batchVectorizeTags(allRecallIds);
...
async function handleClearPool() {
  ...
  try {
    ...
    clearing.value = true;
    await invoke("recall_clear_tag_pool", { modelId: props.modelId });
...
async function handleClearOtherModels() {
  ...
  try {
    ...
    // 调用后端命令一次性清除
    const clearedCount = await invoke<number>("recall_clear_other_tag_pools", {
