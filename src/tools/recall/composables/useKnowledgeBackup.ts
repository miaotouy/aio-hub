...
import type { RecallCollectionMeta, RecallCollectionIndex } from "../types";
import type {
...
export function useKnowledgeBackup() {
  ...
  function requestCancel() {
    ...
    if (operation.value === "export") {
      void invoke("recall_cancel_backup_operation");
    }
  }
  ...
  async function exportSingle(recallId: string) {
    const directory = await selectTargetDirectory();
    if (!directory) return;
    const base = store.bases.find((item) => item.id === recallId);
    begin("export", 1);
    currentName.value = base?.name || recallId;
    try {
      const result = await invoke<BackupExportResult>("recall_export_backup", {
        recallId,
      ...
      customMessage.success("思绪集备份导出完成");
    } catch (error) {
      failed.value = 1;
      current.value = 1;
      items.value = [
        {
          key: recallId,
          name: base?.name || recallId,
          status: "failed",
          detail: String(error),
          warnings: [],
        },
      ];
      errorHandler.error(error, "导出思绪集备份失败");
    } finally {
      running.value = false;
    }
  }

  async function exportMany(recallIds: string[], persistedAll = false) {
    if (recallIds.length === 0) {
      customMessage.warning("没有可导出的思绪集");
      return;
    }
    const directory = await selectTargetDirectory();
    if (!directory) return;
    begin("export", recallIds.length);
    ...
    try {
      const result = await invoke<BackupBatchExportResult>(
        "recall_export_backups",
        { recallIds: persistedAll ? [] : recallIds, targetDirectory: directory }
      );
      ...
      const message = result.cancelled
        ...
        : result.failed.length
          ? `已导出 ${result.succeeded.length} 个思绪集，${result.failed.length} 个失败`
          : `已导出 ${result.succeeded.length} 个思绪集`;
      ...
      errorHandler.error(error, "批量导出思绪集备份失败");
    } finally {
      unlisten();
      running.value = false;
    }
  ...
  async function chooseConflictStrategy(
    ...
    try {
      await ElMessageBox.confirm(
        `即将替换思绪集「${targetName}」（${inspect.libraryId}），${targetReplacementScope}将由备份「${inspect.libraryName}」的 ${inspect.entryCount} 个条目取代。`,
        "确认替换思绪集",
        {
          type: "error",
  ...
  async function syncWorkspaceFromBackend() {
    ...
    const workspace = store.workspace || (await recallStorage.loadWorkspace());
    const previousById = new Map(
      workspace.bases.map((base) => [base.id, base])
    );
    workspace.bases = metas.map((meta): RecallCollectionIndex => {
    ...
    await recallStorage.saveWorkspace(workspace);
    await store.loadBases();
    if (store.activeBaseId) {
      const active = await recallStorage.loadBaseMeta(store.activeBaseId);
      if (active) {
  ...
  async function importBackups() {
    ...
    try {
      ...
      errorHandler.error(error, "选择思绪集备份失败");
      return;
    }
    ...
    try {
      ...
      errorHandler.error(error, "思绪集已导入，但刷新工作区失败");
    } finally {
      running.value = false;
    }
  }
