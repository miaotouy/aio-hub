...
改名、拆分和检索重构未形成中间发布版本，日常开发和可运行切片的测试结论不能直接升级为发布结论。这些检查只在正式版（不带预发布标识）前关闭，不阻断 `v0.7.0-alpha` 等预发布周期；完整清单见施工计划的「正式版前 Recall 检查清单」。

- 迁移基线现由版本化 fixture（`recall-migration-baseline-v1.json` + `migration_baseline.rs`）与确认式旧目录迁移契约保证；它只证明契约与确定性，不证明真实历史数据。
- debug Tauri E2E、工程 fixture、当前工作区生成的 SQLite 和已经迁移过的数据根只能证明局部契约，不能替代正式版前的首次迁移、幂等重启、失败恢复、只读旧源或发布二进制验证。
- 为正式版本 appData 迁移新增的具名 preset 必须在缺少必要输入时 `fail`，不能沿用外部 corpus 的可选 `skip` 语义；正式版记录须保留输入版本、脱敏 fingerprint、状态转移和报告摘要。
...
- 外部语料和真实模型仍可保护隐私地显式选择；若被列为正式版的必要输入，缺失时需记录原因。
