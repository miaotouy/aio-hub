...
- 主库和向量库分别记录迁移状态。用户明确确认后才执行导入；主库未完整成功时迁移事项保持待处理，应用其他模块仍可启动；向量迁移失败可以保留已迁移源内容并降级为待重建。
...
- Guided Flow 在执行前要求用户确认备份和写入风险。手工恢复前必须退出应用并备份 `recall.db` 与 `recall-vectors.db`；不得手工合并 SQLite 文件。
