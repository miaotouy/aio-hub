...
export interface OrchestratorConfig {
  requestSettings?: RecallRequestSettings;
}
...
export class IndexingOrchestrator {
  ...
  async syncTags(params: {
    ...
    // 1. 获取缺失标签
    const missingTags = await invoke<string[]>("recall_get_missing_tags", {
      modelId,
      tags,
    });
    ...
    if (tagVectorMap.size > 0) {
      await invoke("recall_sync_tag_vectors", {
        modelId,
  ...
  async indexEntry(params: {
    ...
    const { recallId, entry, modelId, profile } = params;
    ...
    await invoke("recall_update_entry_vector", {
      recallId,
      entryId: entry.id,
      vector,
      model: modelId,
...
export class VectorSyncManager {
   ...
   * 检查指定思绪集在特定模型下的向量覆盖率
   */
  async checkCoverage(params: { recallIds: string[]; modelId: string }) {
    const { recallIds, modelId } = params;
    const report = await invoke<any>("recall_check_vector_coverage", {
      recallIds,
      modelId,
    });
    ...
    return {
      ...
      missingMap: report.missingMap as [string, string][], // [recallId, entryId][]
    };
  }
  ...
  async syncMissingVectors(params: {
    ...
    for (let i = 0; i < total; i++) {
      ...
      const [recallId, entryId] = missingMap[i];
      try {
        // 加载条目内容
        const entry = await invoke<RecallEntry | null>("recall_load_entry", {
          recallId,
        ...
        logger.error(`条目[${entryId}]自动同步失败`, error);
      } finally {
        current++;
