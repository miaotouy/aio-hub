...
/**
 * 思绪集 (基础) 实例 - 日常 Agent 常驻
 */
const recallBasic: ToolRegistry = {
  id: "recall-basic",
  name: "思绪",
  description: "思绪集的日常读写操作：搜索、创建和更新条目",
    ...
    return {
      methods: [
        ...
        },
        {
          ...
          displayName: "创建或更新条目",
          description:
            "创建新条目或更新已存在条目的完整内容。推荐使用 recallName 定位思绪集。",
          agentCallable: true,
          parameters: [
            {
              name: "recallName",
              type: "string",
              description: "思绪集名称 (推荐)",
              required: false,
            },
            {
              name: "recallId",
        ...
        },
        {
          ...
          parameters: [
            ...
            },
            {
              name: "targetContent",
              type: "string",
              description: "要替换的原文片段 (至少 15 字符)",
              required: false,
            },
            {
              name: "replaceWith",
              type: "string",
              description: "替换为的新内容片段",
              required: false,
            },
            {
              name: "replaceContent",
              type: "string",
              description: "替换整个条目的内容",
              required: false,
            },
            {
              name: "replaceKey",
 ...
 * 思绪集 (管理) 实例 - 管理员专用
 */
const recallAdmin: ToolRegistry = {
  id: "recall-admin",
  name: "思绪集 (管理)",
  description: "思绪集的高级管理操作：索引浏览、元数据批量管理、条目删除",

  getMetadata(): ServiceMetadata {
    return {
      methods: [
        {
          name: "listRecallCollections",
          displayName: "列出思绪集列表",
          description:
            "列出所有思绪集及其基本信息和统计数据。支持名称搜索和统计信息开关。",
          agentCallable: true,
          parameters: [
            {
              name: "query",
              type: "string",
              description: "思绪集名称关键词搜索（模糊匹配）",
              required: false,
            },
            {
              name: "includeStats",
          ...
          returnType: "Promise<ListRecallCollectionsResult>",
        },
        {
          name: "listEntriesMetadata",
          displayName: "查询条目元数据",
          description:
            "列出并筛选思绪集条目的元数据（不含完整内容）。支持标题搜索、标签过滤、分页和排序。适用于批量管理场景，配合 batchUpdateMetadata 使用。",
          agentCallable: true,
          parameters: [
            {
              name: "recallName",
            ...
            },
            {
              name: "sortBy",
              type: "string",
              description: "排序字段 (updatedAt/key/priority)",
              required: false,
            },
            {
              name: "sortOrder",
  ...
  async listRecallCollections(
    args: Record<string, unknown>
  ): Promise<Record<string, unknown>> {
    const normalizedArgs = normalizeAgentBooleanFields(args, ["includeStats"]);
    return (await agentActions.listRecallCollections(
      normalizedArgs as any
    )) as any;
  },
...
// 导出实例数组（多实例注册）
export default [recallBasic, recallAdmin];
