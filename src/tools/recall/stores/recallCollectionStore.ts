...
export const useRecallCollectionStore = defineStore("recallCollection", {
  state: () => ({
    /** 全局工作区数据 */
    workspace: null as WorkspaceData | null,
    /** 思绪集列表索引 (从 workspace 同步) */
    bases: [] as RecallCollectionIndex[],
    /** 当前激活的思绪集 ID */
    activeBaseId: null as string | null,
    /** 当前激活的思绪集元数据 */
    activeBaseMeta: null as RecallCollectionMeta | null,
    /** 条目 ID 列表 (用于列表展示) */
    loadedEntryIds: [] as string[],
    /** 条目缓存 (Map 存储 ID -> RecallEntry) */
    entriesCache: new Map<string, RecallEntry>(),
    /** 当前选中的条目 ID */
    ...
    /** 监控系统状态 */
    monitor: {
      logs: shallowRef<RecallMonitorMessage[]>([]),
      buffer: [] as RecallMonitorMessage[], // 临时缓冲区
      isPaused: false,
      filter: {
        keyword: "",
        level: null as string | null,
        module: null as string | null,
        type: null as RecallMessageType | null,
      },
      stats: {
        logsPerMinute: 0,
        errorRate: 0,
  ...
  actions: {
     ...
     * 获取或加载条目内容
     */
    async getOrLoadEntry(entryId: string): Promise<RecallEntry | null> {
    ...
    async loadAllBasesVectors(modelId: string) {
      ...
      await Promise.allSettled(
        this.bases.map((base) =>
          invoke("recall_load_model_vectors", { recallId: base.id, modelId })
        )
      );
    },
    ...
    async updateGlobalStats(force = false, targetModelId?: string) {
      ...
      try {
        ...
        const [libStats, poolStats] = await Promise.allSettled([
          invoke<any>("recall_get_library_stats", { modelId }),
          invoke<any>("recall_get_tag_pool_stats", { modelId }),
        ]);
    ...
    async clearLegacyVectors() {
      ...
      this.loading = true;
      try {
        const count = await invoke<number>("recall_clear_legacy_vectors", {
          recallId: this.activeBaseId,
    ...
    async clearAllOtherVectors() {
      ...
      this.loading = true;
      try {
        const count = await invoke<number>("recall_clear_all_other_vectors", {
          keepModelId: modelId,
        });
