...
export interface UpsertEntryOptions {
  /** 思绪集 ID (与 recallName 二选一) */
  recallId?: string;
  /** 思绪集名称 (优先使用，与 recallId 二选一) */
  recallName?: string;
...
/**
 * 7. listRecallCollections（列出思绪集列表）参数
 */
export interface ListRecallCollectionsOptions {
...
/**
 * 7. listRecallCollections（列出思绪集列表）返回值
 */
export interface ListRecallCollectionsResult {
  success: boolean;
  /** 思绪集总数 */
  total?: number;
  /** 思绪集列表 */
