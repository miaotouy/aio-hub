 ...
 * 条目索引项 (轻量级，用于列表展示)
 */
export interface RecallEntryIndexItem {
  id: string;
  key: string;
  /** 内容摘要 (前 100 字符) */
  summary: string;
 ...
 * 思绪集元数据
 */
export interface RecallCollectionMeta {
  /** 库 ID */
  id: string;
  /** 思绪集名称 */
  name: string;
  ...
  models?: string[];
  /** 条目索引列表 (用于快速显示列表) */
  entries: RecallEntryIndexItem[];
  /** 标签 */
  tags?: TagWithWeight[];
  /** 图标 Asset ID */
  icon?: string | null;
  /** 库级别配置 */
  config?: RecallCollectionConfig;
}

/**
 * 思绪集级别配置 (可覆盖全局配置)
 */
export interface RecallCollectionConfig {
 ...
 * 思绪集索引（用于列表展示的轻量级元数据）
 */
export interface RecallCollectionIndex {
  id: string;
  name: string;
  description: string | null;
  entryCount: number;
 ...
 * 请求配置接口 (限流、重试等)
 */
export interface RecallRequestSettings {
  /** 请求超时时间 (毫秒) */
  timeout: number;
  /** 最大重试次数 */
  maxRetries: number;
...
export interface WorkspaceConfig {
  ...
  /** 向量化请求设置 */
  embeddingRequestSettings: RecallRequestSettings;
  /** 缓存配置 */
  cache: RecallCacheConfig;
  /** 实验室 (Playground) 配置 */
