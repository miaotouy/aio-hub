...
const logger = createModuleLogger("recall/query-preprocessor");
...
export interface PreprocessResult {
  ...
  /** 分词后过滤停用词的 token 列表 */
  tokens: string[];
  /** 从查询中自动提取的标签（用于增强 RecallSearchFilters） */
  matchedTags: string[];
  /** 原始查询（用于日志和调试） */
...
export interface PreprocessOptions {
  /** 可用的标签池（通常来自 recallCollectionStore.globalStats.allDiscoveredTags） */
  tagPool?: string[];
  /** 是否启用 Tag 匹配（默认 true） */
...
/** KB 占位符 */
const RE_KB_PLACEHOLDER = /【(?:recall|knowledge)(?:::[^【】]*?)?】/g;
