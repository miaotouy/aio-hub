...
const logger = createModuleLogger("recall/vector-cache");
...
async function resolveEmbeddingCacheMaxItems(): Promise<number> {
  try {
    ...
    const { useRecallCollectionStore } =
      await import("../stores/recallCollectionStore");
...
export class VectorCacheManager {
  ...
  async getVector(
    ...
    try {
      ...
      await invoke("recall_set_embedding_cache", {
        modelId,
        text: query,
        vector,
        maxItems,
