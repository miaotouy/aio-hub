...
import { useRecallMonitor } from "../composables/useRecallMonitor";
import RecallLogCard from "../components/monitor/RecallLogCard.vue";
...
const monitor = useRecallMonitor();
const errorHandler = createModuleErrorHandler("recall-monitor-view");
...
function exportLogs(format: "json" | "md" = "json") {
  try {
    ...
    if (format === "json") {
      ...
      fileName = `recall-monitor-export-${timestamp}.json`;
    } else {
      let md = `# 思绪集监控日志导出\n\n`;
      ...
      fileName = `recall-monitor-export-${timestamp}.md`;
    }
    ...
    <div class="monitor-panel">
      ...
      <div class="log-list" v-bind="containerProps">
        <div v-bind="wrapperProps">
          ...
          >
            <RecallLogCard :message="item.data">
              <template #content>
                <RagTraceContent
...
<style scoped>
.monitor-view {
  /* 彻底移除外层间距，融入上游 RecallCollection.vue 的容器 */
  height: 100%;
  width: 100%;
  display: flex;
