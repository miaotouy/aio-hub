...
- **流式响应式驱动**: 深度集成 Vue 响应式系统，将文本流实时转换为 AST 指令集，实现打字机般的流畅体验。
- **低闪烁 Patch 更新**: 通过 Diff 算法和 Patch 系统复用稳定节点，尽量减少整段卸载重建带来的闪烁。
- **复杂结构支持**: 深度支持 HTML 与 Markdown 混合排版、代码块、数学公式、Mermaid 图表以及 LLM 特有的思考过程标签。
- **动态样式与隔离**: 支持用户通过图形化界面实时自定义样式，并引入了 **CSS 作用域隔离机制**，确保内联 `<style>` 标签不会污染全局 UI。
...
1.  **预处理层 (Preprocessor Layer)**
    ...
    - **核心功能**:
      ...
      - **裸 HTML 页面防御**: `wrapBareHtmlPage` 函数自动检测模型不规范输出的裸 HTML 页面（以 `<!DOCTYPE html>` 开头）并以围栏代码块包裹，防止 UI 爆破。
      - **资产路径解析**: AST 渲染模式下不再对全文做全局 `resolveAsset` 替换，避免本地 URL 被 Markdown 解析器二次编码；图片、音视频、结构化 HTML 和 HTML 预览等具体节点会在渲染阶段按需解析资源。纯 `markdown-it` 模式仍会在预处理阶段对全文调用 `resolveAsset`。

2.  **处理层 (Processor Layer)**
    - **职责**: 接收预处理后的文本流，将其解析为 AST，计算变更并生成 Patch 指令。
    - **核心组件**:
      - `StreamController`: 流式平滑化控制器，通过内部缓冲区平滑 Token 发射节奏，避免网络包抖动导致的视觉卡顿。位于 `core/StreamController.ts`。
...
3.  **状态层 (State Layer)**
    ...
    - **核心组件**:
      - `useMarkdownAst`: Vue Composable，提供 AST 的响应式状态（基于 `shallowRef`）和 Patch 应用逻辑（如 `text-append`, `replace-node`），支持可配置的 `throttleMs` 节流参数及 `dispose` 内存回收方法。位于 `composables/useMarkdownAst.ts`。
      - `Pinia Store` (`stores/store.ts`): 集中管理测试工具的配置、LLM 思考标签规则、正则配置，以及最重要的 **动态 Markdown 样式 (`richTextStyleOptions`)**。采用单一 `reactive` 状态对象 + `createDefaultState()` 函数模式，通过 `configManager` 实现配置的持久化和自动保存。

4.  **视图层 (View Layer)**
    - **职责**: 将 AST 渲染为 Vue 组件树，并应用动态样式。
    - **核心组件**:
      - `RichTextRenderer`: 统一入口组件，接收文本流和配置，支持 4 种渲染器版本切换和双引擎代码编辑器（Monaco / CodeMirror）选择。
      - `AstNodeRenderer`: 基于 JSX 的递归渲染组件，根据节点类型分发到具体的 Vue 组件，并注入动态样式。当前会给块级节点标记 `rich-text-block` class，但 renderer 内部的块级 `content-visibility` CSS 仍处于关闭状态，避免和消息列表层滚动估高互相影响。
...
- **稳定区 (Stable Region)**:
  ...
  - 策略：V2 会在稳定区文本未变化时复用已有 `stableAst`；当稳定区文本变化时重新解析稳定区，再与当前 AST 做 diff。
  - 优势：在流式末尾持续增长、稳定区不变的常见场景下，避免重复解析历史稳定内容。

- **待定区 (Pending Region)**:
  - 位于文本流末尾，可能包含未闭合的标签或标记。
  - 策略：每次处理最新 buffer 时重新解析整个待定区，并与稳定区 AST 合并后统一 diff。
...
- **样式隔离与作用域 (Style Isolation)**:
  - **CSS 作用域化**: 引入 `utils/cssUtils.ts`。通过正则解析 `<style>` 标签内的 CSS，自动为选择器添加基于容器 ID（如 `#html-node-xxx`）的前缀，实现类似 Shadow DOM 的样式隔离效果。
  - **Style 节点特化**: 当 `generic_html` 节点的标签名为 `style` 时，系统会自动分发至 `StyleNode` 组件。该组件负责提取 CSS 文本并应用作用域处理，防止样式溢出到渲染区域之外。
  - **动画自动注入**: 具备"魔法注入"能力。如果 CSS 中定义了 `fadeIn` 动画但未显式绑定，渲染器会自动将其绑定到作用域根节点，提升 LLM 生成内容的展示效果。

- **裸 HTML 页面防御 (Bare HTML Defense)**:
  - 模型有时会输出不规范的裸 HTML 页面（如以 `<!DOCTYPE html>` 开头），导致渲染器解析异常。引擎内置 `wrapBareHtmlPage` 函数，自动检测并以围栏代码块包裹裸 HTML 页面，防止 UI 爆破。
  - 支持流式输出场景：在 `</html>` 闭合标签出现前即提前插入开启围栏，避免界面闪烁。
...
- **VCP 协议深度支持**: 支持 `<<<[TOOL_REQUEST]>>>` 工具调用格式、VCP 角色分割 (`vcp_role`) 和日记容器 (`vcp_daily_note`) 解析。支持嵌套工具调用与参数值转义。

### 3.3 样式系统与隔离机制 (Styling & Isolation)

渲染引擎具备双层的样式处理能力：一层用于用户个性化排版定制，另一层用于生成内容的安全性隔离。

#### 3.3.1 动态 Markdown 样式 (User-defined Styling)

允许用户对标准 Markdown 元素的视觉表现进行深度定制。

- **配置中心**: `stores/store.ts` 中的 `richTextStyleOptions` 存储了所有基础元素（如 `paragraph`, `h1`, `strong`, `quote`, `alert`）的自定义样式配置。
- **CSS 变量注入**: `RichTextRenderer` 将样式配置映射为 CSS 自定义属性（如 `--md-strong-color`），通过根元素的 `:style` 绑定注入。
- **分层合并策略**: `utils/styleUtils.ts` 支持全局样式与 Agent/User 个性化样式的分层合并。覆盖配置中已启用的分类会优先级最高，未启用的则回退到全局设置。
- **所见即所得**: `MarkdownStyleEditor` 提供图形化界面，修改会通过 Pinia 实时响应，无需重渲染即可观察效果。

#### 3.3.2 内容样式隔离 (Content Style Isolation)

针对 LLM 在 HTML 块中生成的 `<style>` 标签，引擎实现了自动的作用域隔离机制。

- **自动加缀 (Scoping)**: 核心逻辑位于 `utils/cssUtils.ts`。引擎会解析 CSS 文本，并自动为每个选择器添加基于容器 ID（如 `#html-node-xxx`）的作用域前缀。
- **隔离边界**:
  - **HtmlBlockNode**: 样式作用域限制在当前 HTML 容器内部。
  - **StyleNode**: 样式作用域通过"相邻兄弟选择器"（`#anchor ~ *`）限制在样式节点之后的内容。
- **动画注入魔法**: 针对 LLM 经常"忘记绑定动画"的问题，如果检测到 CSS 中定义了 `fadeIn` 动画但未引用，引擎会自动将其绑定到作用域根节点，确保内容优雅展现。
...
- **更新节流 (Throttling)**: `useMarkdownAst` 当前主要使用 `requestAnimationFrame` 循环检查队列，并按 `throttleMs` 控制 flush 间隔。它会将短时间内密集的 Patch 指令合并，并与浏览器渲染周期同步批量应用。节流时间 `throttleMs` 是可配置的（默认 80ms），可以通过 `throttleEnabled` 开关控制。
- **流式平滑化 (Stream Smoothing)**: `core/StreamController.ts` 通过内部缓冲区平滑 Token 的发射节奏，避免因网络包大小不一导致的视觉卡顿。支持自动加速（当积压过多时）和紧急冲刷（超过阈值时立即显示）。通过 `smoothingEnabled` 控制。
- **批量处理**: Patch 队列会合并连续的、针对同一节点的 `text-append` 操作，将多次小更新合并为一次大更新，进一步提升效率。
- **队列溢出保护**: 当队列长度超过 `MAX_QUEUE_SIZE`（当前为 1000）时，立即执行刷新以避免队列无限堆积。
...
- **支持的库**: d3、mermaid、echarts、three.js、chart.js、anime.js、gsap、p5.js。
- **控制**: 通过 `enableCdnLocalizer` prop 控制是否启用，默认开启。

### 3.7 渲染性能优化

- **消息级 `content-visibility: auto`**: 长消息列表的主要跳过渲染逻辑目前位于 `llm-chat/components/message/MessageList.vue`，对整条消息应用 `content-visibility: auto` 并保持最后一条消息可见，以稳定底部滚动。renderer 内部的块级 `content-visibility` 曾经试验过，但当前 CSS 被注释关闭，避免单条消息内部高度估算导致滚动跳动。
...
- **代码块延迟初始化 (Lazy Code Block Init)**: 为 CodeMirror 和 Monaco 代码块组件引入 `IntersectionObserver` 延迟初始化，仅在代码块进入视口时才创建重型编辑器实例。在编辑器初始化前渲染轻量级 **PreCodeNode** 预览组件（`components/nodes/code-block/PreCodeNode.vue`），支持模拟 CodeMirror 和 Monaco 两种主题样式（行号、折叠占位区、字体栈等）。通过 `debugPreFallback` 调试开关可强制保持预览状态。
- **HTML 预览冻结**: `RichTextRenderer` 通过 `shouldFreeze` prop 将冻结策略传给 HTML 预览节点；消息深度阈值由调用方（如 `MessageContent.vue`）根据聊天设置计算。弹窗打开时会冻结内嵌预览，避免同一 HTML 同时运行两份。
- **流式平滑化 (Stream Smoothing)**: `core/StreamController.ts` 通过内部缓冲区平滑 Token 的发射节奏，避免因网络包大小不一导致的视觉卡顿。支持自动加速（当积压过多时）和紧急冲刷（超过 1000 字符阈值时立即显示）。通过 `smoothingEnabled` 控制。
- **内存管理 (Memory Management)**: 引擎实现了全套组件卸载清理机制。`useMarkdownAst` 提供 `dispose` 方法主动释放 AST 和节点索引；`StreamProcessorV2` 在销毁时停止异步解析；各节点组件（ImageNode、HtmlInteractiveViewer、MermaidNode、CodeBlockNode 等）在卸载时取消进行中的异步请求、移除事件监听和释放 blob URL，有效防止长会话切换时的内存泄漏。
- **渲染安全护栏 (Safety Guard)**: 针对极端流式场景（如超大 buffer、边界长期停滞或 AST 节点数异常），引擎内置安全护栏机制。触发后会追加降级提示并停止后续 patch，`useMarkdownAst` 也会清空待处理队列。通过 `safetyGuardEnabled` 控制。
...
- **调试辅助**:
  ...
  - **PreCodeNode 调试**: 通过 `debugPreFallback` 开关可强制代码块保持预览状态，方便样式调试。
...
│   │   ├── AudioNode.vue         # 音频节点（AudioPlayer 集成）
│   │   ├── BlockquoteNode.vue    # 引用块
│   │   ├── CodeBlockNode.vue     # 代码块（Monaco/CodeMirror + HTML 沙箱 + 延迟初始化）
│   │   ├── DetailsNode.vue       # 折叠面板（拦截 <details>，支持 variant/tone/max-height）
│   │   ├── EmNode.vue            # 斜体
│   │   ├── GenericHtmlNode.vue   # 通用 HTML 标签（SVG/安全过滤/视频音频拦截）
│   │   ├── StyleNode.vue         # 样式节点（支持 CSS 作用域隔离）
...
│   │   ├── TextNode.vue          # 纯文本
│   │   ├── VcpDailyNoteNode.vue  # VCP 日记容器
│   │   ├── VcpRoleNode.vue       # VCP 角色分割容器
│   │   ├── VcpToolNode.vue       # VCP 工具调用
│   │   ├── VideoNode.vue         # 视频节点（VideoPlayer 集成）
│   │   └── code-block/           # 代码块子组件
│   │       ├── CodeBlockHeader.vue
│   │       ├── CodeMirrorSourceViewer.vue
│   │       ├── MonacoSourceViewer.vue
│   │       └── PreCodeNode.vue   # 代码块轻量预览
...
├── composables/                  # 组合式函数
│   └── useMarkdownAst.ts         # AST 状态管理与 Patch 逻辑（含 dispose）
├── config/                       # 配置
│   └── presets.ts                # 测试预设注册表
├── core/                         # 核心处理器
│   ├── CustomParser.ts           # V2 自研解析器入口
│   ├── StreamController.ts       # 流式平滑化控制器
│   ├── StreamProcessor.ts        # V1 处理器（基于 markdown-it）
│   └── StreamProcessorV2.ts      # V2 处理器（基于 CustomParser，含 destroy）
├── parser/                       # V2 解析器模块
│   ├── block/                    # 块级元素解析逻辑
│   ├── inline/                   # 内联元素解析逻辑
│   ├── utils/                    # 解析器工具函数
│   ├── Tokenizer.ts              # 词法分析器（支持 VCP 嵌套转义）
...
│   └── ...
├── stores/                       # 状态管理
│   └── store.ts                  # 全局配置与样式状态（Pinia，单一 reactive 状态对象）
├── utils/                        # 工具函数
│   ├── cdnLocalizer.ts           # CDN 资源本地化
│   ├── cssUtils.ts               # CSS 作用域处理与隔离工具
│   └── styleUtils.ts             # 样式合并工具
├── docs/                         # 设计文档
├── richTextRenderer.registry.ts  # 工具注册文件
├── RichTextRenderer.vue          # 统一入口组件（含 wrapBareHtmlPage 裸页防御）
...
- **HTML 原生支持**:
  ...
  - **视频与音频原生支持**:
    - **视频节点 (`VideoNode`)**: 当 HTML `<video>` 标签被解析时，自动拦截并分发至 `VideoNode` 组件，集成项目通用 `VideoPlayer` 提供全功能播放体验（倍速、画中画、截图、快捷键控制）。支持从子节点 `<source>` 元素提取 `src`。
    - **音频节点 (`AudioNode`)**: 当 HTML `<audio>` 标签被解析时，自动拦截并分发至 `AudioNode` 组件，集成项目通用 `AudioPlayer` 提供精美的可视化播放界面（支持 `full`/`compact`/`minimal` 布局模式）。
    - **隐形音频兜底**: 对于"隐形"音频元素（无 `controls` 属性或 `display: none`），自动生成一套精美的可视化播放控制器。
    - **音量联动**: 音频音量会自动根据全局设置（`globalMediaVolume`）和智能体私有设置（`defaultMediaVolume`）进行加权计算。
  - **危险标签黑名单**: `script`, `iframe`, `object` 等危险标签默认被拦截并降级为 `<span>`，可通过 `allowDangerousHtml` 配置绕过。
  - **裸 HTML 页面防御**: 引擎内置 `wrapBareHtmlPage` 函数，自动检测模型不规范输出的裸 HTML 页面（以 `<!DOCTYPE html>` 开头）并以围栏代码块包裹，防止 UI 爆破。支持流式场景下提前插入围栏。
  - **样式隔离保障**: 即使在非沙箱模式下，通过 `StyleNode` 和 `cssUtils` 也能确保内联样式仅作用于当前 HTML 块及其后续兄弟节点，不会破坏主应用布局。
  - **嵌入式单页应用 (SPA) 渲染**: `html` 代码块作为功能完备的沙箱环境（`HtmlInteractiveViewer`），可以直接渲染和运行包含 JavaScript 的复杂单页应用，并支持在悬浮窗中进行完整交互。
  - **性能冻结 (Freeze) 策略**: HTML 预览节点接收 `shouldFreeze` 控制是否运行 iframe。聊天场景中该值由 `MessageContent.vue` 根据消息深度和用户设置计算；弹窗打开时内嵌预览会冻结，避免同一内容同时运行两份。
- **折叠面板增强 (`<details>`)**:
  - 当解析到 `<details>` 标签时，引擎自动拦截并交给 `DetailsNode` 接管渲染，提供统一的折叠面板视觉语言（旋转箭头、强调色边框、复制按钮等），取代浏览器默认的朴素样式。
  - 由于 `<details>` 不在解析器的 Raw Mode 名单内，**内部可正常嵌套 Markdown 代码块（` ``` `）、表格、再一个 `<details>` 等任意内容**，是包裹「带代码块的 README 片段」的理想容器（避免与外层 ` ``` ` 围栏冲突）。
  - `<summary>` 子节点会被自动识别为标题区，其余子节点作为正文区。标题区支持 Markdown 内联格式（粗体、行内代码、斜体等）的递归渲染。
  - 通过 HTML 属性进行参数化定制：

    | 属性              | 取值                                                  | 默认      | 说明                                                                           |
    | ----------------- | ----------------------------------------------------- | --------- | ------------------------------------------------------------------------------ |
    | `open`            | 原生 boolean                                          | 否        | HTML 标准属性，存在即默认展开                                                  |
    | `data-variant`    | `default` / `card` / `ghost`                          | `default` | 视觉变体                                                                       |
    | `data-tone`       | `neutral` / `info` / `success` / `warning` / `danger` | `neutral` | 强调色调（与 GitHub Alert 体系对齐）                                           |
    | `data-max-height` | 数字（px）或 CSS 长度                                 | 无        | 内容区最大高度，超出启用纵向滚动                                               |
    | `data-summary`    | 字符串                                                | 无        | 当 `<summary>` 标签缺失时的兜底标题                                            |
    | `data-dense`      | boolean                                               | false     | 紧凑内边距                                                                     |
    | `data-no-copy`    | boolean                                               | false     | 隐藏右上角复制按钮（默认显示）                                                 |
    | `style`           | 原生                                                  | 无        | 容器内联样式，自动安全清理（拦截 `position: fixed/sticky`、过大 `z-index` 等） |

  - 复制按钮当前复制内容区 `innerText`（v1），后续可升级为反向序列化 Markdown 源码。
...
- **外部工具调用 (VCP Support)**:
  ...
  - **VCP 角色分割**: 支持 `vcp_role`（user/assistant/system 角色容器）和 `vcp_daily_note`（日记容器）的结构化渲染。
- **LLM 思考过程**:
...
- **会话变量徽章 (`<svar>`)**: 原生支持 `<svar>` 标签，渲染为可交互的变量变更徽章（`SvarNode`），支持展示变量路径、操作符、新旧值变更，以及 tooltip 详情。
- **正则规则预处理**: 支持在渲染前对内容应用聊天正则规则（`regexRules`），实现内容的自动替换和转换，流式和静态模式均支持。

### 5.3 核心交互与渲染特性

- **流式渲染与更新节流**: 文本以打字机效果输出，并通过 AST 更新节流机制（默认 80ms）减少高频 patch 刷新。当前实现主要依赖 `requestAnimationFrame` 调度 flush，并在解析器忙碌时只保留最新 buffer 等待下一轮处理。
- **流式平滑化**: `StreamController` 通过内部缓冲区平滑 Token 发射节奏，避免网络抖动带来的视觉卡顿。支持自动加速和紧急冲刷。
- **渲染动画**: 新增了节点淡入（fade-in-up）动画效果。对于需要立即显示以填充内容的动态节点（如思考块、代码块、图表、VCP 工具、HTML 块、图片等），动画会被自动禁用。
- **代码高亮与应用沙箱**: 支持 **Monaco Editor** 和 **CodeMirror** 双引擎切换（通过 `codeEditorEngine` prop）。支持流式输出下的**增量追加 (Incremental Append)**，显著提升长代码块的打字机体验。对于 `html` 代码块，支持 **Seamless (无边框)** 渲染。通过 `IntersectionObserver` 实现代码块延迟初始化，进入视口前渲染轻量级 `PreCodeNode` 预览。
- **列表层渲染优化**: 长消息列表当前依赖 `MessageList.vue` 的消息级 `content-visibility: auto`。renderer 内部仅保留块级 class 标记，未默认启用块级 `content-visibility`。
- **动态样式系统**: 提供图形化界面，允许用户实时自定义所有 Markdown 元素的 CSS 样式，支持全局/Agent/User 三级样式合并。
- **稳定区/待定区策略**: 智能分离已完成和未完成的语法，确保流式输出的稳定性和正确性。
- **内存管理**: 全套组件卸载清理机制，防止长会话切换或频繁切换对话时的内存泄漏。
...
| `version`                  | `RendererVersion`              | `V2_CUSTOM_PARSER` | 渲染器版本                             |
...
| `enableCdnLocalizer`       | `boolean`                      | `true`             | 是否启用 CDN 资源本地化                |
| `throttleMs`               | `number`                       | `80`               | AST 更新节流时间（ms）                 |
...
| `smoothingEnabled`         | `boolean`                      | `true`             | 是否启用流式平滑化（StreamController） |
...
| `codeEditorEngine`         | `"monaco" \| "codemirror"`     | `"codemirror"`     | 代码编辑器引擎选择                     |
...
- **`codeEditorEngine`**: 代码编辑器引擎选择（`"monaco"` | `"codemirror"`）。
...
- **性能冻结策略**: `MessageContent.vue` 根据 `messageDepth` 和聊天设置计算 `shouldFreeze`，再传入渲染器冻结 HTML 预览沙箱，以释放内存和 CPU 资源。
- **样式合并**: 渲染器接收来自 `ChatSettings` 的全局样式，并根据当前智能体（Agent）的配置进行合并覆盖。
- **编辑器引擎选择**: 通过 `codeEditorEngine` prop 传入用户在设置中选择的代码编辑器引擎。
