...
- **零闪烁 Patch 更新**: 通过增量 Diff 算法和 Patch 系统，仅对受影响的 Vue 组件进行局部更新，彻底避免全量重绘带来的闪烁。
...
- **HTML 预览冻结**: 通过 `shouldFreeze` prop 控制，当消息深度超过阈值时冻结 HTML 预览以节省资源。弹窗打开时提供动态冻结原因提示和"关闭弹窗以恢复"按钮。
...
- **HTML 原生支持**:
  ...
  - **性能冻结 (Freeze) 策略**: 当 HTML 预览节点过多或处于非活动状态时，引擎会自动冻结其渲染以节省资源，弹窗打开时提供动态原因提示和恢复按钮。
- **折叠面板增强 (`<details>`)**:
...
- **块级渲染优化**: 块级节点自动应用 `content-visibility: auto`，显著提升长消息列表的滚动性能。
