...
3.  **视图层 (View Layer)**
    ...
    - **核心组件**:
      ...
      - **节点组件**: `MermaidNode`, `LlmThinkNode` 等负责各自节点的渲染。特别地：
        - `CodeBlockNode`: 它不仅仅是代码展示，而是一个基于 **Monaco Editor** 的微型代码查看器，提供了代码高亮、复制、动态字体调整、自动换行和展开/折叠等丰富的交互功能。
...
- **流式渲染**: 文本以打字机效果流畅输出，无整体刷新和闪烁。
- **代码高亮与交互**: 使用 **Monaco Editor** 作为渲染引擎，提供专业级的语法高亮。代码块组件还内置了**复制、字体缩放、自动换行切换、展开/折叠**等一系列交互功能。
