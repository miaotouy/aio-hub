...
- **对象图层 (ObjectLayer)**: 基于 Konva.js 的矢量节点系统，用于放置可独立变换的形状（矩形、椭圆、星形、线段、箭头、文本、图片）。对象数据以 JSON 格式持久化。
...
| 形状 | 矩形 / 椭圆 / 星形 / 线段 / 箭头 / 文字 / 图片 | R / O / S / L / A / T / I | 对象图层 |
...
- 修饰键: Ctrl+S（保存）、Ctrl+Shift+S（增量保存）、Ctrl+Z/Y（撤销/重做）、Ctrl+A（全选）、Ctrl+D（复制选中对象）、Ctrl+0（重置视图）、Ctrl+±（缩放）、Ctrl+[/]（对象层级调整）。
- 单字母: V/H/B/M/E/R/O/S/L/A/T（工具切换）、Delete/Backspace（删除选中）。
...
形状处于拖拽创建状态时，`KonvaCanvas` 会根据指针事件中的修饰键实时重新计算临时节点：

- `Shift`：矩形、椭圆与星形固定比例；线段和箭头约束到 45° 倍数。
- `Alt` / `Option`：将鼠标按下点作为形状中心；在线段和箭头中创建中心对称的两端。
- `Ctrl` / `Cmd`：临时移除矩形、椭圆和星形的填充，只显示轮廓。
- 滚轮：拖拽圆角矩形时调整圆角半径；拖拽星形时调整内角半径（尖角程度），并暂停常规视图缩放。

`Ctrl`/`Cmd`+`D` 会在原对象图层创建一个偏移 20px 的副本并选中副本。对象对齐、分布和通过右键菜单锁定/解锁均会记录历史，可撤销。
...
- 暴露方法：`getStage()`、`getCanvases()`、`resetView()`、`duplicateSelected()`、`collectObjectLayerData()` 等。
- 在临时形状绘制期间拦截滚轮，用于圆角与星形内角调节；其他情况下维持以指针为中心的视图缩放。
