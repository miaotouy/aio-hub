...
const logger = createModuleLogger("CloudOcrRunner");
const errorHandler = createModuleErrorHandler("CloudOcrRunner");
...
async function getBaiduAccessToken(apiKey: string, apiSecret: string): Promise<string> {
  ...
  if (cached && cached.expiresAt > Date.now()) {
    logger.debug("百度云 Access Token 缓存命中", {
      expiresAt: new Date(cached.expiresAt),
    });
    return cached.token;
  }

  logger.debug("百度云 Access Token 缓存未命中，请求新 token");
  ...
  try {
    ...
    if (!data.access_token) {
      errorHandler.error(new Error("access_token missing"), "百度云 access_token 响应缺失");
    ...
    logger.info("百度云 Access Token 获取成功", {
      expiresIn: `${Math.floor(expiresIn / 1000 / 60)} 分钟`,
    });

    return data.access_token;
  } catch (error) {
    errorHandler.error(error, "获取百度云 access_token 失败");
...
async function callBaiduOcr(
  ...
  try {
    ...
    if (!data.words_result || !Array.isArray(data.words_result)) {
      errorHandler.error(new Error("返回格式异常"), "百度云 OCR 返回格式异常", {
    ...
    errorHandler.error(error, "百度云 OCR 请求失败", { context: { endpoint } });
 ...
 * 调用自定义 OCR API
 */
async function callCustomOcr(imageBase64: string, profile: OcrProfile): Promise<string> {
  // 检查自定义配置是否存在
  if (!profile.apiRequest) {
    errorHandler.error(new Error("服务未配置"), "自定义 OCR 服务未配置", {
  ...
  try {
    ...
    errorHandler.error(error, "自定义 OCR 请求失败", {
      context: { profileName: profile.name },
    });
 ...
 * 使用云端 OCR 识别图片块
 */
async function recognizeWithCloudOcr(imageBase64: string, profile: OcrProfile): Promise<string> {
  logger.debug("开始云端 OCR 识别", {
  ...
  switch (profile.provider) {
    case "baidu": {
      if (!profile.credentials.apiKey || !profile.credentials.apiSecret) {
        errorHandler.error(new Error("凭证缺失"), "百度云 OCR 凭证缺失", {
          context: { profileName: profile.name },
        });
  ...
  const runCloudOcr = async (
    ...
    const processBlock = async (index: number, skipDelay: boolean = false) => {
      ...
      try {
        ...
        errorHandler.error(error, "图片块识别失败", {
          context: {
            current: index + 1,
            total: blocks.length,
            blockId: block.id,
