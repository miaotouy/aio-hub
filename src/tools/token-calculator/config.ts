...
const tokenCalculatorConfigManager = createConfigManager<TokenCalculatorConfig>(
  {
    ...
    mergeConfig: (defaultConfig, loadedConfig) => {
      return {
        inputPanelWidthPercent:
          loadedConfig.inputPanelWidthPercent ??
          defaultConfig.inputPanelWidthPercent,
        calculationMode:
          loadedConfig.calculationMode ?? defaultConfig.calculationMode,
        selectedModelId:
          loadedConfig.selectedModelId ?? defaultConfig.selectedModelId,
        maxDisplayTokens:
          loadedConfig.maxDisplayTokens ?? defaultConfig.maxDisplayTokens,
        version: "1.0.0",
      };
    },
  }
 ...
 * 加载配置
 */
export const loadTokenCalculatorConfig =
  async (): Promise<TokenCalculatorConfig> => {
    try {
      return await tokenCalculatorConfigManager.load();
    } catch (error) {
      errorHandler.handle(error as Error, {
        userMessage: "加载 Token Calculator 配置失败",
        showToUser: false,
      });
 ...
 * 保存配置
 */
export const saveTokenCalculatorConfig = async (
  ...
  try {
    ...
    errorHandler.handle(error as Error, {
      userMessage: "保存 Token Calculator 配置失败",
      showToUser: false,
    });
    throw error;
  }
 ...
 * 更新部分配置
 */
export const updateTokenCalculatorConfig = async (
  updates: Partial<TokenCalculatorConfig>
): Promise<TokenCalculatorConfig> => {
  try {
    return await tokenCalculatorConfigManager.update(updates);
  } catch (error) {
    errorHandler.handle(error as Error, {
      userMessage: "更新 Token Calculator 配置失败",
      showToUser: false,
    });
    throw error;
  }
