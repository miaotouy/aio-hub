...
sequenceDiagram
    ...
    alt 异步方法
        ...
        TM->>Tool: 调用方法（注入 ToolContext 作为第二参数）
        Tool->>TM: reportStatus() [可选]
        TM->>Store: 更新进度
        Tool-->>TM: 返回结果
...
export class TaskExecutor {
  ...
  async execute(toolName: string, args: Record<string, unknown>, context: AsyncTaskContext): Promise<string> {
    ...
    try {
      const result = await (method as Function).call(toolInstance, args, toolContext);
...
**重要说明**: 本项目使用 `ConfigManager` 进行持久化，而非 Tauri Store 插件。
...
export class TaskStore {
  private data: TaskStoreData | null = null;
  ...
  private async initialize(): Promise<void> {
    try {
      this.data = await configManager.load();
      logger.info("任务存储已初始化", { taskCount: Object.keys(this.data.tasks).length });
    } catch (error) {
      logger.error("任务存储初始化失败", error);
  ...
  async saveTask(task: AsyncTaskMetadata): Promise<void> {
    ...
    await configManager.save(this.data!);
  }
  ...
  async updateTask(taskId: string, updates: Partial<AsyncTaskMetadata>): Promise<void> {
    ...
    this.data!.tasks[taskId] = { ...task, ...updates };
    await configManager.save(this.data!);
  }
...
- 使用 ConfigManager 存储任务记录（项目标准持久化方案）
- 应用重启后保留历史记录
...
使用 ConfigManager，文件路径：`{appDataDir}/tool-calling/async-tasks.json`
...
本设计方案已于 **2026-03-07** 全面完成实施，所有核心功能均已上线并通过测试。

### 11.2. 核心成果

#### Phase 1: 核心基础设施 ✅

- ✅ 扩展 `MethodMetadata` 类型定义（`src/services/types.ts`）
- ✅ 实现 `TaskManager` 和 `TaskStore`（`core/async-task/`）
- ✅ 实现 `AsyncTaskStore` (Pinia)（`stores/asyncTaskStore.ts`）
- ✅ 修改 `executor.ts` 支持异步模式检测

#### Phase 2: 协议、发现与上下文注入 ✅

- ✅ 复用现有 VCP 协议格式，无需额外扩展
- ✅ 修改 `discovery.ts` 在提示词中标注异步方法
- ✅ 实现内置的 `getTaskStatus`、`cancelTask`、`retryTask` 系统方法
- ✅ 在 `llm-chat/core/context-processors` 中实现 `async-task-processor.ts`

#### Phase 3: UI 组件 ✅

- ✅ 创建 `AsyncTaskMonitor.vue` 主组件
- ✅ 创建 `TaskTable.vue` 和 `TaskToolbar.vue`
- ✅ 创建 `TaskDetailDialog.vue`
- ✅ 在 `ToolCallingTester.vue` 中集成新 Tab

#### Phase 4: 测试与优化 ✅

- ✅ 创建测试用的异步工具方法（`testAsyncTask`）
- ✅ 测试取消、重试、进度汇报功能
- ✅ 测试应用重启后的状态恢复（`interrupted` 状态）
- ✅ 性能优化（响应式轮询机制）

### 11.3. 代码质量优化（2026-03-07）

在实施完成后，进行了以下代码质量优化：

1. **消除重复代码**：
   - 创建 `core/utils/tool-parser.ts` 统一工具名称解析逻辑
   - 在 `executor.ts`、`task-manager.ts`、`task-executor.ts` 中复用

2. **优化初始化流程**：
   - 移除 `ToolCallingRegistry.initialize()` 中的重复调用
   - 统一由 `asyncTaskStore.initialize()` 负责中断任务标记

3. **完善 AsyncTaskContext**：
   - 添加 `log` 方法，支持任务内部日志记录
   - 保留 `checkCancellation` 方法用于取消检测
...
1. **协议对齐**：完全复用现有的 `[[AIO工具调用结果信息汇总]]` 格式，无需维护新的标记语法
2. **上下文注入**：通过 Context Processor 实现 LLM 对任务状态的"隐式感知"
3. **响应式设计**：Pinia Store + 轮询机制，确保 UI 实时更新
4. **持久化策略**：应用重启后保留历史，`running` 任务标记为 `interrupted` 可重试
5. **向后兼容**：不破坏现有同步工具的调用方式
...
- ✅ 应用重启后的状态恢复
- ✅ UI 组件交互与数据绑定
- ✅ Context Processor 动态结果注入

### 11.6. 参考文档

详细的实施过程和技术细节，请参考：

- [Phase4 实施总结](./Phase4-实施总结.md)
- [异步任务系统 README](../../core/async-task/README.md)
- [组件使用文档](../../components/README.md)

### 11.7. 持久化问题修复（2026-03-07）

在实施过程中发现了持久化方案的错误使用，已完成修复：

#### 问题描述

错误地使用了 `@tauri-apps/plugin-store` 进行数据持久化，导致应用启动时出现 `store.load not allowed. Plugin not found` 错误。

#### 根本原因

项目从未使用 Tauri Store 插件，所有持久化都通过 `ConfigManager` 实现。

#### 修复方案

1. **重写 TaskStore**：使用 `createConfigManager` 替代 `Store`
2. **添加初始化等待**：为 `TaskStore` 和 `TaskManager` 添加 `waitForInitialization()` 方法
3. **修复服务注册**：将 `tool-calling.registry.ts` 的默认导出从实例改为类
4. **更新 asyncTaskStore**：在 Pinia Store 初始化中等待 TaskManager

#### 数据存储位置

修复后，异步任务数据存储在：`{AppData}/tool-calling/async-tasks.json`

#### 验证结果

- ✅ 前端类型检查通过
- ✅ 使用项目标准的 ConfigManager 进行持久化
- ✅ 与项目其他模块的持久化方式保持一致
- ✅ 不再依赖未安装的 Tauri Store 插件

详细修复过程请参考：[持久化问题修复总结](./持久化问题修复总结.md)

### 11.8. 后续优化方向

- ⏳ VCP 分布式支持：异步任务在远程节点的执行
- ⏳ 并发控制：限制同时运行的异步任务数量
- ⏳ 优先级队列：支持任务优先级调度
- ⏳ 更丰富的进度可视化：支持子任务进度、日志流式展示
...
在执行沙盒的结果卡片 (`ExecutionResultCard.vue`) 中增加了异步任务状态监控面板，当检测到工具调用结果包含 `taskId` 时，自动显示任务的实时状态。

### 12.2. 核心特性

- ✅ **自动检测**: 使用 `extractTaskId` 工具函数从结果 JSON 中提取任务 ID
- ✅ **实时轮询**: 每 2 秒自动更新任务状态，任务完成后自动停止
- ✅ **状态展示**: 显示任务状态、进度条、进度消息、耗时等信息
- ✅ **交互操作**: 提供取消和重试按钮（根据任务状态动态显示）
- ✅ **视觉反馈**:
  - 运行中状态显示旋转动画
  - 不同状态使用不同颜色和图标
  - 进度条实时更新
- ✅ **结果展示**: 任务完成后显示完整结果，失败时显示错误信息

### 12.3. 实现细节

#### 12.3.1. 组件增强

在 `ExecutionResultCard.vue` 中添加：

1. **状态管理**:
   - `taskId`: 提取的任务 ID
   - `task`: 从 Store 获取的任务元数据
   - `pollTimer`: 轮询定时器

2. **生命周期**:
   - `onMounted`: 检测 taskId 并启动轮询
   - `onUnmounted`: 清理轮询定时器

3. **计算属性**:
   - `taskStatusIcon`: 根据状态返回对应图标
   - `taskStatusColor`: 根据状态返回对应颜色
   - `taskStatusText`: 状态文本映射
   - `elapsedTime`: 计算已用时间
   - `showCancelButton`: 是否显示取消按钮
   - `showRetryButton`: 是否显示重试按钮
...
| 状态        | 图标           | 颜色    | 文本     |
| ----------- | -------------- | ------- | -------- |
| pending     | Clock          | info    | 等待执行 |
| running     | Loader2 (旋转) | primary | 执行中   |
| completed   | CheckCircle2   | success | 已完成   |
| failed      | XCircle        | danger  | 失败     |
| cancelled   | Ban            | warning | 已取消   |
| interrupted | XCircle        | warning | 已中断   |
...
1. ✅ 打开工具调用测试页面
2. ✅ 切换到执行沙盒 Tab
3. ✅ 选择 `tool-calling_testAsyncTask` 方法
4. ✅ 设置参数并触发调用
5. ✅ 观察结果卡片显示异步任务状态面板
6. ✅ 验证进度条实时更新
7. ✅ 验证任务完成后状态变化
...
1. **响应式集成**: 完全集成 Pinia Store，状态变化自动反映到 UI
2. **智能轮询**: 任务完成后自动停止轮询，避免资源浪费
3. **用户友好**: 提供直观的视觉反馈和交互操作
4. **代码复用**: 复用现有的 `extractTaskId` 和 `asyncTaskStore`
