...
4. **时间中立**：**绝对禁止评价内容中的时间信息**。即便内容中出现的时间在你看来是“未来”或“不可能”的（例如 2026 年或更晚），也必须视为当前现实并如实记录。严禁发表诸如“这是未来的时间”、“这不符合逻辑”等任何基于你知识库检查点的评价。
5. **结构清晰**：输出采用层次分明的 Markdown 格式。

## 输出要求
- 使用中文输出描述（除非内容本身是其他语言）。
- 严禁提及任何基于“常识”或“过时知识”推断出的细节（如：未标明身份的角色名、未出现的品牌背景等）。
- **禁止评价内容真实性**：严禁对识别到的文字、日期、数据进行真伪评价或合理性质疑。
...
## 核心准则
- **视觉实证**：仅描述图像中明确可见的内容。严禁利用训练知识去“脑补”或“猜测”图中未明确标注的信息（例如：严禁猜测未注名的角色身份、品牌背景或地理位置）。
- **时间脱敏**：**严禁评价图中出现的任何日期和时间**。无论图中显示的是哪一年（即使是 2026, 2030 等），请直接作为事实转录。禁止以“这不符合现实”或“这是未来时间”为由进行任何说明或评价。
- **客观中立**：保持描述的客观性，不添加主观臆断，不质疑内容的合理性。
...
### 2. 核心元素
- **主体识别**：画面中的主要对象、人物、物品（如身份不明，仅描述外貌/形态特征）
- **空间关系**：各元素的位置布局和相互关系
- **视觉特征**：色彩、光影、构图等显著特点
...
使用清晰的 Markdown 结构，必要时使用表格、列表等元素增强可读性。`,
  ...
  audio: {
    modelIdentifier: "",
    customPrompt: `## 任务
执行音频内容分析。正在处理音频：{filename}。精通语音识别、歌词转录及音乐理论分析。

## 分析维度

### 1. 语音与歌词（文本层）
- **语音对话**：准确转录对话内容，区分说话者，保留语气情感。
- **歌词转录**：如果是歌曲，按行/段落准确转录歌词，保留原语言。
...
`,
  ...
  video: {
    modelIdentifier: "",
    customPrompt: `## 任务
执行视频内容分析。正在处理视频：{filename}。具备对动态视觉内容和音频信息的综合理解能力。

## 核心准则
- **视觉实证**：仅描述视频中肉眼可见的画面和耳朵可听的音频。禁止使用外部知识库来填充视频中未明确说明的背景、人物身份或事件。
- **时间绝对中立**：**严禁对视频中出现的任何时间点、年份、日期进行合理性评价**。禁止提及“该时间位于未来”或“时间信息有误”等引导性评论。即便你认为当前是 2024 年而视频显示 2026 年，也请直接记录 2026 年，不要发表任何看法。
- **拒绝猜测**：如果画面中出现新角色、新产品且未通过文字/对白标明身份，仅描述其外观特征，严禁通过"猜测"冠名。
...
### 2. 关键视觉信息
- **场景变化**：描述主要的场景切换和环境细节
- **人物与动作**：识别主要人物及其关键动作（如身份不明，描述特征而非猜测姓名）
- **文字与标识**：提取视频中出现的关键文字（字幕、标题、招牌等）
- **演示文稿与文本提取**：如果视频包含 PPT、Keynote、白板或其他演示文档，务必完整、准确地提取页面上的文本内容、标题、核心要点。
...
## 输出要求
使用清晰的 Markdown 结构。
确保内容是可以以文字尽量还原视频内容，给LLM看的，可以近似理解为给视障或听障对象转写内容。
`,
    temperature: 0.2,
    maxTokens: 4096,
    maxDirectSizeMB: 10,
...
3. **表格还原**：对于文档中的表格，使用 Markdown 表格语法进行完美还原。
4. **数学公式**：如果文档中包含数学公式，使用 LaTeX 语法（$ 或 $$）进行转录。
...
export const transcriptionSettingsConfig: SettingsSection<TranscriptionConfig>[] = [
  {
    ...
    items: [
      ...
      {
        id: "repetitionConsecutiveThreshold",
        label: "连续重复阈值 ({{ localSettings.repetitionConfig.consecutiveThreshold }}次)",
        component: "SliderWithInput",
        props: { min: 2, max: 10, step: 1 },
        modelPath: "repetitionConfig.consecutiveThreshold",
        hint: "同一行或同一句话连续出现的次数超过此值将视为复读",
        keywords: "repetition threshold consecutive 复读 阈值 连续",
        visible: (s) => s.enableRepetitionDetection,
      },
      {
        id: "repetitionGlobalThreshold",
        label: "全局片段阈值 ({{ localSettings.repetitionConfig.globalThreshold }}次)",
        component: "SliderWithInput",
        props: { min: 2, max: 20, step: 1 },
        modelPath: "repetitionConfig.globalThreshold",
        hint: "文本中任意片段出现的总次数超过此值将视为复读",
        keywords: "repetition threshold global 复读 阈值 全局",
        visible: (s) => s.enableRepetitionDetection,
      },
      {
        id: "repetitionWhitelist",
        label: "复读白名单",
        component: "ElSelect",
        props: {
          multiple: true,
        ...
        modelPath: "repetitionConfig.whitelist",
    ...
    items: [
      {
        ...
        label: "图片模型",
        component: LlmModelSelector,
        props: { capabilities: { vision: true, embedding: false, rerank: false } },
    ...
    items: [
      {
        ...
        label: "音频模型",
        component: LlmModelSelector,
        props: { capabilities: { audio: true, embedding: false, rerank: false } },
      ...
      {
        id: "audioEnableCompression",
        label: "启用音频压缩",
        layout: "inline",
        component: "ElSwitch",
        modelPath: "audio.enableCompression",
        hint: "开启后，当音频体积超过限制时，将尝试自动压缩以减少上传体积（需配置 FFmpeg）。",
        keywords: "audio compression 压缩",
      },
      {
        id: "audioMaxDirectSizeMB",
        label: "体积限制 ({{ localSettings.audio.maxDirectSizeMB }}MB)",
        component: "SliderWithInput",
        props: { min: 1, max: 50, step: 1 },
        modelPath: "audio.maxDirectSizeMB",
        hint: "音频体积限制，超过将尝试压缩。",
        keywords: "audio size 体积 限制",
      },
      {
        id: "audioBitrate",
        label: "目标比特率",
        ...
        modelPath: "audio.bitrate",
        hint: "压缩后的音频比特率。较低的比特率可以显著减小体积，但会损失音质。",
        keywords: "audio bitrate 比特率",
        visible: (s) => s.audio.enableCompression,
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        label: "视频模型",
        component: LlmModelSelector,
        props: { capabilities: { video: true, vision: true, embedding: false, rerank: false } },
      ...
      },
      {
        ...
        layout: "inline",
        component: "ElSwitch",
        modelPath: "video.enableGpu",
        hint: "使用 NVIDIA GPU (h264_nvenc) 加速压缩，需硬件支持",
        keywords: "gpu hardware acceleration 硬件加速",
        visible: (s) => s.video.enableCompression,
      },
      {
        id: "autoAdjustResolution",
        label: "自动调整分辨率",
        layout: "inline",
        component: "ElSwitch",
        modelPath: "video.autoAdjustResolution",
        hint: "当目标体积过小时，自动降低分辨率以保持画质清晰度",
        keywords: "auto resolution 自动 分辨率",
        visible: (s) => s.video.enableCompression,
      },
    ],
  },
    ...
    items: [
      {
        ...
        label: "文档模型",
        component: LlmModelSelector,
        props: { capabilities: { document: true, vision: true, embedding: false, rerank: false } },
