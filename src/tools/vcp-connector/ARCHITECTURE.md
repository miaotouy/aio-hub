...
消息监控中的六类 VCP 消息都可以独立选择通知输出方式：`关闭`、`仅浮动`、`通知中心` 或 `同时`。

- `notificationMode` 是通知总设置，作为未覆盖消息类型的默认值；默认值为 `通知中心`，以避免高频 WebSocket 事件打断操作。
- `notificationModes` 是按 `VcpMessageType` 保存的可选覆盖；移除覆盖后，该类型立即恢复跟随总设置。
- 通知策略只控制 `customMessage` 浮动提示与 AIO Hub 通知中心写入，不影响 VCP 消息接收、监控列表、统计或历史持久化。
- 通知中心条目附带 `vcpMessageType` 元数据，便于后续按来源和消息类型扩展筛选。
...
分布式工具注册能力声明 `cancelTool`，协议消息类型支持 `cancel_tool`，服务端可按 `requestId` 取消在途执行。`VcpNodeProtocol` 为每个请求维护 `AbortController`，分布式超时会先 abort 再返回错误；WebSocket 断开时批量终止该节点的所有在途调用，并在 finally 中清理映射。

远程工具若需要真正释放文件扫描、网络请求或其他长任务资源，必须消费传入的 `ToolContext.signal`；仅依赖 VCP 返回错误不能保证本地任务停止。
