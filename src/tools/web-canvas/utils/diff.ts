...
export function applySearchReplaceDiff(
  ...
  const findBestMatchIndex = (
    ...
    return indices.reduce((prev, curr) => {
      return Math.abs(curr - targetIndex) < Math.abs(prev - targetIndex)
        ? curr
        : prev;
    });
  };
  ...
  while (lastExactIndex !== -1) {
    const lineIndex =
      originalContent.substring(0, lastExactIndex).split(/\r?\n/).length - 1;
  ...
  if (exactIndices.length > 0) {
    ...
    if (exactIndices.length > 1) {
      if (options.startLine) {
        warnings.push(
          `匹配到 ${exactIndices.length} 处，已根据行号提示选择第 ${matchedIndex + 1} 行的匹配`
        );
      } else {
        warnings.push(
  ...
  const tryLineMatch = (strategy: "trimEnd" | "trim"): DiffResult | null => {
    ...
    const processLine = (l: string) =>
      strategy === "trim" ? l.trim() : l.trimEnd();
    const processedSearchLines = searchLines.map(processLine);

    for (
      let j = 0;
      j <= resultLines.length - processedSearchLines.length;
      j++
    ) {
      let match = true;
    ...
    if (indices.length > 0) {
      ...
      const originalIndentation =
        resultLines[matchedIndex].match(/^\s*/)?.[0] || "";
      const searchFirstLineIndentation =
        searchLines[0].match(/^\s*/)?.[0] || "";

      const fixedReplaceLines = replaceLines.map((line) => {
        if (line.startsWith(searchFirstLineIndentation)) {
          return (
            originalIndentation +
            line.substring(searchFirstLineIndentation.length)
          );
        }
        return line;
      });
      ...
      if (indices.length > 1) {
        if (options.startLine) {
          warnings.push(
            `匹配到 ${indices.length} 处，已根据行号提示选择第 ${matchedIndex + 1} 行的匹配`
          );
        } else {
          warnings.push(
            `匹配到 ${indices.length} 处，已替换第一处。建议提供 startLine 参数或更精确的上下文以消歧义`
          );
        }
      }
  ...
  for (let j = 0; j <= resultLines.length - searchLines.length; j++) {
    ...
    const averageScore =
      totalScore / (searchLines.length + (searchLines.length > 1 ? 0.4 : 0));
  ...
  if (bestScore >= fuzzyThreshold) {
    const matchedIndex = findBestMatchIndex(
      candidateMatches
        .filter((m) => m.score >= fuzzyThreshold)
        .map((m) => m.index),
      options.startLine
    );
    ...
    const actualBestScore =
      matchedIndex !== -1
        ? candidateMatches.find((m) => m.index === matchedIndex)!.score
        : bestScore;
    ...
    const warnings: string[] = [
      `模糊匹配成功 (置信度: ${(actualBestScore * 100).toFixed(0)}%)`,
    ];

    const duplicateCount = candidateMatches.filter(
      (m) => m.score >= fuzzyThreshold
    ).length;
    if (duplicateCount > 1) {
