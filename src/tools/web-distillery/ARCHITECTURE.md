...
> **当前状态**：P0-P1 已完成。P2 (交互模式) 基建已到位，交互落地工程待设计。P3-P4 数据层已实现，UI 层与闭环交互仍处于实验性阶段。

---

## ⚠️ 核心警告：文档与实现同步说明

本工具正处于从“模拟浏览器”向“Iframe 代理”架构重构的过渡期。

1. **ARCHITECTURE.md** 中部分描述（如“智能路径推断”、“DOM 冻结”）属于**设计目标**，目前尚未实现。
2. **交互模式**目前仅支持查看网页、Cookie 提取和 API 嗅探，可视化规则拾取（Selector Picker）链路正在修复中。
3. 请参考 `design/Plan/INTERACTIVE-MODE-AUDIT.md` 了解最新的审计结果。
...
## 2. 获取模式设计 (Distillation Modes)

网页蒸馏室提供三种核心模式，覆盖从极速获取到深度交互的不同场景。已移除旧版 Level 0/1/2 分层，统一为以下模式：

### 2.1 Quick Mode (快速模式)

**无浏览器渲染**。Rust 端直接使用 `reqwest` 发起 HTTP 请求。

```
Agent 调用 → Rust reqwest → 原始 HTML → 清洗管道 → 返回结果
```
...
优势：毫秒级响应、资源消耗极低、支持高并发。

### 2.2 Render Mode (渲染模式)

**后端代理 + 隐藏 Iframe 渲染**。通过本地 Rust 代理服务器注入脚本并绕过跨域限制，在前端 Iframe 中完成 JS 渲染。

```
Agent 调用 → Rust Proxy (注入脚本) → 隐藏 Iframe 渲染 → 自动提取 DOM → 销毁 Iframe → 返回结果
```

适用场景：

- SPA / CSR 动态渲染页面（React, Vue 等）
- 需要执行 JS 才能看到的内容
- 快速模式质量分数低时的自动升级

优势：无需创建独立窗口，UI 稳定，通信延迟极低。

### 2.3 Interactive Mode (交互模式) [Developing]

**可视化工作台（设计中）**。在 UI 中直接嵌入 Iframe。当前实现：可见 Iframe 创建正常、API Sniffer 可用、Cookie Lab V1 可用。**"手术刀"标注工具（selector-picker）因 IPC 通信方式与当前 Iframe 架构不兼容，链路断裂，尚不可用。**

适用场景：

- 复杂页面，自动识别效果不佳
- [Planned] 需要用户手动选择元素、配置配方（待交互设计落地）
- 需要 API Sniffer 发现隐藏数据接口（✅ 已可用）
...
```typescript
// Quick Mode
async function quickFetch(options: QuickFetchOptions): Promise<FetchResult>;

// Render Mode
async function renderExtract(options: RenderExtractOptions): Promise<ExtractResult>;

// Interactive Mode
async function openInteractive(url?: string): Promise<void>;
```
...
### 4.1 Iframe + Proxy 方案 (Render/Interactive 共用)

> ✅ **架构升级**：为解决旧版独立 Webview 窗口带来的 IPC 延迟和 UI 同步撕裂问题，V2 架构全面转向 **"后端代理 + 前端 Iframe"**。

**工作原理**：

1. **Distillery Proxy (Rust)**：在本地 Axum 服务器启动代理路由，转发目标网页请求。
2. **内容注入**：代理在返回 HTML 时自动注入 `<base>` 标签（修复相对路径）和核心脚本（Bridge/Sniffer）。
3. **安全降级**：后端自动移除 `X-Frame-Options` 和 `CSP` 限制，允许页面在同源 Iframe 中嵌入。
4. **零延迟通信**：由于 Iframe 与主应用同源（localhost），直接使用 `window.postMessage` 通信，彻底告别 Tauri 事件总线。
...
| **P1**  | Iframe + Proxy 架构重构                                  | ✅   | 解决跨域与 UI 同步问题          |
| **P2**  | Level 1: `smartExtract`（隐藏 Iframe 渲染 + 自动提取）   | ✅   | 已上线                          |
| **P3a** | Site Recipe 系统：动作回放、配方存储与匹配               | ⚠️   | 数据层 ✅，可视化配置工具待完善 |
| **P3b** | 交互模式落地：可视化规则拾取与实时预览                   | ⏳   | 核心交互设计中                  |
| **P4**  | API Sniffer + Cookie Lab（v1: 基于 JS 注入）             | ✅   | 数据层 ✅，交互层未完成         |
