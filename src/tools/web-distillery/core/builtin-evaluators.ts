...
export function githubRepoEvaluator(doc: Document, url: URL): string | null {
  ...
  const repoNameEl =
    doc.querySelector("strong[itemprop='name'] a") ||
    doc.querySelector("[itemprop='name']");
  if (repoNameEl) {
    md += `# GitHub Repository: ${repoNameEl.textContent?.trim()}\n\n`;
  }
  const aboutEl =
    doc.querySelector("p.f4") || doc.querySelector(".BorderGrid-cell p");
  if (aboutEl) {
  ...
  const fileRows = Array.from(
    doc.querySelectorAll("tr.react-directory-row, div.react-directory-row")
  );
  if (fileRows.length > 0) {
    md += `## 文件列表\n`;
    fileRows.forEach((row) => {
      const nameEl = row.querySelector(
        ".react-directory-truncate a, a.Link--primary"
      );
      if (nameEl && nameEl.textContent) {
        const isDir =
          row.querySelector("svg.icon-directory") ||
          row.querySelector('[aria-label="Directory"]');
    ...
    const fileLinks = Array.from(
      doc.querySelectorAll(".js-navigation-item .js-navigation-open")
    );
  ...
  if (pathname.includes("/blob/")) {
    const blobTextArea = doc.querySelector(
      "textarea#read-only-cursor-text-area"
    ) as HTMLTextAreaElement | null;
    if (blobTextArea && blobTextArea.value) {
      const fileNameEl =
        doc.querySelector("[data-testid='breadcrumbs-filename']") ||
        doc.querySelector("#blob-path");
      ...
      // 回退：尝试获取代码区域
      const codeArea =
        doc.querySelector(".js-file-line-container") ||
        doc.querySelector("table[data-paste-markdown-skip]");
      if (codeArea) {
 ...
 */
export function githubIssueListEvaluator(
  doc: Document,
  _url: URL
): string | null {
  const issueRows = doc.querySelectorAll(
    "[data-testid='issue-row'], .js-issue-row"
  );
  ...
  issueRows.forEach((row) => {
    const titleEl = row.querySelector(
      "[data-testid='issue-title-link'], .js-navigation-open"
    );
