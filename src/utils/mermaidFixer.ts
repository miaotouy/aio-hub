...
export function fixMermaidCode(code: string): string {
  ...
  const fixedLines = lines.map((line) => {
    ...
    // 跳过图表类型声明行
    if (
      /^(flowchart|graph|sequenceDiagram|classDiagram|stateDiagram|erDiagram|gantt|pie|journey|gitGraph|mindmap|timeline|quadrantChart|sankey|xychart)\b/i.test(
        trimmedLine
      )
    ) {
      return line;
...
function fixSubgraphLine(line: string, sensitiveChars: RegExp): string {
  ...
  // 匹配 subgraph ID["..."] 格式
  const bracketLabelMatch = line.match(
    /^(\s*subgraph\s+)([A-Za-z0-9_]+)(\[")(.*?)("\].*)$/
  );
  if (bracketLabelMatch) {
    const [, prefix, id, openBracket, content, closeBracket] =
      bracketLabelMatch;
  ...
  if (unquotedTitleMatch) {
    ...
    if (sensitiveChars.test(trimmedTitle) || /\s/.test(trimmedTitle)) {
      const fixedTitle = trimmedTitle.replace(/\(/g, "（").replace(/\)/g, "）");
