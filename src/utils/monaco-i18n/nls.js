 ...
 * Version: 0.54.0 (Official zh-cn messages integrated by Gugu)
 *-----------------------------------------------------------*/
...
function localize(data, message, ...args) {
  ...
  // 1. 如果 data 是数字，说明是 Index 模式（Monaco 内部常用）
  if (typeof data === "number") {
    if (ZH_CN_MESSAGES[data]) {
      translated = ZH_CN_MESSAGES[data];
    }
  }
  // 2. 如果 data 是对象，尝试通过 key 匹配（虽然 Index 模式更可靠）
  else if (data && typeof data === "object" && typeof data.index === "number") {
    if (ZH_CN_MESSAGES[data.index]) {
      translated = ZH_CN_MESSAGES[data.index];
    }
  }
...
function localize2(data, message, ...args) {
  return {
    value: localize(data, message, ...args),
    original: _format(message, args),
  };
}
...
function getNLSLanguage() {
  return globalThis._VSCODE_NLS_LANGUAGE || "zh-cn";
}
...
// 挂载到全局，供一些非 ESM 模块使用
globalThis._VSCODE_NLS_MESSAGES = ZH_CN_MESSAGES;
globalThis._VSCODE_NLS_LANGUAGE = "zh-cn";

// 为了兼容 AMD 加载器，这里不使用 export 关键字
// ESM 模式下通过 import "@/utils/monaco-i18n/nls" 导入即可初始化全局变量
globalThis.localize = localize;
globalThis.localize2 = localize2;
globalThis.getConfiguredDefaultLocale = getConfiguredDefaultLocale;
globalThis.getNLSLanguage = getNLSLanguage;
globalThis.getNLSMessages = getNLSMessages;
