...
describe("Knowledge cross-tool smoke", () => {
  it("creates an isolated library through the real Tauri UI", async () => {
    ...
    if (!(await existingLibrary.isExisting())) {
      const createButton = await $('[data-testid="knowledge-create-library-empty"]');
    ...
    if (!libraryId) throw new Error("Knowledge library did not expose a stable ID");
  });
  ...
  it("persists Agent Knowledge access through the real editor", async () => {
    ...
    const libraryId = await libraryRow.getAttribute("data-library-id");
    if (!libraryId) throw new Error("Knowledge library ID is required for Agent access");

    await navigateTo("/agent-manager");
    await $('[data-testid="agent-manager"]').waitForDisplayed({ timeout: 30_000 });
    ...
    for (const selector of [
      ...
      if (!(await checkbox.isSelected())) {
        throw new Error(`Agent Knowledge switch was not persisted: ${selector}`);
      }
    }
    await $(
  ...
  it("loads Agent Manager and Chat from the same isolated instance", async () => {
    ...
    await $('[data-testid="chat-message-input"]').waitForDisplayed({ timeout: 30_000 });
    await $('[data-testid="chat-knowledge-reference-button"]').waitForExist();
  });
  ...
  it("starts with the deterministic local model mock available", async () => {
    ...
    if (!(await profile.getText()).includes("E2E Local Mock")) {
      throw new Error("The isolated model profile was not loaded by the application");
    }
    ...
    if (!response.ok) throw new Error(`Local model mock returned ${response.status}`);
